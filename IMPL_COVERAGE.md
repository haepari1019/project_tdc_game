...
- **P4b 이관 진행:** M0 데이터위생 · M0b 결속키 · M1 마석 · M2 참 · **CS-1 Passive폐기/참일원화** · **CS-2 F-030 doctrine(골격+Trait 런타임)** · **M3 스킬 트리**(DRIFT-150) · **M4 gear 슬롯 귀속·D2 소멸 + 건 모딩 UI + per-kill 스킬북 드롭 폐지→공유 재료**(DRIFT-151) 완료. **M5 구 경제·affix·탄 폐기 + 마석 일원화**(DRIFT-152) 완료. 다음 = **M6 허브 화면 재구성**(UI-005 §3 · UX 정비) → M7 잭팟(선택). **CS-3 F-005 §3.3a NC 변조**는 독립. M4 판정 4건 ✅전파(spec `890bf73`·`DEC-20260820-001`, 재핀 완료). M5 잔여 `PENDING-PROP` 1건 = **모딩 시술비**(`F-008` §3.10 「재구매」 구체화) → M6 전 `OPS_30`. 작업 지시서 [docs/plan/P4b_WORK_ORDER.md](docs/plan/P4b_WORK_ORDER.md) §6.5.
