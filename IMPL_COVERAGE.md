...
- spec_ref_pin: `2bf37b2` (`staging`, 2026-07-09; **결속 정본=구현** DEC-20260709-001 + **정체성↔결속 대원칙** DEC-20260709-002(ROLE-000 §C-4) + 8 키스톤 본문 + ROLE §4.5(4역할)·CombatContentMap Kit Bindings·F-020 §3.3·D-016 castTier + DRIFT-073~077 역전파. 그 전: b9b0a96 IDA 프리픽스 DEC-20260705-001)
...
- **spec_ref_pin 갱신: `fd317b3`** (`staging`, 2026-08-14) — 이전 `09e045c`. 사이 구간: `DEC-20260812-004`(DPS 스타터 AB-028→AB-053) · `DEC-20260813-001`(결속 키 = bindingProfileId 기본값 = identity · Tank 규약 2종 「진격」/「응보」) · `DEC-20260813-002`(IDA-052 둔화·응보 시간소멸·진격 순서) · **`DEC-20260813-003/004/005`(성장 개편 — Passive 폐기·참 일원화 / F-030 doctrine / F-005 §3.3a)**.
- **P4b 이관 진행:** M0 데이터위생 · M0b 결속키 · M1 마석 · M2 참 · **CS-1 Passive폐기/참일원화** · **CS-2 F-030 doctrine(골격+Trait 런타임)** · **M3 스킬 트리**(DRIFT-150) · **M4 gear 슬롯 귀속·D2 소멸 + 건 모딩 UI + per-kill 스킬북 드롭 폐지→공유 재료**(DRIFT-151) 완료. **M5 구 경제 폐기·마석 일원화**(DRIFT-152) · **M6 마을 화면·건물 재편·의뢰 수락·난이도 폐기**(DRIFT-153) 완료. **P4b 본선 M0~M6 전부 종료** — 남은 건 **M7 잭팟(선택)**과 독립 건인 **CS-3 F-005 §3.3a NC 변조**. 판정 전파 3회 완결(`DEC-20260820-001/002` · `DEC-20260821-001`, 재핀 `660706e`). **대장간 사다리 도달 가능화 완료**(DRIFT-154 · `DEC-20260821-002`, 재핀 `1bab866`) — T1 문턱을 초반 재료로 내리고 **T3를 Expansion → Slice-01**로 편입(`Q-HUB-032` 심층 노두). 3칸 gear의 세 번째 칸이 이제 도달 가능하다. **플테 확인 대기:** 새 사다리 체감 속도(T1≈3런 · T2≈5 Deep 방문 · T3 심층 클리어) — 수치는 placeholder. 작업 지시서 [docs/plan/P4b_WORK_ORDER.md](docs/plan/P4b_WORK_ORDER.md) §6.5.
- **🚨 QA-032 §2.1 회귀 게이트가 CI에 상주**(`tools/nc_baseline_smoke.gd`, ci_smoke 12번째). CS-3이 8단계 훅을 넣는 순간부터 카운터가 실물이 되어 doctrine 0에서 **호출 0 + 항등 통과**를 강제한다 — 깨지면 doctrine 전체 롤백(F-030 §3.7 R2). 전체 ENC 바이트 리플레이는 현 하네스로 불가(도구 헤더에 한계 명시) → **F5 수동 확인 병행 필요**.
- **CS-1 특기:** 게임에 Passive 구현이 **애초에 없어** 체크리스트 9항 중 1-1~1-4는 무작업. 실작업 = 조건부 참 훅 + CHARM-PROTO-006/007. 판정 3건(chapel 이연 / threat 참 역할한정 / trapDetection 보류) = DRIFT-147.
