...
## 샌드박스 소환 가능 오브젝트 레지스트리 — 새 오브젝트 = 여기 1줄(label + script). 소환은 정중앙 +
## _spawned_objects 추적("오브젝트 리셋"이 제거). setup(_combat)은 has_method로 자동(torch만 필요).
const SPAWNABLE_OBJECTS := [
	{"label": "배럴 (oil · breakable)", "script": Barrel},
	{"label": "토치 (carriable · lit)", "script": Torch},
]
...
const IDENTITY_VERIFY := {
	...
	"sentinel_form": "거북이 태세 — 받는 피해 감소(DR) + **둔화**(구 이동 잠김). 6m 내 적 있을 때만 발동.\n• HP가 평소보다 훨씬 천천히 깎임\n• 굼떠지되 이동은 가능(둔화 −55%) + [파랑] 펄스\n• 받는 피해의 40%를 공격자에게 반사\n• 「응보」 결속: 태세 중 맞은 만큼 누적 → Q/E/R로 방출(마지막 피격 후 6s 유지)",
...
func _build_ui() -> void:
	...
	# dungeon_run 패리티(전투 피드백) — 피격 글로우·벽 투시·분리 경고·PIP
	_damage_indicator = DamageIndicator.new()
	layer.add_child(_damage_indicator)
	_combat.party_hit.connect(_on_party_hit)
	var wxray := WallXray.new()
	add_child(wxray)
	wxray.setup(_party)
	...
	layer.add_child(_alert_banner)
	_party.party_alert.connect(_on_party_alert)
	var pip := PipCamera.new()
	layer.add_child(pip)
...
func _build_control_panel(layer: CanvasLayer) -> void:
	...
	# 오브젝트 소환 — 드롭다운(SPAWNABLE_OBJECTS 확장) + 중앙 소환 + 전체 리셋.
...
## 드롭다운에서 선택한 오브젝트를 정중앙에 소환 + 추적(리셋 대상). 레지스트리 = SPAWNABLE_OBJECTS.
## setup(_combat)은 has_method로 자동 — torch: 던진 토치가 FireDamageHit로 착탄(ignite_at 배선) / barrel: 없음.
func _on_spawn_object() -> void:
...
## 피격 방향 화면 가장자리 글로우(F-011 info-war HUD). 컨트롤드=붉은 / 오프스크린 아군=앰버(0.5배).
func _on_party_hit(from_dir_world: Vector3, severity: float, is_controlled: bool, member: Node) -> void:
	...
	if not behind and sp.x >= 0.0 and sp.x <= sz.x and sp.y >= 0.0 and sp.y <= sz.y:
		return  # 아군이 화면 안 → 자기 HP바로 보임, 엣지 글로우 불필요
	var dir: Vector2 = sp - sz * 0.5
	if behind:
		dir = -dir
	_damage_indicator.flash(dir, severity * 0.5, Color(1.0, 0.62, 0.1))  # 앰버 = 아군(네가 아님)


## 분리/MIA 경고 배너(UI-006, 논블로킹·자동숨김·안티스팸).
