...
# F-009 Haste (Swift Grace AB-069) → **Hastened outcome로 통합**(이동=_outcome.move_mult · 공격=attack_interval
# · 만료=_outcome.tick). 별도 _haste_mult/_haste_timer 제거 — move_mult 곱연산 단일창구로.
...
## Ignited/WindBuffeted/Hastened — shared container with enemy_unit. Movement folds into move_speed_mult
## (곱연산); INERTIA(OilSlick·IceGlide) adds inertia (player_controller); Ignited DoT in _physics_process.
...
func apply_haste(pct: float, dur: float) -> void:
	if not _alive:
		return
	if not _outcome.has("Hastened"):
		popup_status("가속", Color(0.9, 1.0, 0.4))
	# Hastened outcome — strongest wins: mag(=pct)를 max로 갱신, dur도 apply가 max. 이동=move_mult, 공격=attack_interval.
	_outcome.apply("Hastened", dur, maxf(_outcome.mag("Hastened"), pct))
	_update_status_icons()
...
## 관성 강도 배율(OilSlick 1.0 / IceGlide 0.7) — 컨트롤러가 SLIP_ACCEL에 곱해 미끄럼 정도를 조절.
