...
const ALLY_CACHE_POOL := [
	...
	"AB-064", "AB-065", "AB-067", "AB-068", "AB-069", "AB-057", "AB-046", "AB-047",
...
#
# 클릭 ↔ 드래그 판별: **누른 지점으로부터의 직선 거리**로 본다. 예전엔 이동량을 계속
# 더하는 방식(Σ|dx|+|dy|)이었는데, 그 합은 줄어들지 않아서 손떨림·고DPI 마우스만으로도
# 제자리 클릭이 8px를 넘겨 카메라 드래그로 오판됐다 → 이동 명령이 씹혔다. 직선 거리는
# 제자리로 돌아오면 다시 0에 가까워지므로 이 오판이 사라진다.
# orbit 시작(DRAG_START)과 클릭 인정 한계(CLICK_MAX)는 **다른 값**이고 겹친다 — 그 사이
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("rally_party"):
		_party.rally()   # T: 흩어진 파티 집합 — 이동 오더 전원 해제 + 진형 복귀 (DRIFT-090)
