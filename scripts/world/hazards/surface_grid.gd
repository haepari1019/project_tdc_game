...
## 연료별 Fire 확산 **속도(m/s)** — 셀 크기 무관(rings/tick = round(mps·GRID_TICK_S/CELL_M), `_mps_to_rings`). 연료 없으면 안 번짐.
## (0.25m·0.06s에서 Oil 8.33→2링, Veg 4.17→1링 = 기존과 동일. CELL_M 바꿔도 m/s 유지 = 재튜닝 불요.)
const FIRE_CREEP_MPS := {"Oil": 8.33, "Vegetation": 4.17}
...
## **순서 = 우선순위**(위가 먼저 매치; 예: Fire+Water+Oil이면 Water 먼저 = quench). result 속성은 RX_RESULT_PRESET.
const SAME_CELL_RX := [
	{"a": "Fire", "b": "Water", "result": "Steam"},                   # 물이 불을 끔(quench)
	{"a": "Fire", "b": "Oil", "result": "Fire", "burst": "explode"},  # 기름 점화 + RX-OIL-FIRE 국소 폭발
	{"a": "Fire", "b": "Vegetation", "result": "Fire"},               # 초목 점화(폭발 없음)
]
## 반응 산물 매질의 셀 프리셋(붕괴 시 부여: ttl/dps/slow/lethal).
const RX_RESULT_PRESET := {
	"Steam": {"ttl": STEAM_CELL_TTL, "dps": 0.0, "slow": 0.0, "lethal": true},
	"Fire": {"ttl": FIRE_CREEP_TTL, "dps": FIRE_CREEP_DPS, "slow": 0.0, "lethal": true},
}
...
var _render_dirty: bool = true       # S5 dirty-track: 렌더 재빌드 필요 플래그(디버그 토글 등)
var _last_render_size: int = -1      # 마지막 렌더 시 _cells 수(구조 변화=add/remove 감지)
...
## S4c/S4d: 셀 안에 공존하는 passive 반응 해소(Overlap combo RX, per-cell). **SAME_CELL_RX 테이블 주도** — 매질 전이는
## 그리드가, 일회성 전투효과(burst)는 reaction_system이(effect layering). 새 반응 = 테이블 한 줄(코드 무수정).
## _by_medium는 primary-only라 _cells 직접 순회하되 extra 빈 셀(대다수)은 즉시 skip → stacked 셀만 처리.
func _react_same_cell() -> void:
	var bursts := {}   # burst_kind → { key → [fsafe, sfac, source] } (붕괴 前 승계 수집 → 일괄 콜백)
	for key in _cells.keys():
		var c: Cell = _cells[key]
		if c.extra.is_empty():
			continue
		var rx := _match_same_cell_rx(c)
		if rx.is_empty():
			continue
		var burst := String(rx.get("burst", ""))
		if burst != "":
			if not bursts.has(burst):
				bursts[burst] = {}
			bursts[burst][key] = [c.friendly_safe, c.safe_faction, c.source]   # 붕괴 前 승계(DRIFT-094·aggro)
		var preset: Dictionary = RX_RESULT_PRESET[String(rx["result"])]
		_set_cell_single(c, String(rx["result"]), float(preset["ttl"]), float(preset["dps"]), float(preset["slow"]), bool(preset["lethal"]))
	for burst in bursts:
		_dispatch_burst(burst, bursts[burst])   # 일회성 전투효과 = reaction_system 콜백(effect layering)


## 셀의 활성 매질(primary+extra)에 매칭되는 첫 반응 규칙(SAME_CELL_RX 순서=우선순위). 없으면 {}.
func _match_same_cell_rx(c: Cell) -> Dictionary:
	for rx in SAME_CELL_RX:
		if _cell_has(c, String(rx["a"])) and _cell_has(c, String(rx["b"])):
			return rx
	return {}


## 셀에 매질 m이 활성(primary 또는 extra)인가.
func _cell_has(c: Cell, m: String) -> bool:
	return c.medium == m or c.extra.has(m)


## 일회성 전투효과 디스패치(effect layering — 그리드 감지 / reaction_system 효과). 새 burst kind = 여기 case 추가.
func _dispatch_burst(kind: String, cells: Dictionary) -> void:
	match kind:
		"explode":   # RX-OIL-FIRE 국소 폭발
			if _combat != null and _combat.has_method("rx_explode_at"):
				_burst_explode(cells)


## burst "explode" — 수집 셀 centroid에 1회 폭발 → reaction_system 콜백(fsafe/source 승계). ref: RX-OIL-FIRE.
func _burst_explode(cells: Dictionary) -> void:
	var sx := 0.0
	var sz := 0.0
	var n := 0
	var fsafe := false
...
func _fire_creep() -> void:
	for fuel in FIRE_CREEP_MPS:
		_creep_fuel(String(fuel), _mps_to_rings(float(FIRE_CREEP_MPS[fuel])))


## S5: m/s 속도 → grid tick당 셀-링(셀 크기 무관). round(mps·tick/CELL_M), 최소 1(느려도 틱당 1셀은 전진).
func _mps_to_rings(mps: float) -> int:
	return maxi(1, int(round(mps * GRID_TICK_S / CELL_M)))
...
func _wind_push() -> void:
	...
	if winds.is_empty():
		return
	var rings := _mps_to_rings(WIND_PUSH_MPS)   # S5: m/s → 셀-링(셀 크기 무관)
	var moved := 0
...
## S5 dirty-track: 휘발성(fade) 매질이 있으면 intensity가 매틱 변함 → 항상 재렌더. 모든 in-place 반응(creep·경계·셀내)은
## fade 매질(Fire/Steam/Smoke)을 만들므로 여기서 잡힘. 구조 변화(add/remove)는 _cells.size()로 잡음. → 정적 비휘발성만 스킵.
func _has_fade_media() -> bool:
	for m in FADE_MEDIA:
		var b = _by_medium.get(m)
...
func _render_cells() -> void:
	var n := _cells.size()
	if not _render_dirty and n == _last_render_size and not _has_fade_media():
		return   # S5 dirty-track: 셀 수 불변 + 휘발성 매질 없음 → 정적 표면, coverage 재빌드 스킵
	_render_dirty = false
	_last_render_size = n
...
func cycle_debug() -> String:
	...
	_render_dirty = true                 # 디버그 토글 → 강제 재렌더(dirty-track 우회)
