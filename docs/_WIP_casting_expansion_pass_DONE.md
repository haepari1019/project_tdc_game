...
| **AB-106** Devour | 즉발 → **1.5s**(DRIFT-129) · cd 8→13 | **누커 유일 처형기로 확정.** 데이터 변경 없음 — 060이 비켜서 중복 해소. 획득 경로가 **EN-3RD-03 처치 드롭뿐**이 됨(ALLY_CACHE_POOL 비대상) | 잠행 BIND-012 slot 2(**사거리 비례 +25%** — 밴드 교정 후) | 적 EN-3RD-03 `enemy_execute` — 배율·쿨 파리티 유지(DRIFT-124) | — | ✅ 완료(DRIFT-130) · 밴드 Melee→**Mid** 교정(DRIFT-131) |
