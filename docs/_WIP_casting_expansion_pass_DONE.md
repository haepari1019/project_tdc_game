...
| **AB-012** Hex Bolt | A→**B 커밋**(cast_s 3.0·cd 4→12; 강력 하드 CC라 긴 캐스트+긴 쿨) · 적 telegraph 0.45→3.0 | **취약 표식(Vulnerable)→개구리 폴리모프 하드 CC 전면 재정의**: 30s 변이(공격/시전/컨트롤 불가·랜덤 hop)·**피해 시 즉시 해제**(sheep式)·**양측 타겟 후보 제외**(파티 평타·적 어그로 무시 → 자동 브레이크 방지). 신규 `skillbook_polymorph`+양 유닛 CC 인프라+미니멀 개구리 큐(축소 0.45·초록·hop). AB-057(순수 증폭)과 차별화로 §5 "취약 2" 통폐합 해소 | Nuker/Healer 존과 동일 — 훅 없는 디버프라 **generic 결속 없음**(bespoke 미적용) · **AB-070 Purge에 아군 개구리 dispel 경로 신설** | 적 EN-007 ↔ 아군 **통일**(단일 거동·apply_polymorph 공유). 적=`telegraph_s` 3.0 / 아군=`cast_s` 3.0 | `skillbooks.json`·`abilities.json`·`sb_polymorph.gd`·`enemy_unit.gd`·`party_member.gd`·`enemy_ai.gd`·`player_controller.gd`·`party_controller.gd`·`combat_controller.gd`·`skill_cast.gd`·`beam_channel.gd`·`dungeon_run.gd`·`combat_sandbox.gd`·`ability_dispatch.gd`·`aim_controller.gd`·`sb_purge.gd`·(회귀수정)`combat_controller/cast_context.spawn_zone` | ✅ 완료(DRIFT-099) |
| **AB-013** Backstab Dash | A→**1.0s 커밋 다이브**(sb_dash `cast_s` 1.0; 근접 다이브 리스크를 짧은 캐스트로 커밋 — 이동계열이나 접근+피해라 순수 A(즉발) 아님, AB-042 "A/B 밖 별도" 선례) · 적 telegraph 0.3→1.0·cd 5→10·kb 1→0 | **`skillbook_charge`(전방 콘·다수 넉백)→`skillbook_dash`(단일 접근+피해)**: 조준 적에게 캐스터 돌진(gap 1.4)+단일 ×1.5+`report_hit_target`(집중 훅). Tank charge와 분리 = 이동계열 접근+피해 슬롯(AB-006 무피해 gap-close ↔ AB-013 접근+피해). DRIFT-085 ② 발전형 아군 구현 완결 | Nuker **집중**(BIND-037 `focus_backstab`: 명중 +2스택·max 도달/이미 max면 준 피해만큼 캐스터 보호막 1s) · **잠행**(BIND-038 `flank_backstab`: 근접 사거리패널티 무·주변 처치 시 이 스킬 쿨초기화=암살연쇄, `notify_kill`). overlay 35→37 | 적 EN-008 `enemy_dash`(hit_on_arrival) ↔ 아군 `skillbook_dash` — **거동 통일**(돌진+단일피해). 적 `telegraph_s` 1.0 / 아군 `cast_s` 1.0 (대칭) | `skillbooks.json`·`abilities.json`·`sb_dash.gd`·`ability_dispatch.gd`·`binding_overlays.gd`·`party_member.gd`·`aim_controller.gd`·`tools/binding_smoke.gd` | ✅ 완료(DRIFT-100) |
...
| **AB-062** Smoke Veil | **즉발 유지**(반응형 — 맞고 나서 켜는 것에 시전을 붙이면 존재 이유가 사라진다, DRIFT-120 ③) · cd 14 유지 · **veil_s 1.5→60** | **순수 이탈기 → 은신 오프너**: `hold_fire`(은신 중 **평타·정체성 정지**) + `next_hit_bonus 2.0`(**첫 타격 ×3.0**) + **해제 시점 = 시전 시작 → 첫 타격**. 셋이 한 쌍 — 자동공격을 안 멈추면 평타가 증폭을 즉시 삼키고, 시전 시작에 풀면 `cast_s` 3~5초 내내 노출돼 "은신 중 캐스팅"이 불가능하다. 그래서 veil은 **최장 시전(AB-059 5.0s)을 덮는 준비 창**이고, 은신을 끝내는 건 시간이 아니라 **능동 타격**이다. 갇힘 방지로 **재입력 = 취소**(토글 오프 — 차지·쿨 게이트보다 앞이라 쿨 중에도 먹히고, 차지·쿨은 무비용이되 **대기 증폭은 폐기** = 은신을 건너뛰고 보상만 챙기는 경로 차단) | next-hit 훅 **`maxf` → 곱누산**(AB-007 이탈 결속 +30%가 통째로 먹히던 문제; 축이 다른 잠행 본체 `band_dmg`는 무관) = **×3.9**. 해제 규칙이 `hold_fire` 기준이라 `disengage_veil`도 "첫 타격 해제"로 편입 | (Ally-only — 적 미사용) | `skillbooks.json`·`sb_stealth.gd`·`ability_dispatch.gd`·`combat_controller.gd`·`party_member.gd`·`skill_text.gd`·`party_pool_smoke.gd` | ✅ 완료(DRIFT-121) — 스왑해도 은신 유지 = **안전 주차**(유휴는 의도된 대가, 사용자 확정) |
| **AB-056** Longshot Bolt | **즉발 유지**(저쿨 견제가 정체성) · cd 4→9 | **전격 → 냉기 분화**(056↔073이 형태 없이 눈금만 달랐다 — 지속 화력 0.244↔0.278로 거의 동일). 냉기를 고른 근거는 **형태 적합**: range 15 즉발 단일 잠금 = 저격 견제기라 Chill(둔화)의 *멀리서 쏘아 발을 묶는다*가 맞물린다(화염 DoT는 쿨 9초 갱신으로 의미가 흐려진다). `chill_dur_s 2.0` — **즉발 무리스크라 캐스트 3.5s 커밋인 AB-041의 3.0보다 짧게**. 부수로 누커 RX가 전격 단일 → 전격+냉기 2축(물 결빙 신규) | 훅 없는 볼트 — generic 결속 그대로 | (Ally-only) | `skillbooks.json` | ✅ 완료(DRIFT-128) |
| **AB-030** 전격 제압 (구 Voltaic Interrupt) | 즉발 → **1.5s**(DRIFT-129 1.0 → 인터럽트가 아니게 돼 사다리에서 내려옴) · cd 8 유지 | **인터럽트 → 침묵 「제압」 전면 재정의.** 원래 미판정 사유(*"캐스트 차단 필드 없음"*)는 **틀린 전제** — stun 자체가 양 진영에서 시전을 끊는다. 진짜 문제는 **실측**: 적 27종 중 `cast 1.0`으로 끊을 수 있는 게 **AB-012 1종뿐**(텔레그래프가 0.2~1.0s에 몰림)이고 즉발인 Tank AB-011이 오히려 더 잘 끊었다. → `stun 0.5` → **`silence 3.0`**, `skillbook_stun` → **`skillbook_silence` 2변주**(044 광역·무피해 ↔ 030 단일 잠금·타격), 이름 교체. **침묵이 진행 중 시전도 끊게 함**(사용자 추가 — 노리고 못 쓰는 *얻어걸림 보너스*) | 잠금 조준(DRIFT-122) · 인터럽트 역할은 **Tank AB-011**이 존치 | (Ally-only) | `skillbooks.json`·`display_names.json`·`sb_silence.gd`·`enemy_ai.gd`·`enemy_unit.gd`·`skill_text.gd`·`backpack.gd` | ✅ 완료(DRIFT-133) |
| **AB-100** Pounce | 즉발 → **1.0s**(DRIFT-129) · cd 6→10 | 콤보 **시작** 단계로 확정 — `Pinned` 0.6s는 `MOVE_MULT` 0.0(완전 이동잠금)이라 실동작. 급습·고딜·짧은 고정. 데이터 변경 없음 | 잠금 조준(DRIFT-122) | 적 EN-3RD-01 — 배율·쿨 파리티 | — | ✅ 완료(DRIFT-132) |
| **AB-103** Tether | 즉발 → **2.0s**(DRIFT-129) · cd 8 유지 | 🐞 **미구현 복원**: `Tethered`가 `MOVE_MULT`·`ATK_MULT`·`CC_TENACITY` 어디에도 없고 leash 판정도 없는 **표시용 배지**였다(= 배율 0.4 + 무효과). 스펙대로 **leash 8m 밖 = break DoT 3dps**를 구현하고 **끌려오기 2.5m/s** 추가(사용자). 상태가 **anchor(시전자)** 를 들도록 `apply_tether` 신설 — 거리 판정에 시전자 위치가 필요. 끌림은 신규 이동 경로 없이 `apply_drift`(AB-042 바람 넛지) 재사용. 팝업 라벨 `포박`도 `float_text.OUTCOME_KO`에 누락돼 있어 추가 | 잠금 조준 | 적 EN-3RD-02도 **같은 경로**(`apply_tether`)로 통일 — 종전엔 적 킷 절반이 무효였다. leash·dps·지속 파리티 게이트 | `skillbooks.json`·`abilities.json`·`outcome_status.gd`·`party_member.gd`·`enemy_unit.gd`·`sb_tether.gd`·`enemy_ai.gd`·`float_text.gd` | ✅ 완료(DRIFT-132) |
| **AB-060** 파열 처형 | — (**폐기**) | **아군판 폐기**(N4 통폐합) — AB-106과 `kind·cast 1.5·mult 2.4·radius·execute_under·execute_mult·tier·equip·잠금`이 전부 같고 실차이는 흡혈 0.0↔0.2 하나였다(DRIFT-123이 배율을, DRIFT-129가 시전을 동일하게 만듦). 106의 밴드 Melee가 잠행 보상을 +0.15로 깎아 흡혈 이득과 상쇄 → **축 0**. 106은 Shared(EN-3RD-03)라 못 지우고 060은 Ally-only라 깨끗이 빠짐. **ID는 등록만 잔존** | 잠행 **BIND-012 → AB-106 재배선**(payoff: 사거리 비례 → generic +15%) | (Ally-only) | `skillbooks.json`·`dungeon_run.gd`(ALLY_CACHE_POOL)·`binding_overlays.gd`·`combat_sandbox.gd` | ✅ 폐기(DRIFT-130) |
| **AB-106** Devour | 즉발 → **1.5s**(DRIFT-129) · cd 8→13 | **누커 유일 처형기로 확정.** 🐞 **`ON-KILL-FEED` 쿨 환급 복원**(DRIFT-132) — 스펙이 060과의 차별점으로 못박은 킬-보상 루프인데 회복만 있었다. 처치 시 **남은 쿨 50% 환급**(전액이면 저HP 무리에서 무한 연발). 효과가 `inst.cooldown_s`를 직접 못 깎아(`cast_s>0`은 해소 **뒤** 덮어씀) `report_cd_refund` 보고 채널 신설. 획득 경로가 **EN-3RD-03 처치 드롭뿐**이 됨(ALLY_CACHE_POOL 비대상) | 잠행 BIND-012 slot 2(**사거리 비례 +25%** — 밴드 교정 후) | 적 EN-3RD-03 `enemy_execute` — 배율·쿨 파리티 유지(DRIFT-124) | — | ✅ 완료(DRIFT-130) · 밴드 Melee→**Mid** 교정(DRIFT-131) |
| **AB-073** Overcharge | **즉발 유지** · cd 10→18 | 056의 냉기 분화로 **중복 해소** — *멀리서 크게 지지는 저격*(lightning+shock 2.0 · mult 5.0)으로 자리 확정. 데이터 변경 없음(DRIFT-123 배율·쿨만) | — | (Ally-only) | — | ✅ 완료(DRIFT-128) |
| **AB-058** 방전 작렬 | **즉발 유지** · cd 8→14 | N2 유일 **광역**(r3.0 · range 10 · 잠금 없음 · speed 18) = *가까이서 넓게* 로 형태가 이미 갈려 중복 혐의 없음. 데이터 변경 없음 | — | (Ally-only) | — | ✅ 완료(DRIFT-128) |
| **AB-054** 절단 광선 | (D5에서 확정) | 🐞 **정합 수정** — lightning인데 `shock_s`가 없어 **툴팁만 감전을 약속**하고 있었다(`Elements.TABLE`에서 lightning만 `dur_default: 0.0`). 채널 틱 0.25s에 맞춰 `shock_s 0.4`(갱신형 — 채널 중 유지, 종료 직후 해제) | — | (Ally-only) | `skillbooks.json` | ✅ 정합(DRIFT-128) |
