...
- **S2 착수 — 국소 점화(첫 조각).** 체감 피드백: Fire를 Oil **가장자리**에 맞혀도 옛 `_ignite_oil`이 **존 전체를 중심
  ...
  **맞힌 자리부터** 태운다. 존 제거해도 detach된 셀 생존. 재귀 fire_hit 연쇄 제거(creep이 대체). `surface_grid.ignite_oil_local`
  + `combat_controller.surface_grid_ignite_oil` facade + `reaction_system._ignite_oil` 분기. 게이트: surface_smoke(국소점화) +
  ci_smoke 11/11. **남은 S2:** Fire↔Water 등 나머지 경계 반응(`_zone_reaction_tick` 중점/shrink 완전 대체) → DRIFT-096 종결.
