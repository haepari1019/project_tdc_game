...
| **기어/정체성 (F-008/D-019)** | ✅ 카탈로그 + 롤테이블 G1 | 17 신규 기어·6 정체성·6 ability effect·기어귀속 평타 (DRIFT-056). **롤테이블 G1**(DRIFT-061): 스타터 4 id 스펙 정렬(_set→spec)+세이브 마이그레이션·파생 롤테이블·bind fwd-prep. 잔여=G2(획득 롤+인스턴스 저장)·G3(서브옵션 mult+UI)·affix·ba 특수거동 |
...
- ◐ **gear `identityRollTable` 이행 G1 완료(DRIFT-061·IMPL-DEC-021):** 스타터 4 id 스펙 정렬+세이브 마이그레이션·파생 롤테이블(`get_gear_identity_roll_table`)·`_bind_gear` rolled fwd-prep. 설계=`docs/design/gear_roll_table.md`. **G2(위험·다음):** 획득 시 롤+기어 저장 문자열→인스턴스 딕셔너리. **G3:** 서브옵션 mult+UI.
- **고위험 게이트(잔여):** roll-table **G2/G3** + affix(D-018 §7.3) + per-AB tier 데이터 — 단독 결정([[refactor-risk-preference]]).
