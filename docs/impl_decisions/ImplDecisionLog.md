...
### IMPL-DEC-20260827-001 — 런 목표 소유권 = **계약(blueprint)**; 가드는 「지목」과 「실재」로 갈라 둔다
- **결정(2026-08-27, 사용자 지시):** `objective_rule`/`objective_room`을 맵 문서에서 **blueprint `objective`**로 옮겼다. 소유자 이동이지 기능 추가가 아니다 — 관측 가능한 런 거동은 불변이고, 바뀐 것은 **누가 그 결정을 들고 있는가**뿐이다. 배경: 의뢰 구조 재편 논의에서 *「맵을 훨씬 늘릴 계획」*이 확정되며, **맵 수에 비례해 비싸지는 결정만** 먼저 옮기기로 했다.
- **왜 지금인가:** 맵이 목표를 소유하면 **한 맵 = 한 목표**로 굳는다. 맵이 2개일 땐 공짜지만, 늘어날수록 **같은 맵을 재사용해 계약을 늘리는 길**(`F-006` §3.1.3의 *「동일 Map, 다른 Contract」*)이 닫힌 채로 굳는다. 지금은 데이터 4파일 + 코드 3파일이고, 맵이 열 개가 되면 맵마다 재작업이다.
- **왜 blueprint인가:** 이미 **출정 선택 단위**이고 `map_id`·`contract_id`의 단일 소유자다(`_note`에 명시). 계약 메타를 담을 그릇이 새로 필요하지 않았다.
- **구조체를 쓴 이유(스펙보다 잘게):** 스펙은 스칼라 `objectiveRef`인데 `{rule, door_ref?, room_ref?}`로 갔다 — **규칙마다 가리키는 대상의 종류가 다르기 때문**이다(`onDoorOpen`=전이 앵커 `ref` / `onObjectiveRoomCleared`=`room_ref`). 하나의 `objectiveRef`로 뭉치면 「이 문자열이 방인가 문인가」를 **읽는 쪽이 규칙으로 되짚어야** 하고, 그 되짚기가 코드에 흩어지면 검증이 불가능해진다. → 필드 형태는 `PENDING-PROP`([[DRIFT-201]]).
- **가드를 두 곳으로 가른 이유:** 「**지목했는가**」(`_validate_blueprint`)와 「**그게 실재하는가**」(`_validate_rooms`)는 **읽어야 하는 문서가 다르다**. 계약 검증은 맵 문서를 아직 안 읽었을 수 있으므로 실재 검사를 할 수 없고, 맵 검증은 계약이 이미 로드된 뒤라 둘을 맞붙일 수 있다. 한 곳에 몰면 호출 순서에 의존하는 검증이 되고, 그건 조용히 통과하는 검증으로 퇴화한다.
- **부수 수정(스모크):** `_check_entry_requirements`가 `doors[0]` — **배치 순서에 달린 아무 문**을 집어 「이 문이 곧 목표」를 주장하고 있었다. 데모 맵은 관문이 셋이라 심부 관문을 집고도 통과했다. 목표를 든 문을 **골라서** 보게 고쳤다.
- **대안(기각):** (a) 맵에 `objective_rule`을 남기고 계약이 **덮어쓰기** — 두 벌이 되어 「어느 쪽이 이겼는지」를 매번 물어야 한다. 소유자를 옮기는 것이 목적인데 소유자를 둘로 만드는 안. (b) 앵커의 `completes_objective`를 그대로 두고 규칙만 계약으로 — 문 지목이 맵에 남아 「같은 맵, 다른 계약」이 반쪽만 열린다. (c) 이전을 미루고 의뢰 층부터 — 의뢰 목표물이 계약 위에 얹히는 구조라 어차피 먼저 와야 하고, 미루면 맵 수만큼 비싸진다.
- **영향 파일:** `data/slice01/blueprints/DBP-{DEMO,UPPER}-001.json` · `data/slice01/maps/MAP-{DEMO,UPPER}-001.json` · `scripts/autoload/slice01_data.gd` · `scripts/run/dungeon_run.gd` · `tools/map_smoke.gd` · `docs/ARCHITECTURE.md`.
- **검증:** `ci_smoke.sh` 전 스위트 PASS · map smoke 두 출정지 PASS · 반증 3건(계약 `objective` 제거 / 없는 `door_ref` / 배선 무력화) 전부 의도대로 FAIL 후 복구.

### IMPL-DEC-20260728-002 — 투사체 능력 3종(origin·무장거리·근접히트) + 2단 부채꼴 조준 마커
- **결정(2026-07-28):** AB-055 산탄([[DRIFT-110]])을 구현하며 `Projectile`/조준계에 **재사용 가능한 능력**을 셋 추가했다. AB 하나를 위한 특수 분기가 아니라 **범용 파라미터**로 뺐다 — 앞으로 파생 투사체(분열탄·튕김 등)가 그대로 쓴다.
- **① `spawn_projectile(…, origin = null)`:** 종전엔 발사 지점이 항상 캐스터였다. 산탄 파편은 **착탄점**에서 나가야 해서 선택 인자로 뺐다(기본 null = 캐스터, 기존 호출부 무영향). ctx 계약이라 `AbilityDispatch`·`CastContext` **양쪽** 갱신(파리티는 `party_pool_smoke`가 검증).
- **② `params.arm_after_m`(무장 거리):** 이만큼 날아가기 전에는 **유닛과 충돌하지 않는다**(수류탄 신관). 없으면 착탄점에 적이 서 있을 때 파편 6발이 그 자리에서 동시 폭발해 피해가 몰린다.
  - ⚠️ **판정을 끄면 안 된다(사용자 지적):** 첫 구현은 무장 전 레이캐스트를 **통째로 건너뛰어** 벽·방벽까지 통과했다 — *"방어막을 쳤는데 바로 앞에 산탄을 날리면 벽이 안 붕괴됐는데 뒤의 적이 맞는"* 상황. → **마스크에서 유닛 레이어(2·4)만 빼는** 방식으로 교체. 같은 레이캐스트 경로를 그대로 타므로 **방벽 흡수·아군 방벽 통과·돔 커버리지 규칙이 전부 유지**된다. 로직 복제 0.
  - 데드존 안에서 목적지에 도달하는 짧은 사격은 무장과 무관하게 페이로드를 해소(탄이 조용히 사라지는 것 방지).
- **③ `params.hit_radius_m`(근접 히트):** 세그먼트 레이는 **무한히 얇아** 콜라이더 중심을 정확히 지나야 맞는다. 대상 지점을 향해 날아가는 일반 볼트는 도착 판정이 가려 줬지만, **임의 방향으로 날아가는 파편은 적을 스치고 지나갔다**(사용자 제보). 매 스텝 위치 주변의 적대 유닛을 반경 검사해 잡는다. **`ctx.enemies_in_radius`를 쓰므로 진영이 자동으로 뒤집힌다**(마스크 직접 조작 불요). 무장 전에는 건너뛴다(데드존 무력화 방지). 기본 0 = 기존 투사체 동작 불변.
- **④ 2단 조준 마커(`AimMarker.show_aim(…, band_inner, band_outer, band_cone_deg)`):** 초탄 원판 + **부채꼴 띠**(데드존 경계 ~ 파편 도달)를 동시에 그린다. 색은 같은 계열에서 **밝기만 이동**(다른 색이면 별개 스킬로 읽힌다). 처음엔 360° 링으로 그렸다가 *"실제 산탄 확산 범위와 다르다"*(사용자)로 **고리형 부채꼴 ArrayMesh**로 교체 + 매 프레임 캐스터→커서 정렬(= `sb_bolt._scatter`의 확산 기준축과 동일 벡터).
  - **데드존 계산은 `sb_bolt.deadzone_of()` 한 곳이 SSOT**이고 `aim_controller`가 그대로 호출한다. 수식을 복제했다면 튜닝 시 **조준선과 실제 동작이 조용히 갈라졌을** 것.
- **대안(기각):** (a) 파편을 데드존 **바깥에서 스폰** — 무장거리 없이 간단하지만 파편이 허공에서 튀어나와 "터진 자리에서 흩어진다"가 안 읽힌다. (b) 히트 판정을 shape cast로 전환 — 모든 투사체 성능·거동에 영향, 이번 문제엔 과함. 반경 검사는 opt-in이라 영향 범위가 좁다.
- **영향 파일:** `scripts/combat/abilities/projectile.gd` · `{ability_dispatch,cast_context}.gd` · `effects/sb_bolt.gd` · `scripts/ui/aim_marker.gd` · `scripts/run/controllers/aim_controller.gd` · `tools/party_pool_smoke.gd`.

### IMPL-DEC-20260728-001 — 조준 모달 우클릭 = **취소 전용**(이동명령 분리) · 짝 없는 RMB release는 탭 아님 · 샌드박스 입력 파리티
- **결정(2026-07-28, 사용자 지시):** *"에임어시스트 취소 동작에서는 이동명령을 내리지 마라 — 조준 중 마우스로 이동하려면 우클릭을 두 번 눌러야 하게."* → 조준 모달이 켜져 있는 동안 **우클릭 1회 = 조준 취소만**, 이동은 그다음 우클릭부터.
- **원인(비자명해서 기록):** 이동 명령은 우클릭 **누름이 아니라 뗄 때** 나간다(드래그 거리로 "탭"을 판정). 그런데 release 분기가 **press와 짝이 맞는지 확인하지 않아서**, 조준 취소로 press가 위에서 소비돼도 release가 `_rmb_max_dx == 0`(드래그 없음)만 보고 탭으로 인정 → **우클릭 한 번에 「취소 + 이동명령」이 동시 발생**했다. 취소 로직의 문제가 아니라 **탭 판정의 문제**였다.
- **수정 ① 짝 판정(일반 규약):** release 분기에서 `was_dragging`(= press를 실제로 받았나)을 먼저 읽고, 그때만 탭으로 인정한다. 조준뿐 아니라 **RMB press를 소비하는 모든 모달**(부활 조준·횃불 투척·오브젝트 배치)에 자동 적용되므로 모달이 늘어도 같은 버그가 재발하지 않는다.
- **수정 ② 샌드박스 입력 파리티:** `combat_sandbox`가 조준 중 **좌클릭만** `AimController.handle_click`에 넘기고 있어(주석: *"RMB는 카메라 orbit 유지"*) 우클릭이 취소 분기까지 도달하지 못했다. `dungeon_run`은 이벤트를 통째로 넘겨 **원래부터 취소되던 것** — 두 씬의 입력 경로가 어긋난 케이스. 샌드박스도 통째 위임으로 통일. **트레이드오프:** 조준 중 우클릭은 카메라 orbit이 아니라 취소가 된다(던전이 이미 그러했으므로 파리티가 맞는 방향). 조준을 끄면 orbit 복귀.
- **대안(기각):** (a) 취소 시 별도 플래그로 다음 release 1회를 삼키기 — 상태가 하나 늘고 모달마다 챙겨야 해서 기각. `was_dragging` 짝 판정이 **상태 추가 없이** 같은 문제를 전부 덮는다. (b) 조준 중 우클릭을 orbit으로 두고 Esc만 취소 — 사용자 지시와 반대이고 던전과도 어긋남.
- **영향 파일:** `scripts/run/dungeon_run.gd`(RMB release 짝 판정) · `scripts/dev/combat_sandbox.gd`(동일 + 조준 모달 이벤트 통째 위임). 관련: [[sandbox-input-parity]] — 샌드박스는 자체 입력 핸들러라 **입력에 닿는 기능은 항상 양쪽에 반영**해야 한다(이번 건이 그 사례).
- **검증:** `ci_smoke.sh` 11/11 PASS + **샌드박스 F5 실제 확인(사용자)**. 입력 경로라 헤드리스 스모크로는 못 잡는 영역.
