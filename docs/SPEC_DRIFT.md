...
- **구현(B1 — 신규 effect kind):** AB-064 Quick Mend→`skillbook_heal`(반경 ally instant heal, heal_pct×maxHP, F-022 healer threat) · AB-046 Shield Wall·AB-047 Aegis Pulse→`skillbook_dr`(반경 ally DR, `member.apply_damage_reduction` — Sentinel 감쇠 재사용, move-lock 없음). 단일타겟 heal/self DR은 소반경 pulse로 근사.
- **분류\전파:** **impl + tuning(로깅만).** 규칙 드리프트 없음. **이연(B1 잔여):** HoT(AB-065 Regenerating status) · Buff(AB-067~069/075) · Mobility/blink(AB-061/062) · channel-beam(AB-054) · vulnerable/mark debuff(AB-057/070) · barrier-spawn(AB-034 Rampart) · **밴드 패널티**(D-016 main/subClasses 차등) · **ally-only lootable 인-런 획득 경로**(적 드롭 안 됨 → shop/chest/드롭표 = S6b).
- **검증:** JSON 유효 + ci_smoke(id 1:1·skillbook 로드·effect 컴파일) + B1 런타임(heal HP↑·dr mult↓). 샌드박스 LOADOUT 드롭다운 장착 검증. 커밋 f0d6c11(AB-053)·4b16fb7(B2)·(B1).
