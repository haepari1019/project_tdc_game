...
- **설계 결정 기록(구현 전):** 파티는 **찢지 않는다** — 레이어 이동은 **전 파티 결집 조건**부이며 4명 전원이 함께 간다(`F-007` §3.6.2 `extractionCohesionRule` + `F-003` `unbound_anchor_max_m`와 같은 모양). **down/MIA는 따라오지 않고 쓰러진 자리에 남는다**(MIA→`down` 전환, 위치 보존) — 찢김은 *조작 가능한* 멤버가 나뉘는 것이고 쓰러진 몸은 **회수 부채**(`F-007`)다. 그 결과 **행동 가능한 파티는 언제나 한 레이어**에 있어 안개·내비·콜리전의 레이어화 비용이 크게 준다. **적은 레이어 고정, `F-028` 3세력만 계단을 쓴다.** 미확정: 비활성 레이어의 `down` 멤버를 적이 건드릴 수 있는가.
