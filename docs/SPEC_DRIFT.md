...
- **상태:** 🔶 구현 완료 · ci_smoke **10/10 PASS**. **샌드박스 체감 대기** — 특히 ① 전투 중 이동 오더가 교전보다 우선할 때 파티가 무너지지 않는지 ② `HOLD` 멤버가 진형에서 빠진 채 방치되는 게 답답하지 않은지(집합키 빈도로 판단) ③ 점선 가독성(인원 4명 전원 오더 시) ④ **팔로워 기절 정지의 난이도 영향**.
