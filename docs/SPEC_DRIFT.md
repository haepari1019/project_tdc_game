...
- **설계 결정 기록(구현 전):** 파티는 **찢지 않는다** — 레이어 이동은 **전 파티 결집 조건**부이며 4명 전원이 함께 간다(`F-007` §3.6.2 `extractionCohesionRule` + `F-003` `unbound_anchor_max_m`와 같은 모양). 그 결과 **활성 레이어가 언제나 하나**라 안개·내비·콜리전의 레이어화 비용이 크게 준다. 미확정: 다운/MIA 멤버 동반 가부 · 적도 계단을 쓰는가.
