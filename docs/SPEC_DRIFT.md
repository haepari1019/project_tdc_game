...
### DRIFT-200 — 이동선은 「가는 곳」이지 「닿는 곳」이 아니다: 시전 지점 절단 + 착탄 표기 분리 🔶 rule **전파 후보**(미전파)
...
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 전파 필요 — 방금 올린 조문이 이미 낡았다.** `F-003` §3.5.3은 [[DEC-20260827-001]]에서 *「각 멤버의 현재 위치 → 그 멤버의 **오더 목표 지점**까지 점선 이동선」*으로 확정했는데, 본 드리프트가 그것을 **「시전 지점까지」**로 바꾸고 **착탄 표기**를 신설한다. 같은 절을 다시 열어야 하므로 `OPS_30` 2차 전파 대상이다(사용자 승인 대기 — Stop Line).
