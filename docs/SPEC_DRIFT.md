...
- **의도적 근사/이연(B1 잔여):** ① **Rampart 투사체 1회 흡수·threat-on-hit = 여전히 이연(BLOCKED)** — 전투 모델이 데미지를 target-locked/즉발로 적용(월드 지오메트리 투사체 엔티티 없음) → 벽에 부딪힐 투사체·벽으로 라우팅되는 공격이 없어 구현 대상 자체가 부재. 투사체 엔티티 시스템 선행 필요(노력이 아니라 의존성). navmesh 미리베이크라 NC 추격은 벽 우회 안 함(물리 차단만, 4s 한시). ② Beam = cone 근사(라인 판정 대신 좁은 cone half_deg). ③ Smoke Veil = 적 타겟만 드롭(인플라이트 locked hit는 명중). ④ Purge 제거 대상 = Bloodlust 외 [Fortified/Hasted/Shielded/Warded/Regenerating]는 현재 적이 안 가짐(전방호환 no-op) → DRIFT-058.
