...
- **G3 완료(rolls 스탯 적용 + UI):** `_bind_gear`가 **dmg_mult→평타 위력**(매 bind fresh 재계산, 비누적), **cd_mult→`cooldown_mult`→identity 쿨**(ability_dispatch.try_identity가 곱). 상세 툴팁(인벤/장착, 표시명 레이어 display_names.json)으로 굴린 identity·옵션 표시. **G3 잔여:** Stash 인스턴스화(스페어 roll 보존)·potencyMult·affix.
