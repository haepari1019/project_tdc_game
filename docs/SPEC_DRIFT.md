...
- **스폰 위치 시드 산포 🔶 impl(게임측):** `_squad_spawn_center`가 룸당 고정 deep point였음 → 위치도 예측 가능. run_seed로 ±SPAWN_SCATTER_M(4.5m) 산포 + **navmesh 스냅**(벽 안 방지). 스펙은 authored `ambushAnchorRefs[]`/`holdAnchor`(LDG-001) **복수 앵커**를 이미 허용 → 별도 규칙 전파 불필요, 데모는 절차적 시드 산포로 단순화(정식 authored 앵커/patrol graph는 후속 레벨디자인).
