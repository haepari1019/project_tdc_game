...
> **2026-08-24 추가:** 맵 고도화 **Phase 1 전파 패킷 초안** = [`_PROP_PACKET_MAP_PHASE1.md`](_PROP_PACKET_MAP_PHASE1.md) (A `routeClass` · B **경로·레이어가 교전 티어 공급**(택일) · C `spatialGrammar` · D `encounterAnchor.category` · E `lootAnchor.tier` · F 신규 ID). 🕒 **미집행 — 판정 2건 접수(§B=옵션 2 **2단 구조**(① `entryRequirement` 잠금 → ② `difficultyProfile` 오버라이드; 잠금은 `run` 소모성·`profile` 영구 **둘 다**) `rule`+DecisionLog · §F=실제 지역명). 판정 4건 추가 접수: `grantOn`=`onExtractionSuccess` · 잠금 없는 난이도 방 허용(전조 SHOULD) · 지역 컨셉 미확정 → **ID는 안정 축(`RM-UPPER-##`)·정체성은 표시명 축(임시)** · `F-010` 문구 갱신. 남은 판정: §F 형태 확인 · 문법 enum 개수 · 그레이박스 도면 채택.** Phase 0(DRIFT-162~166)은 전부 게임 소유라 전파 대상 아님.
>
