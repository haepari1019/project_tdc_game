...
- **전투 감속(수치, 구현):** 교전 시 이동 ×2/3(`COMBAT_MOVE_MULT`) — 적 `enemy_unit.current_move_speed`(engaged 게이트), 아군 `party_member.move_speed_mult`(`combat_slowed`; `party_controller._on_engagement_changed`가 `is_engaged`로 토글 → controlled+follower 공통). 비전투 현행(스프린트). **적** 텔레그래프 중 이동 정지 = `enemy_ai._engage_move` winding 게이트(채널 무관으로 확장; 아군은 기존).
- **아군 선택(impl, 구현):** 좌클릭 party-layer(mask 2) 레이픽 → 스왑(`dungeon_run._select_party_under_mouse` + `party_controller.index_of`, 적 인스펙트 앞) + 1~4 키 병존 + 머리 위 상시 번호 배지 Label3D(`_add_slot_badge`).
...
- **상태:** 🔶 role 레지스트리 커밋(`40982d8`). **B 샌드박스 확인 완료(사용자)**. **A(감속+텔레그래프 정지)·C(클릭스왑+배지) 구현·미검증**(godot 헤드리스 부재 → 샌드박스 확인 대기). 계획 `ee0207b`.
