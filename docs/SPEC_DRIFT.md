...
> **2026-08-24 추가:** 맵 고도화 **Phase 1 전파 패킷 초안** = [`_PROP_PACKET_MAP_PHASE1.md`](_PROP_PACKET_MAP_PHASE1.md) (A `routeClass` · B **경로·레이어가 교전 티어 공급**(택일) · C `spatialGrammar` · D `encounterAnchor.category` · E `lootAnchor.tier` · F 신규 ID). 🕒 **미집행 — 판정 2건 접수(§B=옵션 2 「난이도 축을 방·경로가 소유」 `rule`+DecisionLog · §F=실제 지역명). 남은 판정: 지역 슬러그 8종(GATE·DEEP·EXT는 이름 자체가 없다) · 컨셉존 접두사.** Phase 0(DRIFT-162~166)은 전부 게임 소유라 전파 대상 아님.
>
