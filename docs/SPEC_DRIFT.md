...
- **⚠️ 판정 필요 1건 (`PENDING-PROP`):** `haulMaterial`에 **표시색 필드**를 스펙이 가질 것인가, 아니면 게임측 표시 레이어(`display_names.json` 선례)로 둘 것인가. 지금은 데이터 파일에 넣되 `_note_color`로 **게임측 표시 레이어임을 명시**했다.
