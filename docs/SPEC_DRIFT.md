...
> **2026-08-24 추가:** 맵 고도화 **Phase 1 전파 패킷 초안** = [`_PROP_PACKET_MAP_PHASE1.md`](_PROP_PACKET_MAP_PHASE1.md) (A `routeClass` · B **진입 조건 → 난이도 오버라이드**(2단) · C `spatialGrammar` · D `encounterAnchor.category` · E `lootAnchor.tier` · F 신규 ID · G **레이어**). ✅ **집행 완료(2026-08-24) — spec `2a81e52` · `DEC-20260824-001` · 10파일.** 핀 `a9e67bd` → `2a81e52`. F-006 §3.1.2 교체(난이도 축을 공간이 소유)·§3.2.5 신설(encounterAnchor)·§3.10 진입조건 확대, LDG-001 §9 신설(공간 필드 7종+`entryRequirement`+`layer`), F-028 §3.2.2a(3세력만 층 이동), F-026/D-015/DBP-DEFAULT-001/LDG-SPAWN 정합. 덤으로 `F-006` §3.1 절 번호 중복 부채도 해소. **남은 판정: `layer` 표기 · 3세력 층간 구현 방식 · 문법 enum 6종 · 그레이박스 도면 채택.**
>
...
### DRIFT-141 — `IDA-052` 이동 잠금 → 둔화 · 「응보」 소멸 = 시간 기반 ✅ **전파 완료(2026-08-13, spec `09e045c` · `DEC-20260813-002`)** 🔶 rule
...
- **분류·전파:** `moveLock` = spec 명시 필드값 변경 = **rule** → **`OPS_30` 전파 완료**(`DEC-20260813-002`).
  - **impact_scan이 잡은 것:** 이동 잠금은 `IDA-052` 문서가 아니라 **`EFFECT-CORE`(`APPLY-SENTINEL-FORM` `moveLock: true`)와 `STATUS-ACTOR-CORE`(`SentinelForm` "이동불가")** 에 정의돼 있었다 — 능력 문서만 고쳤으면 카탈로그 두 곳이 거짓으로 남았다.
  - 편집: `EFFECT-CORE`(→ `moveSpeedMultiplier: 0.45`) · `STATUS-ACTOR-CORE` · `IDA-052`(Draft Parameters·notes·Keystone) · `IDA-022`(캡스톤 순서 규약) · `ROLE-010` §4.5. 매퍼 drift 0 · xref 신규 0. **재핀 `66a3dda` → `09e045c`**.
  - 수치(`move_slow` 0.45 · 응보 6.0s · 넉백 2.0m)는 **튜닝** — 플테 후 `D-###` 등재 검토(spec TODO 등재).
...
- **상태:** ✅ RESOLVED — 구현·게이트·역전파·재핀 완료.
- **후속 정정(같은 날, 같은 커밋에 포함):** 「진격」 캡스톤 순서 — 2타째에 **속박부터 걸고 넉백**하던 것을 **넉백 → 그 자리에서 속박**으로 뒤집었다(사용자: *"밀린 후 밀린 자리에서 속박"*). 적 넉백은 `kb_vel` 채널이라 `Rooted`(이동 배수 0)와 독립적으로 굴러가므로 두 효과가 모두 성립하지만, 속박을 먼저 걸면 *"붙잡혔으니 안 밀린다"*로 읽혀 **진격이라는 축 자체가 죽는다.** 같은 교훈이 이미 `enemy_ai.gd`(AB-102 뭉치기+속박)에 주석으로 남아 있었다 — *"뭉치기 먼저, 속박 나중"*. 「밀림」은 캡스톤에서 소모한다(유지하면 밀 때마다 재속박 = 영구 CC).
...
### DRIFT-143 — 넉백이 기절/빙결/변이에 삼켜져 **CC가 끝난 뒤에** 발동 (code-bug · 전파 없음)
- **증상(사용자):** *"진격을 쓸 경우 기절 같은 디버프가 서브에 있는 경우 기절 같은 게 먼저 들어가고 끝나면 넉백이 되는데 이게 이론적으로 이상해."* 밀치고 기절시켰는데 **기절이 풀린 뒤에야 밀려난다** — 인과가 뒤집혀 보인다.
- **근본 원인:** `enemy_ai`의 틱 순서. 기절/빙결 게이트가 `tick_knockback`보다 **위**에 있어서, 기절 중엔 `velocity = ZERO; move_and_slide(); return`으로 빠져나가 **`tick_knockback`이 아예 호출되지 않았다.** `kb_timer`는 그 함수 안에서만 감소하므로 그대로 보존됐다가, 기절이 풀리는 프레임에 비로소 굴러간다 = **지연 발사**. 변이(개구리) 게이트도 동일.
- **범위는 진격보다 넓다.** 「진격」 결속에서 눈에 띄었을 뿐, **넉백을 가진 모든 스킬 × 하드 CC 조합**이 같은 증상이었다 — `AB-002` 방패치기, 평타 `kb_m` 아키타입(`ba_tank_aegis_ram` 등), `AB-051` 견인, 폭발 넉백 전부.
- **처리:** `tick_knockback`을 **모든 행동 금지 게이트보다 먼저** 호출하고, 그 결과(`kb`)를 게이트들이 존중하게 했다 — 넉백이 이번 프레임 이동을 가져갔으면 기절/변이가 `velocity`를 덮어쓰지 않는다. **넉백은 물리 변위라 행동 금지와 무관**하다는 것이 규약이다(스턴 중에도 밀린다).
  - 기절 게이트의 **부수효과(시전·돌진 취소)는 유지** — 넉백 중에도 캐스트는 끊긴다.
- **⚠️ 「넉백 우선」을 시전 순서로 구현하려던 시도는 폐기했다.** `_resolve_sub`에 pre-hook을 넣어 서브 효과보다 먼저 밀어내는 안을 만들었다가 되돌렸다. 이유 둘:
  1. **효과가 없다.** `apply_knockback`은 즉시 이동이 아니라 `kb_vel`을 `KB_TIME`에 걸쳐 미는 **예약**이다. 같은 프레임엔 위치가 안 변하므로 pre/post 어느 쪽이든 관측 차이가 0이다. 실제로 순서를 뒤집던 건 위의 틱 순서였다.
  2. **공짜 넉백 구멍이 생긴다.** `_resolve_sub`가 false면 호출부가 **탄·쿨을 전혀 소모하지 않는다**(`cast_skillbook` 참조). pre-hook이면 시전 실패에도 밀어내기만 남아 키 연타로 무한 밀치기가 된다.
  - → 코드의 순서는 **읽는 사람을 위한 규약 표현**으로 남기고, 강제는 `enemy_ai` 틱이 한다(주석에 명시).
- **CC 중첩(사용자 요구 "동시 부여"):** 서브의 기절과 캡스톤 속박은 **같은 프레임에 나란히** 걸리고 각자 독립 타이머로 동시 진행한다 → 총 행동불가 = **max(기절, 속박)**, 합이 아니다. 순차로 이어붙지 않으므로 추가 조치 불요 — 규약으로 주석에 고정했다.
- **분류·전파:** 틱 순서 결함 = **code-bug**(스펙 규칙 위반 아님, spec은 넉백↔CC 상호작용을 정의한 적 없다) → **전파 없음, 로깅만.**
...
### DRIFT-145 — M1 마석이 **런 쪽만 배선**돼 허브에서 안 보이고, charges와 이중 노출 (code-bug + UX)
- **증상(사용자):** *"허브 스태시에 마석이 없고 아직 CHARGES가 남아서 소모가 어떤 식으로 되는지도 모르겠어."*
- **① `Stash`에 마석 도메인 자체가 없었다.** [[DRIFT-144]]가 마석을 `Backpack.loose`(반입분)에만 넣고 **허브 보관분(Stash)을 만들지 않았다.** 허브에서 마석을 보거나 다음 런용으로 남겨 둘 방법이 없었다. → `Stash.manastones{id:count}` + 시드 + `add/take/manastone_count` 추가.
- **🐞 ② 스태시로 옮기면 영구 삭제됐다.** `main.gd::_sync_stash_from_source`가 `match kind`로 gear/skillbook/consumable만 처리하고 **default 분기가 없어**, 마석 타일을 스태시에 놓고 창을 닫으면 그대로 증발했다. 이 sync는 Stash를 **통째로 재작성**하기 때문이다.
  - **같은 유실 경로를 며칠 전 overflow로 막아 놓고**([[DRIFT-139]] ⑤) 새 kind를 추가하면서 이 함수를 안 봤다. 방어를 세워도 **그 방어가 커버하는 지점 목록**이 없으면 다음 추가에서 또 뚫린다.
  - → `manastone` 분기 + **`_:` catch-all `push_warning`** 신설. 이제 미처리 kind는 조용히 사라지는 대신 소리를 낸다. 주석에 *"새 아이템 종류 = 여기 + `_build_stash_items` 둘 다"* 명시.
- **③ charges와 마석이 동시에 보였다.** 액션바 「Q·8」의 8은 `charges`인데 **실제 소모는 마석**이라 **거짓 정보**였다. [[DRIFT-144]]는 되돌리기 여지를 위해 `charges` **필드**를 남기기로 했는데, 그 결정이 **표시까지 남기는 것으로 잘못 이어졌다.**
  - → player-facing 탄 표기 **전부 마석 비용(◈N)으로 교체**: 액션바 키 라벨 · Q/E/R 툴팁 · 장착 슬롯 타일/툴팁 · 인벤 그리드 라벨/툴팁. **필드는 그대로 살아 있다**(폐기는 M5) — 표시만 내렸으므로 마석을 되돌리는 비용은 변하지 않는다.
- **④ 허브에 경제가 안 보였다.** 마석 보유량이 스태시 창을 열어야만 보였다 → 배치 화면에 **반입/창고를 나눠 찍는 한 줄** 상시 노출(둘은 다른 주머니고, 던전에 가는 건 반입분뿐이다).
- **🐞 ⑤ 파스 에러로 오토로드가 통째로 죽었다.** `var msid := sd.default_manastone_id()` — `sd`가 `get_node_or_null` 반환(untyped)이라 `:=` 추론 실패 → `stash.gd` 파스 실패 → **Stash 오토로드 미생성** → 허브가 null 참조. `ci_smoke`가 잡았다(main.tscn 로드 에러). 명시 타입으로 수정.
- **분류·전파:** 전부 구현 결함·표시 레이어 = **code-bug + UX**. spec 규칙 변화 없음 → **전파 없음, 로깅만.**
- **영향 파일:** `stash.gd` · `main.gd` · `inventory_ui.gd` · `inventory_grid.gd` · `equip_panel.gd` · `controlled_sheet.gd`.
- **게이트:** `ci_smoke.sh` **11/11 PASS**.
- **교훈:** *새 아이템 kind를 추가하면 **런·허브 양쪽 파이프라인**을 다 봐야 한다.* 지점 목록이 머릿속에만 있으면 빠진다 — catch-all 경고가 그 목록을 코드에 박아 넣은 것.
- **상태:** LOGGED.

### DRIFT-146 — M2 참(charm) 도입: 인벤 점유 stat 오오라 🔶 tuning + 축 대체 1건
- **근거:** `F-010` §3.11(참 그리드) · `I-007` §7. **spec에 이미 있는 규칙의 구현** → 전파 없음. 단 아래 ①은 게임에 축이 없어 **대체**했다.
- **🐞 ① spec 프로토 5종 중 1종은 게임에 축이 없다.** `CHARM-PROTO-001` **Voltaic 저항 +5%** — `take_damage(amount, attacker, from_ability)`에 **element 인자가 없어 원소별 저항이 존재하지 않는다**(`elements.gd`는 시전 측 RX/outcome 표일 뿐 피격 측 저항이 아니다). 참 한 개를 위해 원소 저항 축을 신설하는 건 과한 스코프 → **평타 속도**로 대체(별개 축 유지). 원소 저항이 다른 이유로 생기면 그때 복원한다.
- **② `CHARM-PROTO-002` 수치 하향(8% → 5%).** spec은 「**Slag** 피해 +8%」로 **원소 한정**이었다. 그 축이 없어 「주는 피해 +8%」로 일반화하면 **상시 적용이라 훨씬 세진다** — 조건부 8%와 무조건 8%는 같은 숫자가 아니다. tuning.
- **③ 최종 5종**(전부 기존 축에 얹음): 비늘=받는 피해 −5% · 불티=주는 피해 +5% · 바람=이동 +3% · 가시=**근접 피격 시 고정 반사 3** · 속결=평타 속도 +6%.
  - 가시(고정 반사)는 Sentinel/`AB-048`의 **비율 반사와 다른 축**이다 — 작은 피격에도 같은 값이 돌아가 잡몹 다수전에서 의미가 커진다.
- **④ 소유자는 멤버가 아니라 런 인벤이다.** 참은 **칸을 먹는 게 대가**라(「칸 vs 파워」) 멤버가 스스로 알 수 없다 → `InventoryUI.charm_mods()`가 합산하고 `CombatController.refresh_charms()`가 **파티 전원에 push**한다(`party_member.apply_charms`). 매 히트마다 인벤을 뒤지지 않으려는 캐시이고, 인벤이 바뀌면 push가 다시 온다.
  - 갱신 시점 = **인벤 창 닫기**(`charms_changed`). 드래그 중간 상태마다 재계산하면 들었다 놨다에 스탯이 출렁인다.
  - 인벤 미연결(샌드박스)이면 **중립값을 밀어 넣어** 잔여 효과가 남지 않게 한다.
- **⑤ 비스택.** 같은 참 2개 = 타일 2개 = 칸 2개. 스택되면 공짜로 두 배가 되어 긴장이 사라진다. 동일 효과는 **곱연산 중첩**(반사만 합산), 상한은 두지 않았다 — 운반 한도(`quartermaster`)가 이미 상한이다.
- **⑥ [[DRIFT-145]] 교훈 적용 — 이번엔 허브 파이프라인을 같이 깔았다.** 마석 때 빠뜨렸던 `Stash.charms` · `_build_stash_items` · `_sync_stash_from_source` 분기 · `make_charm_stash_item`을 **처음부터** 채웠다. 145에서 넣은 `_:` catch-all 경고가 실제로 이 누락을 잡아냈다(참 추가 직후 경고 발생 → 그 자리에서 보완). **방어가 값을 한 셈.**
- **분류·전파:** 기존 spec 규칙 구현 = 전파 없음. ①(축 대체)·②(수치)·③(효과 조합) = **tuning(로깅만)**. ①은 spec 프로토와 다르므로 **원소 저항 축이 생기면 재검토** 대상으로 남긴다.
- **영향 파일:** `data/slice01/{charms.json,id_registry.json}` · `slice01_data.gd` · `item_factory.gd` · `inventory_ui.gd` · `party_member.gd` · `combat_controller.gd` · `dungeon_run.gd` · `stash.gd` · `main.gd` · `backpack.gd` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **11/11 PASS** — 신규: 카탈로그 5종 · 스타터 시드 ↔ `charms.json` `starter_grant` 대조 · **`effect` 키가 `apply_charms`가 아는 축인지 전수 검증**(오타 하나면 참이 조용히 무효가 된다).
- **F5 필요:** ⑴ 칸 압력이 실제 선택으로 느껴지는지(참 vs 마석 vs 소비) ⑵ 5%/3%/6%가 체감되는지 — 안 느껴지면 수치를 올리는 게 아니라 **효과 종류를 바꾸는** 쪽을 먼저 본다.
- **상태:** LOGGED.

### DRIFT-147 — CS-1: Passive 폐기 · 참 일원화 (spec `d1bc78b` / `DEC-20260813-003`) 🔶 rule 구현 + 판정 3건
- **스펙 개편:** 성장의 정체성이 「캐릭터를 강하게」 → **「4명을 어떤 순서·템포로 조작하는가」** 로 이동하며 `F-020` **Passive Skill 슬롯 폐기** · 비액티브 효과 **참으로 일원화** · 조건부 참 허용(`F-010` §3.11.1). 이관 목차 = spec `docs/context/DoctrineGrowth_WorkPlan.md`.
- **🟢 게임에는 Passive 구현이 애초에 없었다.** 체크리스트 9항 중 **1-1~1-4는 할 일이 없다** — `skillType` 필드·`passiveSkillId`·`tank_bluster`·패시브 슬롯이 전부 부재. spec이 예고한 *"Slice-01은 empty 허용이 기본값이라 회귀 없음"* 이 게임에선 **더 강하게** 성립한다(필드 자체가 없음). 참 그리드(1-4 대체재)는 [[DRIFT-146]]에서 이미 구현됨.
- **실작업 3건 + 판정 3건(사용자 결정 2026-08-14):**
  | # | 항목 | 판정 |
  |---|------|------|
  | **A** 1-5 | `_apply_chapel_passive`(역할별 stat 버프) = 구 F-020 패시브트리의 **데모 근사**(코드 주석에 명시). 폐기 대상이지만 지금 지우면 CS-2까지 chapel이 무효 | **CS-2에서 Doctrine 노드로 교체할 때 함께 제거** — 그때까지 존치 |
  | **B** 1-7 | `F-022` §3.4가 *"**해당 캐릭터**의 threat만 ×2.0"* 인데 참은 `F-010` §3.11상 **런 인벤 공용**이다. 전원 ×2면 **상대 threat 순서가 불변**이라 탱커 유지력이 안 바뀐다 — 스펙 내부의 결이 갈리는 지점 | **역할 한정**(`applies_to`) — 구 `tank_bluster`가 Tank 패시브였던 원 의도를 스코프로 이관 |
  | **C** 1-8 | `trapDetection` — 게임에 **탐지도 Objective Hazard Mark도 없다**(`F-006` §3.7 전체 미구현). 참에 필드만 넣으면 읽는 곳이 없다 | **보류** — `F-006` §3.7 구현 시 함께 |
- **① 조건부 참 훅(1-6).** 조건은 **멤버 상태**라 인벤 집계 시점(창 닫기)이 아니라 **효과가 필요한 순간**에 평가돼야 한다 → 정적 mods는 종전대로 push하고, **조건부는 id만 넘겨 멤버가 자기 상태로 판정**한다(`charm_condition_met`). 조건 어휘는 신규 문법 없이 기존 상태만 참조(spec 하드 제약).
- **② `CHARM-PROTO-006` 허장허세** — `charm_threat_mult` ×2.0, `applies_to: ["Tank"]`. floor/pulse/fixed threat 직접 보정은 **여전히 금지**(`DEC-20260511-009` 의도 보존, 배율만).
- **③ `CHARM-PROTO-007` 가호** — spec 조건 「`Warded` 보유 중」을 게임의 **보호막 보유**(`shield`/`_ward_hp` > 0)로 대응. 이동방해(Rooted/Pinned/Frozen + 감속) **1회 무효**, 보호막 재획득 시 재충전. **자기부여 둔화(Sentinel 태세 `quiet`)는 막지 않는다** — 그건 방해가 아니라 대가다.
- **🔶 스펙 미정의 축 = `applies_to`.** spec `F-010` §3.11에 역할 스코프 개념이 없다. B 판정의 결과물이므로 **전파 후보** — 스펙이 참을 캐릭터 단위로 상정하는 문장(`F-022` §3.4)과 공용 인벤 모델 사이의 간극을 메우는 최소 축이다.
- **⚠️ 위험 등급 변경(스펙 고지 사항):** 구 Passive는 **Hub Safe**였으나 참은 **런 인벤 At Risk**다. 게임엔 승계 대상(`trapDetection`)이 아직 없어 **현재 실피해는 없지만**, C가 풀리는 시점에 UI 피드백이 필요하다.
- **게이트(신규):** `hub_smoke`에 **참 하드 제약 검증** — `applies_to` 역할 유효성 · `condition` 구현 여부 · **`controlContext` 출현 금지**(doctrine 전용, `D-021` §6-3) · **발동 입력 필드 금지**(`cast_s`/`cooldown_s`/`target`/`manastone_cost` → 있으면 그 순간 `AB-###`) · `tank_bluster` 잔존 0. effect 축 검증을 **정적/조건부로 분리**(소비처가 다르다).
- **분류·전파:** 스펙 규칙 구현 = 전파 없음. **`applies_to` 축만 전파 후보(`PENDING-PROP`)**. 수치(×2.0·1회)는 tuning.
- **영향 파일:** `data/slice01/{charms.json,id_registry.json}` · `inventory_ui.gd` · `combat_controller.gd` · `party_member.gd` · `tools/hub_smoke.gd` · `spec_ref.json`.
- **재핀:** `09e045c` → **`fd317b3`**. 워크플랜 본문의 이관 해시(`d1bc78b`)가 **그 다음 커밋에 기입**돼 있어, `d1bc78b`로 핀하면 워크플랜이 자기 자신을 못 가리킨다(사용자 지적).
- **게이트:** `ci_smoke.sh` **11/11 PASS**.
...
### DRIFT-149 — `Taunted` 마커 → **정식 디버프**로 승격 (impl · 전파 없음)
- **판정(사용자, 2026-08-14):** *"턴트상태를 디버프로 추가하고 기존것을 변경해줘."*
- **배경:** [[DRIFT-148]]에서 `Taunted`를 `enemy_unit` **전용 타이머 마커**(`taunt_timer_s`/`taunted_by`)로 넣었다. doctrine 조건·`DurationExtend`를 굴리기엔 충분했지만 **플레이어에겐 안 보였다** — 상태 아이콘도, 한글 라벨도, 부여 팝업도, `enemy_info` 상태칩도 붙지 않았다. 도발은 "지금 누가 누굴 붙들고 있나"를 **읽히게 해야** 의미가 있는 상태다.
- **① 공용 상태 컨테이너로 이관.** `outcome_status.gd`(`STATUS-OUTCOME-CORE` 컨테이너지만 실제론 `Rooted`·`Pinned`·`Vulnerable` 등 `STATUS-ACTOR-CORE`도 함께 담고 있다)에 `Taunted` 등재 — `COLOR`(주황) + `KO`("도발") + `float_text.OUTCOME_KO`. 전용 타이머를 지우고 지속·만료를 컨테이너에 넘겼다. **아이콘·라벨·팝업·틱·상태칩이 기존 기계 그대로 붙는다** — 전용 타이머는 그 전부를 따로 배선해야 했다.
  - `taunted_by`(출처)만 `enemy_unit`에 남는다 — 컨테이너엔 출처 축이 없다.
  - `remaining(id)` 접근자 신설 — `DurationExtend`처럼 **현재 값 위에 얹는** 보정이 잔여를 읽어야 한다(`apply`가 `maxf`라 「잔여 + add」로 재적용).
- **② 이동·공속에 등재하지 않았다**(`MOVE_MULT`/`ATK_MULT` 미포함). 어그로는 여전히 **threat/floor**가 결정하고, 이 상태는 그걸 **표시**할 뿐이다. 여기 값을 넣으면 도발이 조용히 슬로우가 되어 전투 수식이 바뀐다 — `QA-032` 기준선도 함께 흔들린다.
- **🐞 ③ 게이트가 기존 누락 2건을 잡았다(그리고 그건 누락이 아니었다).** 상태 라벨 3표(`COLOR`/`KO`/`OUTCOME_KO`) 대조 게이트를 세우자 `Scorched`·`Poison`이 걸렸다. 확인 결과 **의도적 제외**였다:
  - `Poison` — 호출부가 자체 「중독」 팝업을 띄운다 → 등재하면 **중복 팝업**.
  - `Scorched` — 화염존 **체류 중 매 프레임 재적용** → 일반 팝업이면 화면 도배.
  - → 라벨을 추가하는 대신 **사유를 적은 예외 목록**으로 처리했다. 빈 예외 목록은 곧 구멍이 되므로 *왜* 빠지는지를 게이트 안에 남겼다.
  - 이 게이트가 필요한 이유: `Tethered`가 정확히 같은 방식으로 새어 나갔다([[DRIFT-132]] — `KO`엔 있는데 `OUTCOME_KO`엔 없어 부여 팝업이 안 떴다). 상태 추가는 앞으로도 계속 생긴다.
- **분류·전파:** 표시 레이어 + 내부 구조 변경 = **impl**. spec은 `Taunted`를 `STATUS-ACTOR-CORE`에 이미 갖고 있고 게임이 그걸 늦게 구현한 것이라 **규칙 변화 없음** → 전파 없음.
- **영향 파일:** `outcome_status.gd` · `float_text.gd` · `enemy_unit.gd` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: 상태 라벨 3표 정합(사유 있는 예외 2) · `Taunted` 등재 · **이동·공속 미개입** 단언(나중에 누가 슬로우를 얹으면 운다).
- **상태:** LOGGED.

### DRIFT-150 — M3 스킬 트리(`F-020` §3.10) + CS-2 잔여 doctrine 구매 경로 🔶 rule 구현 + tuning
- **근거:** `F-020` §3.10 · `F-029` `chapel` Tier · `I-007` §14.5. **구 분석 의뢰(N=3)의 후임** — `F-009` §3.9.4가 *"분석 의뢰 UI 제거 — 트리 노드 클릭 = 해금"* 으로 못박았다.
- **① 노드 4종.** `Slot`(gear 슬롯 +1) · `Unlock`(AB 모딩·구매 허용) · `Upgrade`(행동 발전) · `Doctrine`(파티 운용). **`Passive` 노드는 없다** — CS-1에서 슬롯이 폐기되고 그 자리를 `Doctrine`이 받았다.
- **② 해금 상태를 `HubProfile`에 뒀다**(별도 오토로드 안 만듦). 트리가 대체하는 `shop_listing_unlocked`, sink가 되는 금고(`hubVault`), 게이트가 되는 `chapel` Tier가 **전부 거기 있다** — 후임을 전임 옆에 두면 한 곳만 보면 된다. `is_shop_unlocked`는 **트리 우선 + 구 분석 폴백**으로 합쳤다(M5까지 공존; 마이그레이션 중 잠기는 걸 막는다).
- **③ 참조 무결성을 로드에서 막았다.** 노드가 가리키는 `base_ability_id`·`doctrine_id`·`prerequisite`가 실재하지 않으면 **눌리지 않는 버튼**이 생긴다(= 조용한 실패, 이 레포가 세 번 당한 유형). 선행 노드 검사는 **전부 적재된 뒤** 돌려 정의 순서에 의존하지 않게 했다.
- **🐞 ④ 플테 플래그가 게이트를 무력화하고 있었다.** `PLAYTEST_TREE_ALL_UNLOCKED`(전 노드 해금, D4의 트리판)를 `const`로 뒀더니 `is_node_unlocked`가 항상 true라 **스모크가 트리 로직을 안 타고 전부 통과**했다 — 게이트가 아무것도 증명하지 못하는 상태. `var`로 바꿔 스모크가 **끄고** 재게 했다.
  - 이건 이번 세션에서 게이트를 계속 두껍게 깔아 온 것과 같은 이유다: **통과하는 게이트보다 무의미한 게이트가 더 위험하다** — 있으면 검증됐다고 믿게 된다.
- **⑤ `hub_tree_panel` 신설 — CS-2 잔여(2-5) 해소.** doctrine을 살 수 있는 **유일한 경로**라 이게 없으면 `F-030`이 코드에만 있고 게임엔 없다(`DoctrineProfile`이 영원히 중립). 구매(재료 차감)와 **활성**(Identity당 1개·재배치 자유·환불 없음)을 분리해 노출하고, 거부 사유를 **말로** 돌려준다(비활성 버튼만 있고 이유가 없으면 "왜 못 사지"가 남는다).
- **미확정 기본값(사용자 승인 — 플래그만 남김):** **U6 트리 단위 = 클래스별**(`F-029` 「클래스당 트리 루트」) · **U7 가격 = placeholder**(`F-030` OQ-1 · `F-029` 수치 `D-###` 미작성). 전부 tuning — 밸런싱 회차에 일괄.
- **소비처 미배선(후속):** `Slot` → `gear_skill_slot_count`(**M4**) · `Upgrade` `param_override` → AB 파라미터(소비처는 해당 effect). 데이터·해금·파생 게터(`tree_slot_bonus`/`ability_upgrades`)까지가 M3 범위다.
...
- **⚠️ 판정 필요 (`PENDING-PROP`) — 착수 시 4건이었으나 ⓐ는 ⑦에서 자체 해소, 실질 3건 + 표 정합 1건:**

| # | 항목 | 게임이 택한 값 | 근거·충돌 |
|---|------|----------------|-----------|
| ⓐ | 슬롯 증가 경로 | 트리 `Slot` 노드 **단일 축** + `smithy` `facility_req` 게이트 | **아래 ⑦에서 자체 해소** — 남은 전파는 `F-029` 표 정합 1건 |
...
- **🐞 ⑦ (자체 발견·같은 커밋 뒤 수정) 슬롯 수를 트리와 시설 양쪽에서 더해 이중 계산하고 있었다.** OPS_30 impact scan 첫 삽에서 드러났다 — `F-008` §3.10이 사다리를 **못박아 뒀다**: *starter **1** (Q) → `smithy` T2 **2** → T3 **3**.* 그리고 `F-020` §3.10 `Slot` 노드는 「예: **`smithy` 연동**」이다. 즉 둘은 **같은 +1**이고, 트리 노드가 **구매 표면**이며 대장간 tier는 **게이트**다. 더하면 대장간 없이 트리만으로 3칸이 열려 스펙 사다리가 무너진다.
  - → `gear_slot_count`에서 `smithy` 항 제거. `tree_check`에 **`facility_req`** 게이트 신설(거부 사유 `facility_req`). `TREE-{ROLE}-SLOT1`(대장간 T2) · `SLOT2`(T3) 사다리로 재구성.
  - → **루트 노드(`TREE-*-ROOT`) 삭제.** 무비용·무효과로 다른 노드의 선행 자리만 채우고 있었다 — 「샀는데 아무 일도 안 일어남」이 트리의 첫 경험이 되는 구조다. 진입 게이트는 이미 `chapel` T1이 맡고 있으므로 중복이었다. 선행 관계는 **의미 있는 곳에만** 남겼다: `U1 → U2`(해금 사슬) · `U1 → UP1`(**해금 안 한 AB는 발전시킬 수 없다**) · `SLOT1 → SLOT2`.
  - **이게 판정 ⓐ의 실체였다.** "스펙 두 문서가 충돌한다"고 적었지만, 실제로는 `F-008` §3.10을 안 읽고 `D-019` §3의 한 줄(「smithy T2/T3·트리로 증가」)을 **두 축 합산**으로 읽은 것이 원인이다. 스펙은 일관돼 있었다. 남는 전파는 `F-029` 요약표(`smithy` T1 「gear 슬롯 +1」)가 자기 문서의 tier 표·`F-008` §3.10과 어긋나는 **1건뿐**이다.
  - **부수 효과:** `smithy` T3는 `F-029`에서 Expansion이라 **세 번째 슬롯은 Slice-01에서 도달 불가**다. 이게 맞다 — 3칸 gear는 존재하되 열리지 않는다. 플테는 `PLAYTEST_TREE_ALL_UNLOCKED`가 우회한다.
...
- **상태:** ✅ **전파 완료(2026-08-20, spec `16031d3` · `DEC-20260820-002`)** · M5 완료 · 재핀 완료.
  - **모딩 시술비 → `F-009` §3.9.3 `modInstallFee`로 정본화.** impact scan에서 `I-007` §14.2가 *"gear 잭팟·교체 = **스킬 세트 재조립 비용**"* 을 이미 정해 둔 것이 나왔다 — 게임이 만든 건 새 규칙이 아니라 **그 비용을 실을 자리**였다(스킬북이 Frozen이라 「재구매」할 물건이 없고 트리 해금은 영구라, 값을 붙일 곳은 시공뿐). 구 문구 「재구매」는 **그대로 두면 위험**해서(문자대로 읽으면 폐기된 아이템이 되살아난다) 함께 고쳤다.
  - **부수 전파 4건** — ⓐ `F-009` §3.9.2 **상자·아군 캐시** Deprecated 행(구 표는 per-kill만 막고 있었다) · ⓑ `F-008` §3.10.1 굴림 **극소화 확정**(검토 → ±5%/±2%) · ⓒ `D-018` §9 **잉여 인스턴스 처분 + 선행 조건 + 「`charges` 폐기는 게이트까지」** · ⓓ `F-020` §3.10 **`Unlock` 전수 커버 불변식**.
  - ⓒ·ⓓ는 **이 레포가 M5 착수 전 대조에서 실제로 부딪힌 두 구멍**이다. 지침대로 실행했으면 44 AB 영구 잠김 + 스태시 전량 증발이었다 — 그래서 「대조하고 진행」이 옳은 순서였다.
  - **후속 TODO(spec):** `modInstallFee` 수치 `D-###`(현재 구 생본가 placeholder 승계) · 첫 런 슬롯 1칸 체감 F5.

### DRIFT-153 — M6: 허브 → **마을 화면** + 기능 기준 건물 재편 · 난이도 선택 폐기 🔶 rule 구현 + 판정 4건(`PENDING-PROP`)
- **근거:** `UI-005` §3(배치) · `F-029`(시설 tier·`map_pos`) · `F-020` §3.2.0(첫 런 게이트) · `F-008` §3.10. 사용자 판정: **마을 UX(다키스트던전 햄릿)** · **파티 카드는 gear만, 나머지는 툴팁** · **난이도 제거** · **필기소를 필기상점에 흡수** · **미건립은 폐허 표시** · **기능 기준 건물 재편**.
- **① 테이블 → 마을.** 구 허브는 560×400 패널에 버튼 8개가 세로로 쌓인 목록이었고, **누가 뭘 들었는지가 화면에 없었다**(스태시를 열어야 알 수 있었다). P4b가 성장을 「캐릭터 강화」→**「이 건을 어떤 무기로 만들까」**로 옮긴 이상 허브의 1급 정보는 **파티 4인의 건 구성**이다.
  - 건물은 **`F-029` 시설 그대로**다 — 새 화면을 발명한 게 아니라 이미 있는 tier 표를 공간으로 옮겼다. 좌표는 `facilities_tiers.json` **`map_pos`(정규화 0..1)** 가 소유한다: 아트가 배경을 넣으면 **코드를 고치지 않고** 좌표만 맞춘다.
  - **파티 스트립 = 건 이름만.** 정체성 규약·Q/E/R·◈비용·결속 1줄은 **마우스 오버 툴팁**으로. 카드에 다 적으면 4장이 화면을 먹어 마을이 안 보인다.
- **② 기능 기준 건물 재편** — 「지금 필요한 기능에 따라」(사용자). 재편의 요점은 **사는 곳과 조건이 같은 건물**이 되는 것이다.

| 건물 | 하는 일 | 전에는 |
|------|---------|--------|
| **대장간**(`smithy`) | 건 모딩(Q/E/R·시술비·gear 교체) + **슬롯 확장** | 모딩 패널 + 성소 트리(`Slot`) |
| **필기상점**(`scribe_shop`) | 슬롯 스킬 **해금**(`Unlock`)·**강화**(`Upgrade`) | 성소 트리 |
| **성소**(`chapel`) | **운용 교리**(`Doctrine`) | 성소 트리(4종 혼재) |
| **무기고**(`armory`) | gear 구매 | 상점 패널 안 |
| **군수**(`quartermaster`) | 소모품 보급 | 상점 패널 안 |
| **창고**(`stash`) | 보관·금고 | 동일 |
| **성문**(`barracks`) | 포메이션 · 반입 확인 · 출정 | 허브 인라인 |
| ~~필기소~~(`scriptorium`) | **제거** — 분석 폐기(M5)로 기능 0 | — |
  - **`Slot` 사다리가 대장간으로 돌아갔다.** 트리는 성소에 있는데 `Slot`만 대장간 tier가 게이트하는 기형이었다 — 유저는 「왜 성소에서 사는데 대장간을 올리라 하지」를 겪는다.
  - **`Unlock`/`Upgrade`에 `scribe_shop` tier 게이트 신설.** 해금과 시술이 한 건물로 모였으므로 게이트도 같아야 한다: 열 수는 있는데 못 새기는(또는 반대) 상태가 생기면 어느 건물을 올려야 하는지 알 수 없다.
  - `hub_tree_panel`은 **노드 타입 필터**만 받아 두 건물이 같은 데이터를 다른 눈으로 본다. `hub_economy_panel`도 `mode`로 무기고/군수를 가른다 — **재고가 달라야 마을에 문이 둘일 이유가 있다**.
- **③ 건물 안 UX.** 사용자 요청으로 마을 껍데기보다 먼저 손봤다.
  - **대장간:** Q/E/R이 **3칸 타일**로 보이고(텍스트 행일 땐 「여기가 슬롯이다」가 안 읽혔다), 카탈로그에 **계열 필터**를 달아 49행을 줄였으며, **「고르기 → 결속 미리보기 → 확정」 2단계**로 바꿨다. 행마다 [배치]를 두면 **클릭 한 번에 시술비가 빠져나간다** — 되돌릴 수 없는 값을 치르기 전에 이 조합이 무엇을 얻는지 읽게 하는 것이 이 화면의 요점이다(결속은 gear × 정체성 × 슬롯 삼중 매치라 끼워 보기 전엔 알 수 없던 정보다).
  - **필기상점:** `Unlock` 63개를 **계열(`skill_family`)로 접고**, `Upgrade`를 자기 `Unlock` **아래 들여쓰기**로 붙여 선행 관계를 눈에 보이게 했다. 그룹 축을 계열로 잡은 이유는 **모딩 화면의 필터와 같은 축**이라 「여기서 연 걸 저기서 끼운다」가 이어지기 때문이다.
- **④ 난이도 선택 폐기.** 지역·입장조건이 그 축을 가져가기로 했다(사용자 로드맵). **UI만 제거**하고 `RunLoadout`·spawn 데이터·`SHARED_DROP_BY_DIFF`는 **보존**했다 — `get_difficulty()`가 manifest 폴백을 이미 갖고 있어 지역이 나중에 값을 넣기만 하면 된다.
  - **🐞 파급 1건:** `Q-HUB-020`(무기고 T1)이 「Hard 인카운터 클리어」로 게이트돼 있어, 난이도를 못 고르면 `hard_cleared`가 영영 false → **무기고가 안 열려 gear를 못 산다**. → **고정 보스**로 옮겼다: `spawn_table` `force_overrides["P-BOSS-01"] = "ENC-BOSS-001"`(문자열 = 난이도 무관 강제). `P-BOSS-01` 풀과 `RM-BOSS` 방은 **이미 있었고** Hard 행만 있어 안 나왔을 뿐이다. **「어려운 관문」을 토글이 아니라 맵의 방이 소유한다.**
- **⑤ 출정 게이트 = 성문.** ~~Confirm Loadout~~ 폐기(gear=정체성 동기화라 확정할 것이 없었다) → `F-020` §3.2.0 **첫 런 역할당 슬롯 ≥1 차단** + 이후 런 **빈 슬롯 경고(비차단, 세션 1회 확인)**. 판정축은 `HubProfile.runs_started` 신설 — 추출/전멸 카운터로는 「시작해 놓고 이탈한 런」을 구분할 수 없다.
- **세이브 마이그레이션 없음** — 사용자 판정(*"기존 세이브는 다 날려도 괜찮아"*). `scriptorium` tier가 세이브에 남아도 `FACILITY_IDS`에 없어 무시된다.
- **⑥ UI 토대 신설 — `HubTheme` + `HubPanel`.** 화면 8개가 각자 `OK/BAD/DIM/ACCENT`를 선언하고 노드마다 `add_theme_font_size_override`를 **47번** 부르고 있었다. 같은 「제목」이 화면마다 다른 크기, 같은 「경고」가 다른 빨강 — 「정리가 안 된 느낌」의 실체였다. Godot **타입 변주**로 옮겨 override **47 → 0**. 창 크롬(dim/가운데창/제목줄/Esc)도 6곳 복사본을 `HubPanel` 하나로.
- **🐞 ⑦ 「폰트가 안 맞는」 것의 정체 = 가짜 컬럼.** 모든 목록이 `HBox + Label(custom_minimum_size)`였는데 그건 컬럼이 아니라 **최소 폭 힌트**라, 한글처럼 글자 폭이 들쭉날쭉하면 줄마다 어긋난다. → **`GridContainer`**(열 폭 = 가장 넓은 셀). 그룹 머리글은 빈 칸으로 한 줄을 채워 **표 한 장**을 유지한다(그룹마다 컨테이너를 따로 두면 그룹별로 열 폭이 갈려 같은 증상이 재발한다).
- **🐞 ⑧ (내가 만든 회귀 2건, 사용자 스크린샷으로 발견)**
  - **`autowrap` + 폭 없음 = 글자 단위 줄바꿈.** Godot에서 autowrap을 켜면 라벨 **최소 폭이 0**이 돼, 부모가 폭을 강제하지 않으면 「거암 / 의 대 / 방패」로 쪼개지고 카드가 그 폭으로 쪼그라들어 글자가 밖으로 흘렀다. → `HubTheme.line()`(한 줄 + 말줄임) 신설, `para()`는 `min_w` 필수. 좁은 칸은 전부 `line()`.
  - **`RichTooltip`이 `Panel`(비컨테이너)이었다.** 자식을 배치하지도 자식에 맞춰 커지지도 않는데 높이를 `0`으로 두는 순간 **배경이 0px가 돼 화면이 통째로 비었다**. → `RichPanel`(PanelContainer판) 신설. 툴팁 렌더는 `RichTooltip.make()` 재사용.
  - **`set_anchors_preset`은 앵커만 바꾼다** — 오프셋은 「현재 사각형 유지」로 재계산되므로 갓 만든 0×0 컨트롤은 **전체 화면에 앵커된 0×0**이 되고, 안쪽 `CenterContainer`가 0폭에서 가운데를 잡아 **창이 전부 좌상단에 붙었다**. → `set_anchors_and_offsets_preset`.
  - 셋 다 **헤드리스 스모크가 못 잡는 종류**다(「에러 없이 로드됨」과 「보이는가」는 다른 질문). 노드의 실제 `size`/`position`을 재는 일회성 진단으로 확인했다.
- **⑨ 의뢰를 「장부」에서 「그 장소」로** (사용자 판정: *「퀘스트를 그 퀘스트의 결과로 해금되는 공간에서 주고 승낙할 수 있게」*). `HubProfile.quest_accepted` 신설 — **수락하지 않은 의뢰는 완료되지 않는다**(조건을 우연히 충족해도). 그래야 수락이 형식이 아니라 선택이 된다. 조용히 흘리지 않고 사유를 로그로 남긴다. 수락 직후 재평가하므로 이미 채워 둔 상태로 받으면 그 자리에서 완료된다.
  - **`hub_building_panel` 신설이 `hub_facilities_panel`을 대체.** 폐허를 눌렀는데 목록이 열리고 거기서 다시 그 건물을 고르는 건 마을을 만든 이유를 지운다. 승급도 여기로 합쳤다(중복 제거) — 대신 마을 카드에 **뱃지**(`!` 맡을 의뢰 / `▲` 건립·승급 가능). 좌클릭 = 건물의 일 / 우클릭 = 건물 정보.
  - **🐞 `run_controller`가 죽은 의뢰를 완료시키고 있었다** — `Q-HUB-010`(필기소)은 M6에서 건물째 사라졌는데 호출이 남아 있었다. 목표(GIMMICK-DEMO-01) 클리어 → **`Q-HUB-012`(필기 상점 개장)** 으로 옮겼다(재료 임계값보다 사건이 낫다).
- **⑩ 창고 시드 = 스타터만** (사용자 판정). 구 시드는 스페어 **17종**을 부어 놔 첫 화면에서 이미 다 가진 상태였다 — **획득이라는 축 자체가 없었다**. 스타터 4종은 착용 중(`Backpack.equipped`)이므로 창고는 **비어서** 시작한다. 참도 같은 이유로 전량 → `starter_grant` 2종.
  - **🐞 무기고 카탈로그에 `Nuker`가 없었다.** T1·T2 모두 Tank/DPS/Healer뿐 — 전 카탈로그 시드가 가려 왔다. 시드를 접으면 **누커만 확정 획득처가 사라져** 드롭 운에만 매달린다. T1 `scout_frame`(Long) · T2 `flank_knife`(Melee·잠행) 추가.
  - **스타터를 드롭 풀에서 제외** — 4명이 착용 중이라 중복은 꽝이다. 창고를 비운 뒤로 상자는 **두 번째 건을 만나는 자리**가 됐다.
  - 스타터 gear는 슬롯 **1칸**, 나머지 23종은 3칸 → **두 번째 건 = 두 번째 슬롯**이 초반 성장의 첫 마디가 된다.
- **⚠️ 판정 필요 (`PENDING-PROP`):** ⓐ 트리가 **성소 단일 → 세 건물 분산**(`F-020` §3.10 「UI: chapel T1+」) · ⓑ **`scriptorium` 제거**(`F-029` 8→7시설) + `Unlock`/`Upgrade`의 `scribe_shop` tier 게이트 · ⓒ **미건립 폐허 표시**(`F-029` `smithy` T0 「맵 비표시」와 충돌 — 숨기면 첫 플레이에 빈 땅만 보이고 「여기 뭐가 더 있나」를 알 길이 없다) · ⓓ **난이도 선택 폐기 + `Q-HUB-020` 고정 보스 재게이트**(`F-029` §3.3 완료 조건) · ⓔ **의뢰 수락 게이트**(`F-029` §3.3 — 수락 개념 자체가 스펙에 없다) · ⓕ **창고 시드 = 스타터만**(구 D4 「전 카탈로그 개방」의 재해석) + **무기고 카탈로그는 4역할을 덮어야 한다**는 불변식.
- **영향 파일:** **`main.gd`(전면 재작성)** · `scenes/main.tscn`(스텁 → 빈 Control) · **신규 5종**(`hub_theme.gd` · `hub_panel.gd` · `hub_gate_panel.gd` · `hub_building_panel.gd` · `rich_panel.gd`) · **재작성 5종**(`hub_modding_panel` · `hub_tree_panel` · `hub_economy_panel` · `hub_quest_panel` · ~~`hub_facilities_panel`~~ 삭제) · `hub_profile.gd` · `stash.gd` · `loot_service.gd` · `run_controller.gd` · ~~`loadout_stub.gd`(삭제)~~ · `facilities_tiers.json` · `quests.json` · `spawn_table.json` · `id_registry.json` · `tools/hub_smoke.gd` · `tools/party_pool_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: **건물 ↔ 화면 1:1**(양방향 — 「눌러도 아무 일 없는 건물」과 「들어갈 수 없는 화면」을 둘 다 잡는다) · 시설 목록 ↔ 데이터 1:1 · **전 시설 `map_pos` 보유**(빠지면 전부 겹쳐 쌓인다) · **트리 노드 유형 전원 소유 건물 있음**(고아 유형 = 살 수 없는 노드) · 필기소 제거 단언 · 필기상점 T0 해금 차단 · **고정 보스가 Normal에서 실제로 스폰되는가** · 일반 ENC로는 무기고 미해금 · **미수락 의뢰는 조건을 채워도 미완료** + 수락 즉시 재평가 · **모든 의뢰가 어느 건물에선가 수락 가능**(받을 곳 없는 의뢰 = 그 시설 영구 잠김) · 시설 tier → 의뢰 참조 무결 · **창고 시드 기어 0**(「다 가진 채 시작」 재발 방지) · **무기고 카탈로그 4역할 전원 커버**.
- **상태:** ✅ **전파 완료(2026-08-21, spec `660706e` · `DEC-20260821-001`)** · M6 완료 · 재핀 완료.
  - ⓐ 트리 3건물 분산 → `F-020` §3.10에 **유형 × 건물 × 게이트 표** 신설. ⓑ `scriptorium` 폐기(8→7) + `scribe_shop` tier = **AB tier 상한** 재정의 → `F-029`·`D-029`. ⓒ 미건립 **폐허 표시** → `UI-029`(구 `smithy` T0 「맵 비표시」 **철회**). ⓓ 난이도 폐기 + `Q-HUB-020` **고정 보스** → `F-029` §3.3.1. ⓔ **의뢰 수락 게이트** + 「그 장소에서 받는다」 + **불변식**(모든 의뢰는 어느 건물에선가 수락 가능) → `F-029` §3.3 · `UI-029` · `D-029`. ⓕ **창고 시드 = 스타터만** → `F-008` §3.7, **`armory` `catalog` 4역할 불변식** → `F-029` §3.5.
  - **곁다리 해소:** `F-029` **OQ-4**(Nuker armory 세트 Deferred)가 닫혔다 — 창고 시드를 접자 「확정 획득처 없음」이 곧 그 역할의 영구 정체가 돼 더는 미룰 수 없었다.
  - **후속 TODO(spec):** ⓐ 두 번째 건을 얻어도 `smithy` T2가 없으면 칸이 안 열린다(플테 확인; 해소는 `smithy` T2를 초반에 여는 쪽 — 스타터 `gearSkillSlotCountMax` 상향은 `F-008` §3.10 사다리를 깬다) ⓑ 지역·입장조건이 난이도 축을 가져갈 때 `spawn_table` `difficulty`·`RunLoadout.difficulty`를 어디서 세팅할지 확정.

### DRIFT-154 — 대장간 사다리 도달 가능화: T1 문턱 인하 + **T3 실체화**(구 Expansion) 🔶 rule 구현 + tuning
- **근거:** `F-029` §3.5 `smithy` tier 표 · `F-008` §3.10 슬롯 사다리 · `D-029` §3 재료 출처. 사용자 판정: *「1번 제안대로 하고 … 2번은 도달 가능하도록 재편」*.
- **🔴 ① 건 모딩이 ~20런 뒤에 열리고 있었다.** `smithy` T1이 `haul_forge_coal` **×8**을 요구하는데, 연료는 **Deep 분기 전용**이다(`ENC-DEEP-001` 0.4/런 · 구 `ENC-HARD-005` 0.15는 난이도 폐기로 사망). 대장간 T1 = **건 모딩 해금**이라 P4b의 중심 기능이 그때까지 통째로 잠긴다 — 첫 런 프리모딩 Q 하나로 20런을 버티게 된다.
  - → **재료를 진행 단계에 맞춰 재편.** T1 = `haul_ward_splinter` ×6(**2.4/런**, 가장 흔함) · T2 = 파편 ×10 + **연료 ×2**(심층에 갈 이유가 여기서 생긴다) · T3 = 연료 ×6 + `haul_deep_core` ×1(반복 방문). `Q-HUB-030` 완료 조건도 연료 → 파편.
  - **연료를 없앤 게 아니라 뒤로 밀었다.** 「대장간에 심층 연료가 든다」는 서사는 유지하되, **첫 삽**만 초반 재료로 뜬다.
- **🔴 ② 3칸 gear의 세 번째 칸이 도달 불가였다.** `F-029`가 `smithy` **T3+를 Expansion**으로 미뤄 뒀는데, 슬롯 사다리가 `smithy`에 묶인 이상(`DEC-20260820-001`) **T3가 없으면 `TREE-*-SLOT2` 노드를 영원히 못 산다**. gear 23종이 `gearSkillSlotCountMax = 3`을 갖고 있는데 그 칸이 존재하되 열리지 않는 상태 — 「존재하되 도달 불가」는 튜닝이 아니라 **데이터 결함**이다.
  - → **T3 실체화**: 효과 `gear 슬롯 +1 (3칸) · F-008 §3.8 재조합` · **`Q-HUB-032` 신설**(`ENC-DEEP-001` 클리어 1회 — 「심층 노두를 열어라」) · haul 연료 ×6 + 코어 ×1.
  - 완료 조건을 **심층 ENC 클리어**로 잡은 이유: 사다리의 마지막 칸이 「재료를 더 모아라」로 끝나면 반복일 뿐이다. **가 본 적 없는 곳에 가는 것**이 마지막 단계의 대가로 맞다.
- **③ 난이도 폐기의 파급을 여기서 마저 확인했다**([[DRIFT-153]] ④ 연장). `ENC-HARD-*`가 Normal 경로에서 사라졌으므로 재료 공급을 전수로 재계산했다 — 결과: **9종 전부 Normal 경로에 공급이 있다**(파편 2.4 · 판금 0.9 · 필기파편 0.9 · 중계 0.75 · 연료 0.4 · 잉크 0.35 · 프레임 0.2 · 코어 0.15 · 유리 0.1 /런). 모든 풀에 Normal 행이 있고 `P-BOSS-01`은 고정 override라 **빈 풀은 없다**. 문제는 공급이 끊긴 게 아니라 **요구가 공급 곡선과 안 맞았던 것**이다.
- **분류:** 시설 tier 표의 요구 재료 = **tuning**, T3 실체화 + `Q-HUB-032` 신설 = **rule**(`F-029` §3.3/§3.5 · `PENDING-PROP`).
- **영향 파일:** `facilities_tiers.json` · `quests.json` · `id_registry.json` · `hub_profile.gd` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: **대장간 T3 실재** · T1이 **연료 없이** 파편만으로 건립 가능 · T2 → 슬롯 2칸 · T3는 **심층 ENC 전 차단** · 심층 클리어 후 T3 적용 · **플테 우회를 끄고 `SLOT1`·`SLOT2`를 실제로 구매**해 보너스 2 도달(구 스펙에선 `SLOT2`가 영원히 잠겨 있었다 — 사다리의 목적지를 단언한다).
- **상태:** ✅ **전파 완료(2026-08-21, spec `1bab866` · `DEC-20260821-002`)** · 재핀 완료.
  - `F-029` `smithy` tier 표 전면 교체(T1 파편 · T2 파편+연료 · **T3 신설**) · `Q-HUB-032` 등재 · T0 「맵 비표시」 → **「폐허 표시」** 통일.
  - **`F-029` OQ-3 범위 축소** — 「`smithy` T3+ 전체 Deferred」 → 「재조합 **비용·화이트리스트**만」. T3가 Slice-01로 내려왔으므로 **시설 게이트는 더 이상 Expansion이 아니다**. Expansion 목록도 `smithy` **T4+** 로 정정, `F-008` OQ-1 정합.
  - **후속:** 새 사다리의 **체감 속도**는 플테 확인(T1 파편 6 ≈ 3런 · T2 파편 10 + 연료 2 ≈ 5 Deep 방문 · T3 연료 6 + 코어 1 + 심층 클리어). 수치는 `modInstallFee`·트리 가격과 함께 밸런싱 회차에서 `D-###`로.

### DRIFT-155 — 「건 하나에 스킬 하나는 기본」: 첫 칸 무료 — 시술비 × 출정 게이트 교착 해소 🔶 rule 구현 + 판정 1건(`PENDING-PROP`)
- **근거:** `F-009` §3.9.3 `modInstallFee`(`DEC-20260820-002`) · `F-020` §3.2.0 첫 런 슬롯 게이트 · `F-008` §3.10. 사용자 발견·판정.
- **🔴 신규 세이브에서 2클릭이면 닿는 소프트락이었다.**
  ```
  새 게임 (ward_scrap 0 · Tank Q = AB-033 프리모딩)
    → 대장간 → [Q] 칸 비우기        ← 무료·무조건
    → 출정 차단 (첫 런 역할당 ≥1 필요, F-020 §3.2.0)
    → ward_scrap은 추출 성공으로만 들어온다 → 영원히 못 나감
  ```
  **출정해야 재화가 생기는데 재화가 없어서 출정을 못 한다.** `ward_scrap` 초기값 0 · 유일 획득처는 `run_end_controller`의 추출 보상.
- **🔴 두 번째 차단이 겹쳐 있었다.** 신규 세이브는 `scribe_shop` **T0**라 `shop_tier_ceiling()` = 0이고, `mod_install`이 `TIER_RANK > ceiling`으로 막으므로 **Basic조차 시술 불가**다. 그리고 상점 T1은 `Q-HUB-012`(목표 클리어 = **런 완주**)가 선행이다 — 무료로만 만들었으면 등급에서 다시 막혔다.
- **판정(사용자):** *「각 기어당 1개는 원래 당연한 거라 무료고, 2·3으로 확장하는 건 확장이라 비용이 든다」.*
  - 내가 먼저 제안한 틀은 「출정 최소선 구제」였는데, 사용자 쪽이 **예외가 아니라 규칙**이라 더 낫다. 구제는 「왜 가끔 공짜지」를 남기지만, **건에 스킬 하나는 딸려 오는 것**이라는 규칙은 자기설명적이고 「2·3칸은 확장」이라는 슬롯 사다리(`F-008` §3.10)와 결이 같다.
- **① 기본 장착 = 시술비 0 + 등급 상한 불요.** `mod_install(base_id, basic)` — `basic`이면 `ward_scrap`도 `scribe_shop` tier도 보지 않는다. 판정은 `Backpack.is_basic_install`(그 건에 채워진 칸이 0인가)이 한다 — **슬롯 상태를 아는 것은 가방뿐**이다.
  - **등급까지 건너뛰어도 구멍이 아니다:** **해금**(트리 `Unlock`)이 이미 `scribe_shop` tier를 요구한다(`DEC-20260821-001`). 시술 쪽 등급 게이트는 애초에 **이중**이었고, 기본 칸에서 그걸 빼도 실질 게이트는 해금이 유지한다.
  - **D2 소멸은 살아 있다** — gear를 갈면 3칸이 전부 사라지고, 되돌아오는 건 **첫 칸뿐**이다. 2·3칸은 여전히 값을 치른다.
- **② 화면이 규칙을 말한다.** 카탈로그 행의 시술비 자리에 `기본` / `⚙30`, 미리보기에 「건 하나에 스킬 하나는 원래 딸려 오는 것 — 값이 붙는 건 둘째·셋째로 확장할 때」, 슬롯 섹션 머리에 같은 한 줄. **값만 ⚙0으로 두면 「왜 어떤 건 공짜지」가 남는다.**
- **🐞 ③ 가격 함수가 「모르면 공짜」로 답하고 있었다.** 첫 구현의 `slot_install_price`가 `/root/HubProfile`을 조회해서, 트리 밖 인스턴스에서는 **조용히 0**을 돌려줬다(스모크가 이걸 잡았다). → 가격표(`SHOP_PRICE`)를 **스크립트 상수로 직접** 읽어 트리 무관하게. 값을 복제하면 두 벌이 되고, 노드 조회로 두면 폴백이 틀린 방향이다.
- **정리:** `mod_install_price`는 소비자가 사라져 제거(`slot_install_price`가 대체).
- **⚠️ 판정 필요 1건 (`PENDING-PROP`):** `modInstallFee`에 **「그 gear의 첫 칸은 면제(등급 상한 포함)」** 조항 신설 — `F-009` §3.9.3. 근거는 「게이트가 요구하는 최소선을 경제가 막으면 안 된다」이며, 지금 스펙엔 면제 개념이 없다.
- **영향 파일:** `hub_profile.gd` · `backpack.gd` · `hub_modding_panel.gd` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: **소프트락 재현 경로 그대로**(scrap 0 · 상점 T0 · 프리모딩 칸 비우기 → 재설치 가능) · 기본 장착이 scrap 0·상점 T0에서 통과 · **확장은 상점 T0에서 여전히 차단**(값이 붙는 이유가 살아 있는가) · 시드 1칸 → 다음 칸은 유료 · 0칸 → 다음은 무료.
- **상태:** ✅ **전파 완료(2026-08-21, spec `c7f0860` · `DEC-20260821-003`)** · 재핀 완료.
  - `F-009` §3.9.3 `modInstallFee` 표에 **「면제」 행 + 근거 블록** 신설 · `F-020` §3.2.0에 **교차 주석**(「§3.2.0이 요구하는 최소선을 경제가 가로막아서는 안 된다」).
  - 스펙에 남긴 원칙: **게이트가 요구하는 최소선을 경제가 가로막아서는 안 된다.** 이 문장이 있어야 나중에 게이트가 「역할당 2칸」으로 바뀔 때 면제 범위도 함께 따라간다.

### DRIFT-156 — **건이 자기 빌드를 소유한다**: `D2` 소멸 폐기 · 런 중 비전투 교체 · **프리모딩 루팅** 🔶 rule 구현(모델 전환) + 판정 3건(`PENDING-PROP`)
- **근거:** `D-019` §3 `equippedSlotAbilities[3]` · `F-008` §3.10 D2 「교체 시 소멸」 · `F-008` §4.2 전투 중 교체 금지. 사용자 보고·판정: *「게임중 기어를 바꿀때 서브스킬이 안바뀜」* → **「각 gear가 자기 슬롯을 소유」** · **「허용 — 비전투 중에만」** · *「기어가 서브스킬을 장착되서 나오게하되 기어가 루팅되는 확률을 아주낮춰줘」*.
- **🔴 ① 「소멸」은 선택이 아니라 버그로 체감됐다.** 구 모델(`D2`)은 gear를 갈면 그 건의 Q/E/R이 **전부 사라진다**였다. 허브에서만 갈아입던 동안엔 「비싼 선택」으로 읽혔지만, 런 중 교체를 허용하는 순간 같은 규칙이 **「바꿨더니 서브가 없어졌다」**로만 보인다. 던전 한복판에서 되돌릴 방법도, 재설치할 상점도 없기 때문이다.
  - → **인스턴스 귀속으로 전환.** 벗은 건은 자기 빌드를 들고 창고로 가고, 신은 건은 자기 빌드를 데려온다. `Backpack.set_member_gear`는 **잃은 목록 반환(`Array`)을 버리고 `void`**가 됐다 — 잃는 것이 없으므로 셀 것도 없다. 대신 `worn_instance()`가 「지금 착용분을 창고가 보관하는 모양 그대로」 뜬다: **교체 = 두 dict의 교환**이다.
  - `clear_gear_slots()`는 남겼지만 **교체 경로에서 떼어냈다.** 「이 건의 빌드를 지운다」가 진짜 의도인 자리(마이그레이션·디버그)를 위해서다.
- **🔴 ② 빌드가 새는 자리가 네 곳 더 있었다.** 인스턴스 귀속으로 바꾸자 「gear dict를 만드는 모든 곳」이 곧 빌드 유실 지점이 됐다. 전수로 실었다: `ItemFactory.gear_item`(가방 타일) · `equip_panel._worn_with_slots`(벗기기·드래그 시작·표시 교체) · `inventory_ui.make_gear_stash_item`(창고 타일) · `main._sync_stash_from_source`(창고 닫기 = **Stash 통째 재작성**). 마지막 것이 특히 위험했다 — 타일에 안 실린 필드는 **닫을 때마다 영구 삭제**된다.
- **③ 런 중 교체는 이미 비전투로 게이트돼 있었다**(`F-008` §4.2, `_combat.is_engaged()`). 새로 연 게 아니라, **교체가 파괴적이지 않게 되면서 비로소 쓸 만해진 것**이다. 추출 시 반영은 `Backpack.capture_from_party`가 런타임 슬롯을 캡처해 맡는다.
- **🟠 ④ 주운 건은 빌드째로 나온다 — 그리고 그게 새 규칙을 하나 만든다.** `loot_service._premod_slots`가 상자 gear의 Q/E/R을 채워서 내보낸다(일반 1칸 · 희귀 = 그 건의 `gearSkillSlotCountMax`). 뽑는 풀은 **모딩과 같은 게이트**(`gear_allows_slot_ability`, 계열 × 역할)를 쓰되 **트리 해금을 보지 않는다**. 따라서:
  - 건에 **이미 새겨진** AB는 해금 없이 쓸 수 있다 — 주웠으니까.
  - 그걸 **다른 건으로 옮기려면** 여전히 트리 해금이 필요하다(`F-008` §3.10 그대로).
  - 즉 루팅은 「그 AB를 쓸 권리」가 아니라 **「이 건에 한해 쓸 권리」**를 준다. 모딩 패널은 미해금 AB를 뺄 때 **되돌릴 수 없음을 먼저 묻는다** — 조용히 비워 주면 「왜 되끼워지지가 않지」만 남는다.
- **⑤ 드롭률 극소화 — 0.15/0.50 → 0.04/0.15.** 건 하나가 파티 구성을 통째로 흔들게 됐으므로 흔하면 그 무게가 사라진다. 구 값은 **창고가 전 카탈로그를 들고 시작하던 시절**([[DRIFT-153]] ⓕ 이전)의 수치라 이미 의미가 없었다.
- **⑥ 창고 교체 목록을 아키타입으로 접지 않는다.** 구현은 `seen{}`으로 같은 `base_gear_id`를 한 줄로 접고 있었는데, 인스턴스마다 굴림·빌드가 다른 지금은 **고른 줄과 다른 개체가 나간다**. → 줄마다 그 개체의 Q/E/R을 적고, 집을 때도 id가 아니라 **자리**로 집는다(`Stash.take_gear_at`).
- **⑦ 잠긴 칸의 알림을 `push_warning` → `print`로 내렸다.** 프리모딩된 3칸 건을 대장간 T2 전에 주우면 3번 칸은 정상적으로 대기 상태다 — 주울 때마다 경고가 뜨면 경고가 무의미해진다. **내용은 지워지지 않는다**(gear가 소유): 대장간이 칸을 열면 그대로 깨어난다.
- **⚠️ 판정 필요 (`PENDING-PROP`):** ⓐ **`D2` 소멸 조항 폐기 → 인스턴스 귀속**(`F-008` §3.10 · `D-019` §3 — 사용자 판정 「spec 정본 · 교체 시 소멸」의 **철회**다) · ⓑ **프리모딩 루팅** = `slotAbility` 획득 경로 신설 + 「이 건에 한해 사용권」 규칙(`F-008` §3.7 DungeonDrop · `F-009` §3.9.3) · ⓒ **런 중 비전투 gear 교체**를 `F-008` §4.2에 명시(지금은 「전투 중 금지」만 있고 비전투 허용이 문장으로 없다).
- **영향 파일:** `backpack.gd` · `stash.gd` · `party_member.gd` · `hub_modding_panel.gd` · `equip_panel.gd` · `inventory_ui.gd` · `item_factory.gd` · `main.gd` · `loot_service.gd` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: `worn_instance`가 빌드를 싣는가 · 다른 건으로 갈면 **새 건의(빈) 빌드** · **되신으면 그 빌드가 그대로 복귀** · `take_gear_at`이 같은 아키타입 2개 중 **그 자리의** 개체를 집는가 · `add_gear`가 빌드를 보관하는가 · 프리모딩 풀 = 모딩과 동일 게이트 · **드롭률이 실제로 낮은가**(「낮췄다」는 주석만 남는 사고 방지) · **전 gear 프리모딩 풀 ≥1**(빈 풀 = 조용한 꽝).
- **상태:** ✅ **전파 완료(2026-08-22, spec `a9e67bd` · `DEC-20260822-001`)** · 재핀 완료.
  - ⓐ `F-008` §3.10 「gear 교체 = 전부 소멸」 **철회** + `D-019` §3.3 **직렬화 불변식** 신설(인스턴스를 옮기는 **모든** 경로가 `equippedSlotAbilities`를 함께 옮겨야 한다 — 게임에서 실제로 4곳이 새고 있었다). ⓑ `F-008` **§3.10.4 신설** — 루팅 프리모딩 · **`GearBound` 사용권**(이 인스턴스 한정 · 옮기려면 해금 · 빼면 비가역) + `D-019` `slotAbilityGrant`. ⓒ 「교체 타이밍 = 배치 단계·허브만」 **철회** → `partyInCombat=false`면 런 중에도(`F-008` §4.2 정합).
  - **`modInstallFee`의 자리 재정식화** — 값은 「교체」가 아니라 **「재조립」**에 붙는다. `I-007` §14.2가 값을 매긴 대상이 *스킬 세트 **재조립** 비용*이었으므로 이쪽이 원문에 더 가깝다. **`DEC-20260821-003`의 「D2 소멸 유지」 단서 철회** — 면제는 유효하되 조건이 「교체 후 첫 칸」 → **「그 gear의 첫 칸」**으로 넓어졌다.
  - `UI-005` §3.2: 교체 모달은 **경고가 아니라 비교**(무엇으로 바뀌는가) · `GearBound` 제거 경고 · **교체 목록 = 인스턴스 목록**(archetype으로 접으면 고른 줄과 다른 개체가 나간다).
  - **후속 TODO(spec):** 프리모딩 칸 수 · gear 드롭률을 **`modInstallFee`와 같은 회차**에서 `D-###`로(건이 빌드째로 오면 하나가 파티를 흔들므로 빈도와 짝이다) · `GearBound`의 UI 표현(해금된 칸과 구분이 안 되면 왜 되끼워지지 않는지 모른다).

### DRIFT-157 — 창고 ↔ 가방: **마석·참이 들어가지 못하고 있었다** + 스택 합치기 일반화 🔶 버그 수정 + tuning
- **근거:** `F-009` §3.8 마석 · `F-010` §3.11 참 · `F-029` §3.2 금고(haul 전용). 사용자 보고: *「스태시에 마석이나 참이 배낭과 잘 연동이 안된듯함. 옮겨지지 않고, 중복된건지 아닌지도 모르겠으」*.
- **🔴 ① 한 방향으로만 열려 있었다.** `_build_stash_items`(창고 → 타일)와 `_sync_stash_from_source`(타일 → 창고)는 **마석·참을 이미 다루고 있었는데**, 드롭 가드가 `["gear", "consumable"]`만 허용했다. 결과: 창고에서 꺼낼 수는 있는데 **되돌려 놓을 수가 없다**. → `STASHABLE_KINDS = [gear, consumable, manastone, charm]`. `haul`만 계속 막는다(승급 재화는 금고 하나로 모여야 세어진다).
- **🔴 ② 마석이 합쳐지지 않았다.** 드롭 시 스택 병합 분기가 `kind == "consumable"` 하드코딩이라, 마석 40개를 가방의 마석 위에 놓으면 **타일 두 개**가 됐다 — 「중복된 건지 아닌지 모르겠다」의 정체. → `_stack_key(item)`으로 종류별 id를 정규화하고(소비/마석/재료), 참·기어는 `""`(비스택 — 같은 참 2개는 칸도 2개를 먹어야 「칸 vs 파워」 긴장이 성립한다).
- **🐞 ③ 창고를 닫을 때 가방 쪽 커밋이 빠져 있었다.** `_open_stash()` 닫기가 `_sync_stash_from_source()`만 부르고 `commit_loose_to_backpack()`을 안 불러, **창고에서 가방으로 옮긴 것이 증발**했다. → `_close_stash_commit()`으로 두 커밋을 한 자리에 묶었다(`_commit_run_loadout`과 같은 쌍).
- **분류:** ①②③ 모두 **구현 버그**(스펙은 마석·참을 창고 보관 대상으로 이미 규정) — 전파 불요. `STASHABLE_KINDS`·`_stack_key`는 게임측 구조.
- **영향 파일:** `inventory_ui.gd` · `main.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: 입금 허용 종류 소스 단언 · `_stack_key` 존재 단언(**「합쳐지지 않는다」는 화면에서만 보이는 실패**라 소스로 못 박는다).

### DRIFT-158 — 재료 11종이 **구분되지 않고 쓸 데도 안 보였다**: 표시색 + 소비처 파생 툴팁 🔶 표시 레이어 + 판정 1건(`PENDING-PROP`)
- **근거:** `D-029` §3 haulMaterial 카탈로그 · `F-029` §3.2 금고. 사용자 보고: *「파밍할때 너무 퀘템이 많이 나오는데 구분이 안되고 딱히 의미도 없으니 크게 파밍하는 맛이 안남」*.
- **① 전수 확인부터 했다 — 「의미 없는 재료」는 실제로는 없었다.** 11종 중 9종을 시설 승급이 먹고, 나머지 2종(`haul_shared_shard`/`haul_shared_core`)은 **트리 해금 61종**이 먹는다. 즉 문제는 쓸모가 아니라 **쓸모가 보이지 않는 것**이었다.
- **② 색.** 재료가 **전부 같은 황토색 1×1 타일**이라 가방이 잡템 더미로 보인다. 이름은 타일이 작아 잘리므로 한눈에 갈리는 축은 색뿐이다. → `haul_materials.json`에 `color` 추가(재료의 정체를 따른다: 돌빛 회청 · 양피지 · 청람 잉크 · 황동 · 청록 유리 · 숯 · 자수정 · 놋쇠 · 올리브), **스킬 계열 공유재료 2종만 무채색**으로 묶어 승급재료와 갈라 놓는다. 바닥 드롭(`loot_service`)과 가방 타일(`ItemFactory`)이 **같은 색**을 쓴다.
- **③ 툴팁이 「무엇에 쓰는가」를 말한다.** 구 툴팁은 11종 전부 「재료 (haul) · 금고로 입금」 한 줄이었다. → `Slice01Data.haul_consumers(id)`가 **`facilities_tiers.json` + `skill_tree.json`에서 파생**해 「대장간 T2 ×2 · 필기 상점 해금 60종 ×1」로 보여주고, 금고 보유량과 「탈출해야 금고로 넘어간다」를 덧붙인다. **비용표를 툴팁에 복제하지 않는다** — 복제하면 둘이 어긋난다.
- **판정 완료(`DEC-20260822-001` ⑦):** 표시 속성(색·아이콘)은 **게임측 소유**로 확정 — 스펙이 값을 쥐면 아트 방향이 바뀔 때마다 SSOT 편집이 필요한데 그 편집엔 규칙적 의미가 없다. 대신 `D-029` §3에 **왜 필요했는지**와 **소비처 실재 불변식**을 남겼다(소비처 없는 재료는 주울 이유가 없고, 그런 것이 드롭 풀에 섞이면 파밍 전체가 잡템으로 물든다).
- **영향 파일:** `haul_materials.json` · `slice01_data.gd` · `item_factory.gd` · `inventory_grid.gd` · `loot_service.gd` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: **재료 전종 표시색 보유** · **재료 전종 소비처 보유**(소비처 없는 재료 = 줍는 이유가 없는 재료 — 새 재료를 추가할 때 이 게이트가 먼저 운다).

### DRIFT-159 — 의뢰: **판정표 데이터화** + 런 트래커 2층화(J) · 죽은 목표 제거 🔶 rule 구현 + 버그 수정
- **근거:** `F-029` §3.3 의뢰 · `F-006` 런 상태. 사용자 보고: *「게임내 미니맵하단의 퀘스트 목록이 현재 퀘스트목록과 대응이 안됨. (단, 너무 길면 안좋으니, 아예 퀘스트확인키를 따로 두는게 더 나을듯」*.
- **🔴 ① 상시 패널에 존재하지 않는 아이템의 목표가 박혀 있었다.** 「Cell 회수 (0/6)」 — `count_item("Cell")`인데 **`Cell`은 이 게임 어디에도 없다**(데모 시절 문자열). 영원히 0인 줄은 목록 전체의 신뢰를 깎는다. 제거.
- **🔴 ② 마을 의뢰가 런에 전혀 나오지 않았다.** 트래커는 런 목표(열쇠/문/탈출)만 알고, 마을에서 맡은 의뢰는 몰랐다 — 「대응이 안 됨」의 나머지 절반.
- **② 두 층으로 갈랐다**(사용자 판정 「퀘스트확인키를 따로」): **상시** = 이 런에서 할 일 3줄(화면을 계속 차지하므로) + 「[J] 맡은 의뢰 N」 한 줄. **J 토글** = 맡았고 아직 안 끝난 의뢰 + 진행도. 개수가 늘어도 상시 패널을 밀어내지 않는다. 목록은 상시 패널 **아래로** 자란다.
- **🔴 ③ 진행도를 물을 방법 자체가 없었다.** `evaluate_quests()`가 `_q_if(...)` 줄의 나열이라 「지금 얼마나 찼는가」를 꺼낼 수 없었다. → **`QUEST_RULES` 표로 데이터화**(vault/extract/wipe/facility/enc/all) 하고 `evaluate_quests()`와 새 `quest_progress()`가 **같은 표**를 읽는다. 두 벌이면 「목록엔 다 찼는데 완료가 안 된다」가 반드시 생긴다.
  - 등재되지 않은 의뢰는 `{"unknown": true}`를 돌려준다 — **「0/0」이라고 우기는 것보다 모른다고 말하는 편이 낫다**(런 훅 전용 `Q-HUB-012`가 그 경우다).
- **④ 목록이 「왜 안 오르는지」를 말한다.** 재료 조건은 **탈출에 성공해야** 금고에 반영되므로, 런 중에 숫자가 안 오르는 게 정상이다. 그 한 줄을 목록 밑에 고정으로 붙였다.
- **⑤ 샌드박스 미반영은 의도.** `combat_sandbox`에는 런 목표도 허브 의뢰도 없다 — 띄울 것이 없다([[sandbox-input-parity]]의 예외).
- **분류:** `QUEST_RULES` 데이터화·트래커 2층화 = 게임측 구조(전파 불요). 죽은 목표 제거 = 버그. 새 입력 액션 `toggle_quests`(J) = 게임측.
- **영향 파일:** `hub_profile.gd` · `quest_tracker.gd` · `project.godot` · `tools/hub_smoke.gd`.
- **게이트:** `ci_smoke.sh` **12/12 PASS** — 신규: `QUEST_RULES` 전건 진행도 산출 + 카탈로그 등재 · **시설 요구 의뢰 전건이 판정표 또는 런 훅으로 완료 가능**(구 `Q-HUB-010/011`처럼 「참조만 남아 영영 안 끝나는 의뢰」 재발 방지) · 트래커에 죽은 목표(Cell) 없음 · J 토글 존재 · 입력 액션 등록.
- **상태:** ✅ 구현 완료 · 전파 불요(게임측 구조). 다만 **불변식은 스펙으로 올렸다** — `F-029` §3.3 **의뢰 완료 수단 실재**(자동 판정 또는 런 훅) + 「판정과 진행도는 같은 규칙에서 낸다」(`DEC-20260822-001` ⑥).

### DRIFT-160 — 참(charm)이 **런 내내 꺼져 있었다**: 오오라 적재 시점 누락 + 표시·왕복 유실 🔶 버그 수정(효과 무효) + 게이트 신설
- **근거:** `F-010` §3.11 참 그리드 · §3.11.1 조건부 참 · `I-007` §7. 사용자 질문: *「참들이 효과가 없는건지 툴팁에 안뜨는건지 확인해줘」* — **둘 다였다.**
- **🔴 ① 참 오오라가 런 시작 시 아무에게도 적재되지 않았다.** `CombatController.refresh_charms()`는 `charms_changed` **시그널에만** 물려 있었고, 그 시그널을 쏘는 곳은 `InventoryUI._close()` **하나뿐**이다. 그래서 실제 거동은 **「인벤을 한 번 열었다 닫아야 참이 켜진다」**였다 — 그때까지 참은 칸만 먹고 아무 일도 안 한다.
  ```
  런 씬 실측 (스타터 참 2종 보유):
    Tank/DPS/Nuker/Healer   dmg_taken=1.000  move=1.000   ← 전원 중립
    inv.charm_mods("Tank")  = {damage_taken_mult: 0.95, move_mult: 1.03}   ← 계산은 정답
    charms_changed 강제 emit 후 → dmg_taken=0.950  move=1.030              ← 즉시 정상
  ```
  계산(`charm_mods`)·적재(`apply_charms`)·연결이 **전부 멀쩡했고 호출 시점 하나만 없었다.** 그래서 소스 단언으로는 잡히지 않는 종류다 — 코드는 다 있는데 아무도 부르지 않는다.
  - → `dungeon_run`이 `Backpack.apply_to_party()` **직후** `refresh_charms()`를 1회 직접 호출. 순서가 중요하다(멤버가 gear 바인딩으로 갈릴 수 있다).
- **🔴 ② 바닥에서 주운 참도 인벤을 닫을 때까지 안 켜진다.** 픽업은 인벤이 **닫힌 채로** 일어난다. → `add_charm_to_backpack` 성공 시 `charms_changed` emit(캐리 최초 적재 `_loading_carry` 중에는 호스트가 직접 부르므로 제외). 버리기(`_do_discard`)도 참이면 emit — 버린 참의 오오라는 즉시 꺼져야 한다.
- **🔴 ③ 참·마석을 버렸다 주우면 정체가 사라졌다.** 두 곳이 짝으로 뚫려 있었다: `InventoryUI._drop_def` 화이트리스트에 **`charm_id`/`manastone_id`가 없었고**, `ItemDrop.interact()`엔 `gear`/`haul` 분기만 있어 나머지가 `add_to_backpack(id, w, h, color)`로 떨어졌다 — **`kind`도 id도 없는 익명 블록**이 된다. 버리기는 실경로다(`_do_discard` → `item_dropped` → `ItemDrop`). → 화이트리스트에 `charm_id`·`manastone_id`·`max_stack`·`at_risk` 추가 + `interact()`에 `charm`/`manastone`/`consumable` 분기.
- **② 툴팁이 참을 설명하지 않았다.** `inventory_grid._item_tip`의 match에 `charm` 분기가 없어 **이름과 크기만** 나왔다. 효과가 꺼져 있던 기간과 「원래 이런 물건」이 화면에서 구분되지 않았다는 뜻이다. → `_charm_tip`: `desc` · `applies_to` 역할 한정 · 조건부 표시 · **곱연산 중첩 규칙**(칸 vs 파워가 이 물건의 전부이므로 대가도 같이 적는다).
- **③ 성문 「반입 확인」이 참 이름을 전부 `?`로 찍고 있었다.** `get("display", "?")`를 읽는데 `charms.json`의 필드는 **`display_name`**이다. 출정 직전에 무엇을 들고 가는지 보는 자리다.
- **분류:** 전부 **구현 버그** — 스펙(`F-010` §3.11)은 「들고 있는 동안 적용」을 이미 규정하고 있다. 전파 불요.
- **영향 파일:** `dungeon_run.gd` · `inventory_ui.gd` · `inventory_grid.gd` · `hub_gate_panel.gd` · `item_drop.gd` · `slice01_data.gd`(`get_charm_ids`) · **신규 `tools/charm_smoke.gd`** · `tools/ci_smoke.sh`.
- **게이트:** `ci_smoke.sh` **13/13 PASS**(스위트 신설 — 12 → 13). `charm_smoke`는 **런 씬을 실제로 띄워 멤버 값을 읽는다**: 배선이 아니라 **결과**를 묻는 게 요점이다.
  - **런 시작 직후 참이 전원에 걸려 있는가**(인벤 개폐 불요) — `refresh_charms()` 호출을 지우면 이 줄이 먼저 운다(반증 확인 완료).
  - `applies_to` 역할 한정이 **Tank엔 걸리고 비Tank엔 안 걸리는가**(전역 효과와 한정 효과를 한 번에).
  - **참 카탈로그 전종 실효** — 상시 참은 집계 키(`charm_mods`)가, 조건부 참은 멤버 구현이 있어야 한다. `effect` 이름이 어긋나면 `charm_mods`가 **조용히 건너뛰므로**(`if not out.has(eff): continue`) 참 하나가 통째로 무효가 되는데 아무 소리도 안 난다.
  - 조건부 참의 `condition` 전건이 `party_member.charm_condition_met`에 구현돼 있는가(모르는 조건 = 영원히 `false`).
  - 버리기 def의 `charm_id`/`manastone_id` 보존 + `ItemDrop` 분기 존재.
  - 툴팁 `_charm_tip` 존재 · 성문이 `display_name`을 읽는가.
- **남는 것:** 참은 **허브에서도 효과 미리보기가 없다**(전투가 없으니 무의미하긴 하다). 성문 반입 목록이 이름만 보여주므로, 「이 조합이 무엇을 주는가」 합산 한 줄은 후속.
- **상태:** ✅ 구현 완료 · 전파 불요(구현 버그). 다만 **불변식은 스펙으로 올렸다** — `F-010` §3.11 **참 오오라 적재 시점**(런 시작 포함) + **카탈로그 실효** 불변식(`DEC-20260822-001` ⑤). 「코드는 다 있는데 아무도 부르지 않는」 실패는 다음에도 같은 방식으로 온다.

### DRIFT-161 — 스택 분리: Ctrl+드래그 → 개수조절바 → **나눈 만큼 커서에 들린다** 🔶 UX 개선 + 유실 경로 차단
- **근거:** `F-010` §3.8.1 그리드 점유 모델 · `F-007` `Run Inventory Slot`. 사용자 요청: *「인벤토리에서 스택으로 적재된 아이템을 분리해야 할 경우가 있어. 컨트롤 드래그하면 개수조절바가 열리고, 거기서 확정한 개수만큼 드래그 위치로 아이템을 나눠 가져올수 있는 기능」*.
- **① 있던 기능이 반쪽이었다.** Ctrl+클릭 분리는 이미 있었지만 ⓐ **`consumable` 하드코딩**이라 마석·재료는 통째로만 옮길 수 있었고 ⓑ 확정 수량을 **같은 그리드의 빈 칸에** 놓았다. ⓑ 때문에 「창고에서 5개만 꺼내 가방에 넣기」가 **두 동작**(쪼개기 → 다시 집어 끌기)이 되고, 빈 칸이 없으면 **아무 말 없이 원상복구**됐다.
  - → 진입 조건을 `_stack_key(item) != ""`로(스택형 전체, [[DRIFT-157]]에서 만든 정규화를 재사용). 확정 즉시 **드래그 상태로 넘어간다** — 사용자가 Ctrl을 누른 이유는 「쪼개고 싶다」가 아니라 **「이만큼만 저기로 옮기고 싶다」**이기 때문이다.
- **② 개수조절바.** `SpinBox` → `HSlider` + **「N 집기 · M 남김」 동시 표시**(집는 수만 보이면 원본이 얼마나 남는지 세느라 눈이 왔다 갔다 한다) + 빠른 선택 3종(1개 / 절반 / 전부−1) + **Enter 확정**(버튼까지 마우스를 옮기게 하면 「이만큼만」이 다시 두 동작이 된다).
- **🔴 ③ 여기서 유일하게 위험한 건 산수가 아니라 되돌리기다.** 나눠 집은 조각은 **어느 그리드에도 속해 있지 않다** — `_revert_drag`의 기본 분기(원위치 복귀)가 통하지 않으므로, 놓을 곳이 없으면 덜어 낸 만큼이 **조용히 증발**한다. → `_drag_src.kind = "split"` + 출처를 실어 두고 `_revert_split()`이 **원본에 합친다**(원본이 사라졌으면 빈 칸 → 그것도 안 되면 가방 → 전부 실패하면 **소리를 낸다**).
- **④ 창밖 드래그 = 버리기 경로에서 분리 조각을 뺐다.** `_revert_split`이 이미 원본에 합쳐 넣었으므로 그 dict는 어느 그리드에도 없다 — 버리기에 넘기면 **유령을 버리게 된다**. 버리려면 원본을 Shift+우클릭하면 된다.
- **⑤ 1개짜리는 나눌 수 없다**를 명시 가드로. 없으면 `clampi(n, 1, 0)`(하한 > 상한)의 결과에 기대게 되는데, 개수 산수를 「읽는 사람에 따라 갈리는 것」에 맡기지 않는다.
- **분류:** UX 개선 + 구현 버그 차단 — 스펙은 스택·그리드 점유를 이미 규정하고 분리 UX는 규정하지 않는다. 전파 불요.
- **영향 파일:** `inventory_ui.gd` · **신규 `tools/inventory_smoke.gd`** · `tools/ci_smoke.sh`.
- **게이트:** `ci_smoke.sh` **14/14 PASS**(스위트 신설 — 13 → 14). 묻는 것은 **어느 경로로 끝나도 총합이 보존되는가** 하나다: 20 → 13+7 · 되돌리면 20 · **원본이 사라져도 조각은 살아남는다** · 전부 집기는 max−1로 clamp · 1개짜리 거부 · `_stack_key`가 참·기어를 **비스택**으로 유지(스택형으로 새면 같은 참 2개가 한 칸이 되어 「칸 vs 파워」가 무너진다).

### DRIFT-162 — 맵 계약 계측기: `map_smoke`(불변식) + `map_shot`(평면 스냅샷) 🔷 도구 신설 · 전파 불요
- **근거:** 맵 고도화 착수(설계 `docs/design/map_upgrade_plan.html`, 현황 `map_demo_001_status.html`). 스펙 쪽 규칙 변경 없음 — `F-026`/`LDG-001`의 레벨 콘텐츠 계층은 그대로고, **게임이 소유하는 맵 계약**에만 계측을 붙였다.
- **왜 도구가 먼저인가.** 맵은 판정 기준이 「보기에 맞다」라서 소스 단언에 안 잡히는 결함이 모인다 — `connects`에는 있는데 벽이 안 붙은 방, 개구부는 있는데 navmesh가 끊긴 방, `pool_slot`은 박혔는데 `spawn_table` 행이 없어 **아무 일도 안 일어나는 방**(남쪽 사슬이 그렇게 자랐다). 리팩터 전에 계측기를 세워야 전후 비교가 성립한다.
- **`tools/map_smoke.gd`** — [계약] 하드 12항목 + [설계] 리포트 7종. 검사는 **맵 계약 getter를 통해서만** 한다(구현이 아니라 계약을 시험한다). 그래야 절차 그레이박스든 Blender authored 씬이든 **같은 스위트가 그대로 돈다** — 그게 계약이 진짜 계약이라는 증거다.
  - 🔴 **핵심 한 줄: 오클루더 = LOS 높이대(y=1.0) 레이어 1 콜라이더 (선언 97 / 유도 97).** `F-011`의 「안개와 적 시야는 같은 출처」 전제를 **수치로** 잡아 둔 것이다. 지금은 절차 생성 도중 손으로 기록하므로 우연히 맞을 뿐이고, authored 맵에선 아무도 안 채운다. 이 97/97이 **콜라이더 유도 전환의 등가성 증인**이다.
  - 바닥(두께 0.3 m, y≤0)이 규칙에서 자동으로 빠진다 — 「시야를 막는가」로 판정하므로 authored 맵의 임의 지오메트리에도 같은 선이 선다.
  - [설계] 줄은 실패시키지 않는다. 현 맵이 **기준선**이다: 사이클 0 · 안개 바운딩 32,909 m²(바닥 10,116, 사용률 31%) · 상자 EV 18.4 · pool 방 14/가중치 14.4 · 동일 규격 27×22.5 ×8 · 장애물 2/16방 · `dungeon_run.gd` 좌표 리터럴 10건.
- **`tools/map_shot.gd`** — 직교 카메라 + 공유 `World3D` SubViewport로 평면 PNG(안개 off · HUD 숨김 · 방 라벨 유지). 1110×1500 px · 1 px ≈ 0.15 m. **에이전트가 3D 뷰포트를 못 보는 것**이 자연어 레벨 편집의 병목인데, 불변식(기하)과 스냅샷(배치)이 그 자리를 대신한다.
  - 방향: 북(+Z) 위 · **동(+X) 왼쪽**(인게임 미니맵과 동일 = 플레이 화면의 180° 회전). 위에서 내려다보는 카메라로는 「북 위 + 동 오른쪽」이 성립하지 않는다(반사 필요). 두 설계 문서의 ASCII 도면은 동이 오른쪽이라 **X가 서로 뒤집혀 있다** — 통일 여부는 결정 사항으로 남김.
- **부수 발견(미해결):** prespawn이 **navmesh 동기화 전에** 스폰 좌표를 질의한다 — `NavigationServer navigation map query failed because it was made before first map synchronization` (`combat_controller.gd:1139` `map_get_closest_point`). 첫 프레임 스폰 스냅이 무효화된다. `ci_smoke`의 `ERRPAT`에 이 부류가 없어 **전부 초록인 채로 계속 난다.** → `map_smoke`가 흡수할 대상(후속).
...
### DRIFT-167 — 다층(백레이어) 기반: LOS 높이·치명존 carve **상대화** + 불변식 2종 🔷 Phase 1 선행 · 전파 불요
- **근거:** 맵 고도화 방향 결정 — **스펠렁키식 백레이어**(같은 XZ 발자국에 레이어를 겹치고 전환)를 채택하고 계단은 **같은 맵 안의 워프**, 포털은 런 경계로 둔다. 그 기반을 열기 전에 **절대 Y 전제** 두 곳을 걷어낸다.
- **🔴 ① `LOS_EYE_Y = 1.0`이 월드 절대 좌표였다.** 오클루더 유도가 「y = 1.0 m를 가리는 콜라이더」로 판정하는데, 바닥이 −6 m인 방(계단으로 내려간 구획)의 벽은 y ∈ [−6, −2.5]라 **그 높이를 안 가린다** → 그 방의 벽이 오클루더에서 **통째로 빠지고 안개 없는 방이 조용히 생긴다.**
  - **게이트도 못 잡는다.** 스모크의 오클루더 대조는 맵과 스모크가 **같은 절대 규칙**을 쓰므로 양쪽이 **사이좋게 0을 반환해 「미일치 0」으로 통과**한다. Phase 0에서 잡은 것들과 정확히 같은 계열(둘 다 틀렸는데 일치하니 초록).
  - → `LOS_EYE_H`(**방 바닥 기준 상대**) + `floor_y_at(xz)`. 스모크도 `_rects`에 바닥 높이를 싣고 **독립적으로** 같은 규칙을 재계산한다.
- **② `_carve_zone`도 절대 높이였다** — `add_projected_obstruction(verts, -1.0, 4.0)`. 내려간 구획의 치명존이 엉뚱한 높이를 깎는다 → 존의 바닥 기준(`center.y - 1.0`)으로.
- **③ 불변식 2종 신설:**
  - **같은 레이어 방 XZ 중첩 없음** — 안개는 XZ 텍스처 하나다. 겹치면 위층이 아래층을 지운다. (레이어 축이 데이터에 들어오면 이 검사는 **레이어별**로 갈린다 — 다른 레이어끼리는 겹쳐도 되고, 그게 백레이어의 요점이다.)
  - **방마다 오클루더 ≥ 1** — 「안개 없는 방」을 방 단위로 잡는다. 총량 대조가 0 = 0으로 통과하던 구멍을 막는다.
- **🔴 반증 확인 완료.** 회귀 테스트를 합성 authored 씬에 넣었다(바닥 y=−6 방 + 벽). `eye`를 절대값으로 되돌리면 **「바닥 y=−6 방의 벽이 오클루더로 잡힌다」와 「지상 1 + 지하 1 = 2」 두 줄이 운다**(유도 1개). 상대화하면 통과. 고침이 실재한다는 증거다.
- **설계 결정 기록(구현 전):** 파티는 **찢지 않는다** — 레이어 이동은 **전 파티 결집 조건**부이며 4명 전원이 함께 간다(`F-007` §3.6.2 `extractionCohesionRule` + `F-003` `unbound_anchor_max_m`와 같은 모양). **down/MIA는 따라오지 않고 그 자리에 기존 상태 그대로 남는다**(강제 전환 없음) — `MIA`(살아서 이탈)는 적에게 **노출되어 돌아왔을 때 `down`이 되어 있을 수 있고**, `down`(이미 쓰러짐)은 더 나빠지지 않는 회수 대상이다. `down`은 선택지가 아니라 **MIA가 죽은 결과**이므로 「비용이 크면 down으로 전환」 초안은 **폐기**했다(성능을 위해 캐릭터를 죽이는 셈) — 찢김은 *조작 가능한* 멤버가 나뉘는 것이고 쓰러진 몸은 **회수 부채**(`F-007`)다. 그 결과 **행동 가능한 파티는 언제나 한 레이어**에 있어 안개·내비·콜리전의 레이어화 비용이 크게 준다. **적은 레이어 고정, `F-028` 3세력만 계단을 쓴다.** 가드: `F-001` §3.6의 **MIA 스왑 차단**은 **이미 구현돼 있다**(`PartyController.try_swap_to()`) — 레이어가 그것을 **전제로 삼는다**(정정: [[DRIFT-170]]). 실제 비용 동인은 CPU가 아니라 **파티 멤버 상태 기계**(「존재하지만 조작 불가·타 레이어」).
...
### DRIFT-169 — 설계 리포트 → **하드 게이트 승격**(맵이 목표치를 선언) + 잠금 데드락 방지 🔷 Phase 1 · 전파 불요
- **근거:** 맵 고도화 플랜 §게이트. `map_smoke`의 [설계] 줄이 **관찰만 하고 아무것도 막지 않았다** — 수치가 나빠져도 초록이었다.
- **🔴 목표치를 코드 상수로 두면 게이트를 통째로 느슨하게 만들게 된다.** 현 데모 맵은 사이클 **0**(목표 2)이고 바운딩이 **213 m**(권장 200)라, 목표를 코드에 박고 승격하면 그 자리에서 빨개진다 → 결국 목표를 낮추거나 검사를 끄게 된다. → **맵이 자기 목표치를 선언한다**(`rooms.json` `design_targets`).
  - 데모 맵 선언 = **현 상태 그대로**(`min_cycles 0` · `chest_ev_band [16,20]` · `bbox_max_m [160,220]`). 목적이 「좋아지기」가 아니라 **「나빠지지 않기」**다.
  - 신규 그레이박스는 같은 자리에 `min_cycles 2` · `bbox 200×200`을 들고 오면 되고, **코드 변경 없이 게이트가 세진다.**
- **① 하드 게이트 3종** — 사이클 ≥ 목표 · 상자 EV ∈ 밴드(구조를 바꾸다 재화가 조용히 반토막 나지 않게) · 안개 바운딩 ≤ 상한(빈 공간도 텍스처를 전액 낸다). 설계 리포트와 **같은 헬퍼**를 쓴다(두 벌 계산이 어긋나지 않게).
- **🔴 ② 잠금 그래프 해결 가능성.** 메트로배니아 최다 사고가 **「열쇠가 잠긴 방 안에」**다. `entry_requirement`가 요구하는 것을 그 방에 **들어가지 않고** 얻을 수 있는지 검사한다(앵커 `yields` 선언 기반). **반증 확인:** `RM-OBJ-01`에 `KEY-DEMO-01` 잠금을 걸면 *「열쇠 `KEY-DEMO-01`가 **잠긴 방 안**에 있다」*로 운다. `min_cycles`를 1로 올리면 사이클 줄도 같이 운다.
- **③ 데이터끼리만 맞으면 못 잡는 것을 잡았다.** 선언은 `KEY-DEMO-01`인데 **코드가 넣는 아이템 id는 `"Key"`**였다 — ID 계약(코드/데이터 문자열 ID = 스펙 1:1) 위반이자, 내가 방금 넣은 선언을 **아스피레이셔널**로 만드는 상태였다.
  - → 상자 아이템 id를 **`KEY-DEMO-01`로 정정**. 문 판정이 `id.to_lower().contains("key")` **부분 문자열**이라 동작은 그대로고 구 세이브(`"Key"`)도 계속 열린다(툴팁도 둘 다 유지).
  - → 게이트 추가: **앵커 `yields` id가 코드에 실재하는가**(소스 대조). 데이터끼리만 맞는 상태를 막는다.
  - ⚠ **남은 취약성(주석으로 명시):** 부분 문자열 매칭이라 id에 `key`가 들어간 다른 아이템도 문을 연다(미래의 `Monkey Charm` 등). 열쇠류가 늘면 정확 일치로 좁혀야 한다 — 지금 넓게 두는 이유는 구 세이브 호환뿐이다.
- **영향 파일:** `data/slice01/rooms.json`(`design_targets` · `key_chest.yields`) · `tools/map_smoke.gd` · `scripts/run/dungeon_run.gd` · `scripts/ui/inventory/inventory_grid.gd` · `scripts/ui/inventory/inventory_ui.gd`(주석).
- **게이트:** `ci_smoke.sh` **15/15 PASS** · 사이클 0 ≥ 0 · 상자 EV 18.4 ∈ [16,20] · 바운딩 155×213 ≤ 160×220 · 잠금 1개 해결 가능 · yields id 실재.
- **상태:** ✅ 완료 · 전파 불요. 다음: `F-001` §3.6 MIA 스왑 차단(스펙에 있고 미구현, 레이어의 전제) → 레이어 시스템 8항목.

### DRIFT-170 — 🔴 **오독 정정**: MIA 스왑 차단은 이미 구현돼 있었다 (+ 회귀 게이트 신설) 🔷 전파 완료(`DEC-20260824-002`)
- **근거:** 레이어 작업의 전제를 확인하다 발견. **내가 「`F-001` §3.6 MIA 스왑 차단이 스펙에 있으나 미구현」이라고 판단해 문서 여러 곳에 적었는데 틀렸다.**
- **① 실제:** `PartyController.try_swap_to()`가 **처음부터** `is_mia()`를 검사해 거절하고 `F-001 §3.6`을 인용한다. 다운(`is_alive()`) 차단도 함께 있다.
- **② 오독의 뿌리는 두 겹이었다.**
  - **낡은 주석:** `run_controller.gd`의 `can_swap()` 주석이 *「Control Lock / MIA … **neither implemented** in slice-01」*이라고 적고 있었다. 실제로 `can_swap()`이 보는 건 **Control Lock뿐**이고(그건 진짜 미구현 — 컷씬 구간이 없다), MIA·다운은 **다른 파일**에 있다. 주석이 자기 함수의 범위를 넘어 「둘 다 없다」고 말한 것이 문제.
  - 🔴 **스펙 자체의 자기모순:** `F-001` §3.6이 예외 목록에 `MIA`(2번)를 적어 놓고 **바로 아래 문장**에서 *「스왑 금지는 **Control Lock에서만** 발생한다(다운은 금지하지 않는다)」*라고 말하고 있었다. 읽는 줄에 따라 답이 갈렸다.
- **③ 조치:**
  - **스펙:** `DEC-20260824-002` — 다운을 **예외 3번**으로 명시(새 규칙이 아니라 **이미 그렇게 동작하던 것을 조문에 올림**) + 트레일링 문장을 「위 3종에서만」으로 정정 + 정정 사유 인라인. 핀 `2a81e52` → `8dc2169`.
  - **코드:** `run_controller.gd` 주석 정정 — 「여기가 보는 건 Control Lock뿐, MIA·다운은 `PartyController`에 있다」.
  - **게이트:** `map_smoke`에 **회귀 시험 신설** — 부팅된 런에서 멤버를 `set_mia(true)` → `try_swap_to()`가 **거절**하는지, `set_mia(false)` 후엔 **되는지**(과잉 차단도 함께 막는다). **시험이 하나도 없었다** — 그래서 「구현됐는가」를 주석으로 판단하게 됐다.
  - **문서 정정:** 패킷 §G · 플랜 Phase 1 표 · [[DRIFT-167]] · spec `TODO.md`.
- **왜 이 불변식이 중요한가:** 레이어 전이 후 남겨진 `down`/`MIA` 멤버로 **스왑이 되면** 조작 가능한 파티가 두 레이어에 나뉘어 「활성 레이어는 하나」 전제가 깨진다(`DEC-20260824-001` §⑤). 즉 이건 파티 규칙이자 **백레이어의 전제**다.
- **교훈:** 「구현됐는가」를 **주석으로 판단하지 않는다.** 주석은 자기 함수의 범위를 넘어 말하면 거짓이 되고, 그 거짓이 스펙 모순과 겹치면 오독이 문서 4곳으로 퍼진다. **시험이 있었으면 5초에 끝났을 질문**이었다.
- **영향 파일:** `scripts/run/run_controller.gd`(주석) · `tools/map_smoke.gd`(게이트) · `docs/_PROP_PACKET_MAP_PHASE1.md` · `docs/design/map_upgrade_plan.html` · `docs/SPEC_DRIFT.md` · spec 4파일.
- **게이트:** `ci_smoke.sh` **15/15 PASS** · 「MIA 멤버로 스왑 불가」+「해제 후엔 스왑됨」.
- **상태:** ✅ 완료 · 전파 완료(`DEC-20260824-002`, 핀 `8dc2169`). 구현 변경 없음 — 조문과 문서가 구현을 따라갔다.

### DRIFT-171 — 레이어 축 기반: 콜리전 비트 분리 + 오클루더 레이어화 (C-1·C-3) 🔷 Phase 1 · 전파 불요
- **근거:** spec `8dc2169` `LDG-001` §9.2 (`DEC-20260824-001`). 백레이어(같은 XZ에 층을 겹치고 전환)의 **기반**. C-1(계약에 layer 축)과 C-3(콜리전 비트)은 **레이어 키 하나로 물려 있어** 나누면 오히려 위험해서 함께 했다.
- **① 레이어 → world 콜리전 비트.** 층이 XZ를 공유해도 서로의 시야·경로를 안 건드리려면 지오메트리가 **레이어별 비트**에 있어야 한다. `world_bit(layer)`: **`layer 0` = 비트 1 = 기존 `"world"`** → **현 맵은 아무것도 안 바뀐다.** 비트 2(party)·4(enemy)·8(AB-033 엄폐 돔)이 이미 쓰이므로 `layer 1`은 **비트 16**부터. 역변환 `layer_of_bit()` + 합집합 `world_mask_all()`.
- **② 지오메트리가 비트를 쓴다.** `map_demo_layout`의 바닥·벽·장애물 빌더가 방의 `layer`에서 얻은 비트를 받는다(`_build_room` → 아래로 전달). 전 방이 `layer 0`이라 값은 전부 1 — **불변**.
- **③ 오클루더가 층을 안다.** `derive_occluders()`가 **콜라이더의 비트에서 층을 읽어** footprint에 실는다 — 「지오메트리의 비트가 곧 레이어」라 별도 선언이 없다(규약이 아니라 구조). `get_occluder_footprints()`는 **활성 레이어만** 돌려주고(안개는 XZ 텍스처 하나라 다른 층이 섞이면 거짓말을 한다), `get_all_occluder_footprints()`가 레이어 무관 전체다.
- **④ 게이트 레이어화.** 「XZ 중첩 없음」이 **같은 레이어 안에서만** 적용된다 — **다른 레이어끼리는 겹쳐도 된다. 그게 백레이어의 요점이다.** 「방마다 오클루더 ≥1」도 자기 층 것만 센다. 「같은 출처」 대조는 레이어 무관 전체와 한다.
- **🔴 ⑤ 증명 — 같은 XZ에 두 층을 겹쳐 세웠다.** 합성 authored 씬의 지하 방에 **layer 1 비트(16)** 벽을 **layer 0 벽과 정확히 같은 자리**에 추가하고 확인:
  - `전체 3 = layer0 2 + layer1 1` — 층이 갈린다
  - **같은 XZ의 벽이 층별로 각각 1개씩** 잡힌다
  - `set_active_layer(1)` → **그 층 것만** 보인다
  - 이게 성립해야 「옆으로 치우지 않고 겹친다」가 가능하고, **안개 바운딩이 안 커진다**(displacement 대비 이득의 근거).
  - ⚠ 시행착오 2건을 기록해 둔다: 프로브 좌표를 **월드로 줬다가**(방-로컬이어야 한다) LOS 높이대를 벗어나 0개가 나왔고, 「layer 1이 layer 0에 안 샌다」는 **주장 자체가 틀렸다**(같은 자리에 layer 0 벽이 원래 있으므로) — 「층별로 각각 1개씩」이 옳은 검사다.
...
### DRIFT-173 — 안개 레이어화: 층별 탐색 기억 + 전환 경로 (C-2) 🔷 Phase 1 · 전파 불요
- **근거:** spec `LDG-001` §9.2. [[DRIFT-171]](비트)·[[DRIFT-172]](nav) 위에 **렌더 경로**를 얹는다 — 여기서부터 기반 작업이 아니라 실제 화면을 건드린다.
- **🔴 ① 탐색 기억이 층별이어야 한다.** `_explored_viewport`(ever-seen 누적)가 하나뿐이면, 계단으로 내려간 순간 **아래층이 「이미 탐색된」 상태로 보인다** — 층이 XZ를 공유하므로 위층에서 본 자리가 그대로 아래층 기억이 된다. → `_explored_by_layer`(layer → SubViewport), **레이어를 처음 방문할 때 생성**(그전엔 미탐색).
- **② 전환 경로 `switch_layer(n)`** — 계단이 부를 자리. 넷이 **함께** 간다: 활성 레이어(맵) · 정적 오클루더 교체 · 탐색 기억 스왑(`exp_tex` 유니폼) · **비활성 층 지오메트리 숨김**. 하나라도 빠뜨리면 층이 겹쳐 보이거나 안개가 남의 층 벽을 쓴다.
  - **동적 오클루더(열쇠문)는 안 건드린다** — 호출자가 소유권을 갖고 수명을 관리한다(`add_box_occluder`가 노드를 돌려주고 문이 열릴 때 직접 해제). 정적만 `_static_occluders`로 추적해 교체한다.
- **③ 비활성 층 숨기기는 방 단위로.** `map_source.set_visible_layer()`가 **방 노드 이름 = `room_ref`** 규약(절차·authored 공통)을 이용해 토글한다. 층이 XZ를 공유하므로 다 보이면 겹쳐 그려진다.
- **왜 안개 텍스처는 여전히 1장인가:** 파티를 찢지 않기로 했으므로(`DEC-20260824-001` §⑤) **활성 레이어가 언제나 하나**다. 층마다 텍스처를 동시에 굴릴 필요가 없다 — 그 결정이 여기서 비용을 돌려준다.
- **게이트:** 실제 런에서 전환을 돌린다 — 전환 전(layer 0 · 오클루더 97 · 방 보임) → `switch_layer(1)`에서 **활성 층이 바뀌고 그 층 오클루더만 남고(0) 방이 숨는다** → 되돌리면 **원복(97 · 보임)**. 현 맵이 layer 0 하나뿐이라 layer 1이 「아무것도 없는 층」이 되는 것이 그대로 검사가 된다(전환이 무언가를 빠뜨리면 0이 아니거나 원복이 안 된다).
...
### DRIFT-176 — 미니맵 층 표시 + 계단 마커 (C-7) 🔷 Phase 1 · 전파 불요
- **근거:** 플랜 §Phase 1이 이 항목을 **「선택이 아니라 필수」**로 적어 뒀다. 스펠렁키식 백레이어가 읽히는 이유의 절반은 back layer가 **작고 명확**했기 때문이고, 「지금 내가 어느 층인지」를 잃으면 재미가 아니라 혼란(`F-024` 인지 예산 위반)이다.
- **① `get_room_rects()`가 `layer`·`room_ref`를 실어 온다.** 미니맵이 **활성 층만** 그릴 수 있게 된다 — 층이 XZ를 공유하니 전부 그리면 겹쳐서 판독이 불가능하다.
  - **반대 방향 주의:** 안개 바운딩은 **전 층 합집합**을 쓴다 → 거기선 필터하지 않는다. 같은 데이터를 두 소비자가 **다른 규칙**으로 읽는다.
  - ⚠️ **정정:** 처음엔 그 이유를 「텍스처가 하나라서 전 층을 덮어야 한다」고 적었는데 **틀렸다.** 한 번에 한 층만 렌더하므로 **층별 바운딩도 가능하다**(각 층이 자기 발자국만 지불). 합집합을 쓰는 실제 이유는 ① **겹치는 층이 전제**라 합집합 ≈ 각 층의 상자여서 **절약분이 0**이고 ② 층별로 하면 전이마다 공유 뷰포트를 리사이즈하고 `_to_fog()` 의존 좌표(오클루더·라이트)를 전부 다시 계산해야 하는데, 바운딩과 텍스처가 어긋나면 **안개가 미터 단위로 밀린다** — 전이는 원자적이어야 하는 경로라 부품을 늘리지 않는다. 애초에 `_compute_bounds()`가 `setup()`에서 한 번만 도는 코드였고 레이어 도입 때 재검토하지 않은 것이 사실이다.
  - **그래서 남는 규칙:** **어떤 층도 다른 층 밖으로 뻗지 않게** 저작한다(뻗으면 그 비용을 모든 층이 함께 낸다 — `bbox_max_m` 게이트가 잰다). 확장 비용의 실체는 바운딩이 아니라 **방문한 층 수 × 탐색 누적 텍스처 1장**(현 맵 기준 장당 2046×2748 ≈ 5.6M px)이다. 누적본이 **첫 방문 때만** 생기는 것이 그 완화책이다.
- **② 계단 마커.** 활성 층의 `transitions` `role: stairs` 앵커를 미니맵에 찍는다 — 「어디서 층을 옮길 수 있나」가 화면에 있어야 한다.
- **③ 층 표시는 여러 층이 있는 맵에서만.** 단층 맵에 「지상」 라벨을 얹으면 노이즈다. `layers_present().size() > 1`일 때만 띄운다.
- **④ 부수 청소:** `map_smoke`가 rect의 `room_ref`를 쓰게 되어 **좌표로 방을 역추적하던 코드가 사라졌다**(O(n²) + 좌표 일치 의존이라 방 중심이 겹치면 오답이 나올 수 있었다).
- **게이트:** 그리기 자체는 검사할 수 없으니 **그릴 목록**(`visible_rects()`)을 본다 — layer 0에서 16방, 활성 층을 1로 바꾸면 **0방**. 계단 마커는 현 맵에 앵커가 없어 0개(신규 그레이박스에서 생긴다).
...
### DRIFT-182 — 제3세력만 층을 넘는다 + **다른 층은 서로 없는 것이다** (D-5) 🔷 Phase 1 · 전파 불요
- **근거:** `F-028` §3.2.2a(`DEC-20260824-001`). 표준 몬스터(`faction: Dungeon`)는 레이어 고정(`F-006` §3.2.4), 제3세력만 계단을 쓴다. 효과는 「파티가 **볼 수 없는 층에서 벌어진 일이 위로 올라온다**」(§3.5 causality LOD).
- **🔴 ① 먼저 발견한 것 — 적대 판정이 층을 안 봤다.** `enemy_ai._is_hostile()`이 진영만 봐서, **layer 1의 적이 layer 0의 파티를 인지**했다. LOS 레이캐스트는 **자기 층 비트**만 쓰므로 남의 층 벽이 광선을 안 막아 준다 — 그건 정상이고(게이트가 이미 「남의 층 벽은 보이지 않는 벽이 되면 안 된다」를 못 박고 있다), **그래서 적대 판정이 걸러야 했다**. 안 고치면 적이 바닥을 뚫고 파티를 보고 영원히 못 닿는 곳으로 몰려간다. 이게 「레이어 고정」의 나머지 반쪽이다.
- **② 넘는 조건은 하나뿐이다 — 자기 층에 적대가 아무도 안 남았을 때.** 이 하나가 두 가지를 동시에 만족한다:
  - **성격**(§3.1.1 유동 위협): 사냥감이 떨어지면 층을 옮겨 찾는다 → 아래층의 사건이 지상으로 올라온다.
  - **anti-pattern 가드**(§3.1.2): **파티가 자기 층에 있으면 안 넘는다.** 층 이동을 「필수 추격」으로 쓰지 않는다 — 못 따라가는 곳으로 도망쳐 목표를 무효화하면 안 된다.
- **③ 계단을 「쓴다」 — 순간이동이 아니다.** 계단까지 **걸어가서**(`THIRD_STAIRS_REACH_M` 안) 넘고, **플레이어가 쓰는 그 실물 계단**을 쓴다(그룹 `interactable` + `on_layer`). 3세력 전용 통로를 따로 두면 데이터에 없는 길이 생긴다. 넘을 땐 **분대 전체가 함께** — 하나씩 새면 「1 분대」 상한이 무의미해진다.
- **🔴 ④ 넘는다는 건 네 가지가 함께 바뀌는 것이다.** `cross_to_layer()`가 **위치 · `nav_layer` · `nav_map_rid` · 기존 경로 폐기**를 한 곳에서 한다(+ `home_pos`도 새 층으로 — 안 그러면 도착 즉시 리시 「끌려왔다」 판정). 하나라도 빠지면 유닛이 **남의 층 navmesh를 걷는다**(구 `get_maps()[0]` 지뢰와 같은 병).
- **🔴 ⑤ 게이트가 내 로머의 틀린 가정을 잡았다.** 처음엔 3세력 유닛 **전부를 한 분대**로 보고 `crew[0].nav_layer`를 「그 층」으로 삼았다. 창발 주입이 겹쳐 분대가 둘이 되는 런에서 그 가정이 깨졌다 — `maxConcurrent` 상한은 **스폰 쪽의 규칙**이지 로머가 기대도 되는 전제가 아니다. → **층별로 갈라** 각 층의 분대를 독립적으로 굴린다. (반증 확인: 되돌리면 3항목이 빨개진다.)
- **⑥ 진영 문자열의 소유자를 유닛으로 옮겼다.** 「층을 넘을 수 있는가」가 `faction == "Third"` 하나에 달려 있어 두 벌이 되면 안 된다 — `EnemyUnit.THIRD_FACTION`을 `CombatController`가 참조한다.
- **반증 확인(6종 전부):** 층 필터를 빼면 남의 층 파티가 적대가 된다 FAIL · 표준 몬스터도 넘게 하면 FAIL · 파티 가드를 빼면 FAIL · nav 바인딩을 빼면 FAIL(**처음엔 안 물었다** — 분대를 layer 0에서 스폰해 이미 목표 rid를 갖고 있어서 검사가 공허했다. rid를 무효로 비우고 시작하도록 고쳤다) · 계단까지 안 걸어가고 넘게 하면 FAIL · 층별 분리를 되돌리면 FAIL.
- **튜닝 수치:** `LAYER_HOP_COOLDOWN_S = 18.0`(층 핑퐁 방지) · `THIRD_STAIRS_REACH_M = 3.0`.
...
### DRIFT-184 — 미결 2건 해소: 심부 관문 잠금 + 테마 축 분리 증명 🔷 Phase 1 · 전파 불요
- **근거:** [[DRIFT-180]] ⑦(데모 맵 `gated_elite` 진입 조건)과 지역 테마 슬러그. 둘 다 「판정 필요」로 남아 있던 것.

**① 데모 맵 `gated_elite` — 카테고리는 옳았고, 빠진 건 잠금뿐이었다.**
- **사실 확인이 판정을 갈랐다.** 스펙 `F-006` §3.2.5는 `gated_elite`를 **진행 게이트**로 정의한다 — *「허브·진행 게이트로 쓰이는 교전(예: 시설 승급 조건)은 `gated_elite`로 두고 확률 추첨에 걸지 않는다」*. `RM-BOSS-01`/`RM-DEEP-01`이 **정확히 그것**이다:
  - `quests.json`: `ENC-BOSS-001 clear once` · `ENC-DEEP-001 clear once` = 허브 승급 퀘스트 완료 조건.
  - `facilities_tiers.json`: `haul_forge_coal`(연료)이 **Deep 분기 전용**(`ENC-DEEP-001` 0.4/런)이라 **대장간 티어 사다리가 여기 걸려 있다**.
  - → 카테고리를 `optional_threat`로 낮추는 안은 **틀렸다**(가중 추첨으로 돌아가면 허브 진행이 확률에 걸린다 — [[DRIFT-176]]이 고친 바로 그 문제).
- **🔴 열쇠를 새로 만들지 않았다.** `LDG-001` §9.1에 `consumeOnUse: false`가 이미 있다 → 두 방에 `requiresItem KEY-DEMO-01` · **`consume_on_use: false`**. 심부 관문은 탈출문과 **같은 열쇠**를 쓰되 먹지 않는다.
  - 먹으면 **심부에 들르는 순간 탈출문이 안 열려 런이 막힌다**. 게이트로 못 박았다: 비소모 문은 열쇠를 안 먹고, 소모 문은 먹는다(과잉 보존도 아님).
  - 설계적으로도 이게 낫다: 열쇠 하나가 「어디에 쓸까」가 아니라 **「어디까지 둘러볼까」의 허가증**이 된다 — 열쇠를 얻은 뒤 *심부에 들를까, 바로 탈출할까*가 실제 선택이 된다.
- **검토했으나 버린 안:** `onObjectiveComplete` — 데모 맵의 목표는 **탈출문 바로 앞에서** 완료되므로, 그걸 조건으로 걸면 심부까지 되돌아가야 한다. 아무도 안 한다 = 사실상 심부 삭제.
- 문 2개(`RM-MID-01`에 앵커 2개, 두 방 모두 이웃이 하나뿐인 막다른 가지) · `require_entry_gate_on_elite: true`로 승격 — 이제 데모 맵도 하드 게이트를 받는다.

**② 테마 축 — 이름은 안 지었다. 「나중에 공짜」를 성질로 만들었다.**
- 지역명은 설계자 몫이므로 발명하지 않았다. 대신 사용자가 말한 *「나중에 스위칭할게」*가 **말이 아니라 성질**이 되게 했다: 게이트가 라벨을 실제로 갈아 보고 **계약(기하·크기·rects·connects·층·조명)이 한 글자도 안 움직이는지** 확인한다.
- **반증 확인에서 내 가드가 뒤집혀 있었다.** 「라벨이 안 바뀜(검사 무효)」을 **옛 값**과 비교하고 있어서, 계약이 라벨을 **아예 안 읽는** 경우에도 통과했다 — 즉 검사가 무의미해지는 바로 그 상황을 못 잡았다. **바뀐 값**과 비교하도록 고치니 문다.
- 임시 라벨 **11/27방**을 매 런 보고한다 — 조용히 출시되지 않게.
- `zone_id` 접두사는 **`ZONE-`로 굳었다**(맵 2개 + 스펙 청사진 `zoneId`). 새 접두사(`CZ-` 등)를 만들지 않는다 — 게이트가 강제.
- **반증 확인(4종):** 항상 소모로 되돌리면 FAIL · 심부 잠금을 빼면 FAIL(+고아 문 = 빈 열쇠 id도 함께 잡힘) · 라벨을 계약에서 떼면 FAIL · `zone_id`를 `CZ-`로 바꾸면 FAIL.
- **영향 파일:** `door.gd`(`consume_on_use`) · `dungeon_run.gd` · `data/slice01/maps/MAP-DEMO-001.json` · `tools/map_smoke.gd` · `docs/design/map_contract.md`.
- **게이트:** `ci_smoke.sh` **15/15 PASS**. [계약/진입] 11항목 · [계약/테마] 2항목.
- **상태:** ✅ 완료 · 전파 불요(스펙이 이미 소유한 어휘의 구현). **미결 0건** — 남은 것은 지역명 확정(설계자)과 `manifest.map_id` 교체 여부(운영 판단)뿐이다.

### DRIFT-185 — 계약이 **문을 알게 됐다** + 진입 즉시 어그로 금지 (Phase 2 ①) 🔷 Phase 2 · 전파 불요
- **근거:** `F-006` §3.2.3이 **grep 0건**이었다. 이유가 분명했다 — 계약이 **문이 어디인지 몰랐다.** 「Room 입구에서 최소 1초」는 문 기준 규칙이라, 문 좌표가 없으면 계산 자체가 불가능하다.
- **① 문은 저작하지 않고 유도한다.** `get_room_openings()` = `connects` + 기하 → 공유벽 위 겹침 구간의 중점. 별도 저작이 없으므로 어긋날 자리도 없고, **게이트가 유도 결과를 실제로 벽에 뚫린 구멍(`_room_openings`)과 대조**한다(30개 일치). 문법(choke/flank)·경로 밴드도 결국 같은 축을 쓴다.
- **② 실측이 설계를 정했다.** 전투존 = `SIGHT_RANGE_M 12 × (1 − 0.2)` = **9.6 m** · 근접 바닥 `PROXIMITY_M` **2.5 m(360°)** · 파티 1초 이동 ≈ **4.0 m**(스펙 초기값). 그래서 두 겹: **6.5 m 무조건**(360°라 등져도 안 통한다) · **13.6 m 방이 허락하면**. 16방 중 15방이 13.6을 낼 수 있고, 못 내는 건 `RM-ROUTE-01`(6×18 복도, 최대 9.5 m) 하나 — **실패시키지 않고 최대에서 포화**한다(그 경우는 §3.2.3의 다른 항목 「좁은 통로 뒤 첫 적은 시야에 들어온 뒤 전투 판정」이 받는다).
- **🔴 ③ facing 축은 버렸다 — 게이트가 그렇게 만들었다.** 처음엔 「문을 맹점(뒤 200°)에 두게 돌린다」로 짰다. 게이트가 즉시 두 유닛을 잡았고, 원인은 `Fixed` 유닛이 **로밍하며 계속 방향을 바꾸는 것** — **스폰 시점의 facing은 한 프레임짜리**였다. 남는 건 거리 축이고, 그래서 **로밍·순회 목표까지** 같은 반경을 통과시킨다.
- **🔴 ④ 「가장 가까운 문에서 방사형으로 민다」도 틀렸다.** 남북에 문이 둘인 방에서는 한 문을 피하면 다른 문에 가까워져 **진동만 하고**, 정작 넓은 좌우로 빠지는 길을 못 찾는다. 방은 박스이고 문은 몇 개뿐이므로 **격자에서 고른다**: 반경을 만족하는 점 중 원래 위치에 가장 가까운 것(스폰 산포 보존), 없으면 최대-최소 거리 점.
- **🔴 ⑤ 목표만 묶으면 안 된다.** 로밍 지점·순회 웨이포인트를 클램프해도 **가는 길**이 문을 스치고, 매복(`AmbushHold`)은 안 움직이는데도 **물리 밀림**으로 서서히 밀린다. → **휴면 틱 끝에서 한 번** 클램프(분기마다 붙이지 않는다). 교전·조사·복귀 분기는 일부러 안 지난다 — 그건 파티를 **이미 감지한 뒤**의 이동이다.
- **⑥ 로밍은 자기 방을 안 떠난다.** 클램프가 거리 조건만 보고 통과시켜서, 문에서 충분히 먼 지점이면 **옆방으로 새어 나가** 남의 방 문 앞에 서 있었다. 방 경계 클램프를 **먼저** 건다.
- **🔴 ⑦ 게이트 자체의 결함 3개를 반증 확인이 잡았다.**
  - **임계값을 구현과 같은 상수에서 읽고 있었다** — `AGGRO_WAKE_BUFFER_M`을 0으로 낮추면 게이트도 같이 내려가 **규칙을 몰래 끄는 것**을 못 잡았다. → **맵이 선언**(`design_targets.aggro_wake_buffer_m`)하고 게이트가 코드 상수와 **대조**한다.
  - **스폰 클램프를 지워도 안 빨개졌다** — 틱 클램프가 한 프레임 만에 되밀기 때문. → **초소(`home_pos`)도 같은 규칙**으로 검사(스폰 클램프의 증인).
  - **「가능한가」를 방 전체에서 재고 있었다** — 클램프는 벽 마진 안쪽만 쓰는데 게이트는 구석까지 세어 「가능한데 안 지켰다」고 잘못 고발했다. → 같은 공간에서 판정.
- **⑧ 대상은 초기 배치뿐이다**(`wake_ruled`). 증원은 전투 중 후방/측면 도착이고 제3세력 창발은 교전 상태로 태어난다 — 둘 다 「모르고 걸어 들어갔을 때」가 아니라, 버퍼를 적용하면 배치 의도를 깬다.
- **튜닝 수치:** `AGGRO_WAKE_BUFFER_M = 4.0`(스펙 초기값, 맵 선언과 대조).
- **반증 확인(4종 전부):** 버퍼 상수를 0으로 → 선언 불일치 + 19기 위반 · 스폰 클램프 제거 → 초소 7건 위반 · 휴면 틱 클램프 제거 → 6기 위반 · 문 유도를 벽 중앙으로 → 벽 구멍 대조 3건 어긋남.
- **영향 파일:** `map_source.gd`(`get_room_openings`) · `enemy_unit.gd`(`wake_clamp`·`wake_ruled`) · `enemy_ai.gd`(휴면 틱 클램프) · `combat_controller.gd`(`_apply_wake_buffer`) · 맵 문서 2개 · `tools/map_smoke.gd` · `docs/design/map_contract.md`.
- **게이트:** `ci_smoke.sh` **15/15 PASS** · 4연속 안정(스폰 시드가 런마다 다르므로 반복 확인).
- **상태:** ✅ 완료 · 전파 불요. 다음 Phase 2: **경로별 교전 밴드**(② `route_class`가 UPPER에만 있다) → `patrolGraphRef`(③) → `exitConvergenceRouteRef`(④).

### DRIFT-186 — 경로가 압력을 소유한다 (Phase 2 ②) 🔷 Phase 2 · 전파 불요
- **근거:** `F-006` §3.10.1 · `DBP-UPPER-001` §8(경로별 교전 1~2 / 3~4 / 4~5). 전역 예산 하나(4~5)면 **조기 탈출로와 심층로의 압력이 같아진다** — 맵 고도화가 만들려던 위험↔보상 축이 그 자리에서 사라진다.
- **① 밴드는 맵이 선언한다**(`design_targets.route_bands`). 선언이 없는 맵(데모)은 **구 동작(전역 예산) 그대로** — 회귀 맵의 전투 분포를 안 건드린다.
- **② 방 하나가 여러 경로에 속한다**(`route_class[]`). 그래서 한 방을 고르면 **그 경로들이 함께** 오르고, 후보 자격은 「**하한 미달 경로를 채우면서** 어떤 경로의 **상한도 안 넘기는**」 것이다. 채울 수 없으면 멈춘다 — 밴드가 불가능해도 **런은 진행**되고, 그 고발은 정적 게이트가 맡는다(런타임이 죽는 게 아니라 데이터가 빨개진다).
- **③ `gated_elite`는 안 센다.** 임계 경로 **밖**의 선택적 관문이라 그 경로의 압력이 아니다 — 예산에서 뺀 것과 같은 이유(`F-006` §3.2.5).
- **④ 활성 맵에선 안 문다 — 그래서 주입으로 검증한다.** 데모 맵은 경로가 하나뿐이라 `route_class`를 **의도적으로 안 적었다**(적으면 「early/mid/deep 경로가 있다」는 거짓 선언이 된다). 그래서 picker를 `pick_with_bands()`로 떼어내 **합성 데이터 + 24시드**로 직접 돌린다.
- **🔴 ⑤ 반증 확인이 내 테스트의 공허함을 잡았다.** 「상한 0인 경로의 방은 안 뽑힌다」를 **한 시드**로만 봤는데, 상한 검사를 꺼도 그 시드는 **우연히 통과**했다(가중 추첨이 마침 안전한 방을 골랐다). 24시드로 넓히니 문다 — 「샌 것: seed 1 O7, seed 2 O7…」. 확률이 섞인 로직은 **한 표본으로 반증 확인이 안 된다**.
- **⑥ 검사가 고정 해답이 아닌지도 본다.** 24시드가 전부 같은 조합을 내면 밴드가 아니라 **유일해**를 검사하는 것이다 — 조합 종류 ≥ 2를 함께 못 박았다(현재 4종).
- **정적 게이트 2종:** 밴드를 선언한 맵은 **전투 방마다 `route_class` 보유**(없으면 영원히 안 뽑힌다) · 밴드가 **기하상 만족 가능**(필수 방만으로 상한 초과 금지 · 가용 방이 하한 이상).
- **반증 확인(4종 전부):** 상한 검사 무력화 → 24시드 중 샘 · 하한 판정 무력화 → 밴드 미달 + 조합 1종 · `route_deep` 하한을 9로 → 「가용 6 < 하한 9」 · 전투 방의 `route_class` 제거 → 미보유 고발.
...
### DRIFT-194 — 도트딜 일괄 -30%: 장판 DoT 너프 패스 🟡 tuning · 전파 불요(로깅만)
- **근거:** 사용자 실플레이 판정 — 「장판딜이 너무 세다, 전체적인 도트딜 너프」. 손잡이 = **원천값 직접 하향**(전역 배율 아님 — 데이터 파일과 실제가 일치해야 유지보수가 정직하다), 강도 = **-30%**.
- **실사(기준 스케일):** 파티 HP 85~205(중앙 ~110) · 적 HP 90~760(중앙 ~240) · 평타 단일대상 dps 5~18(`basic_damage` / `basic_interval_s` 1.0). 이 축에서 재면 `AB-110` 독구름 1회 = **평타 24초분(240)을 1.5초 채널에, 광역으로** 넣고 있었다.

| 자리 | 이전 | 이후 | 1회 총피해 |
|---|---|---|---|
| `abilities.json` AB-039 ToxicGas 존 `dps` | 8.0 | **5.5** | 320 → **220** |
| `skillbooks.json` AB-110 독구름 `poison_dps` | 6.0 | **4.0** | 240 → **160** |
| `reaction_system.FIRE_DPS` | 8.0 | **5.5** | 잔류 40 → **27.5** |
| `surface_grid.FIRE_CREEP_DPS` (미러) | 8.0 | **5.5** | — |
| `outcome_status.DEFAULT_IGNITE_DPS` (미러) | 8.0 | **5.5** | — |
| `hazard_zone.THORN_DMG_PER_M` | 3.0 | **2.0** | 10m 이동 30 → **20** |
| 코드 fallback `poison_dps` (`sb_poison`·`ability_dispatch`) | 8.0 | **5.5** | — |
| 코드 fallback `poison_dps` (`channel_field` cloud) | 6.0 | **4.0** | — |
| 코드 fallback `poison_dps` (`enemy_ai` `enemy_poison`) | 5.0 | **3.5** | 20 → **14** |

- **🔴 같은 값 `8.0`이 3파일에 중복돼 있었다** — `FIRE_DPS`(reaction_system) · `FIRE_CREEP_DPS`(surface_grid) · `DEFAULT_IGNITE_DPS`(outcome_status). 셋은 **같은 불의 같은 dps**인데 상수가 셋이라, 하나만 고치면 「직접 깐 불 5.5 / 번진 불 8.0」처럼 조용히 갈라진다. 이번엔 셋 다 옮기고 **서로를 가리키는 주석**을 달았다. 다음 튜닝 때 한 자리로 합칠 후보(부채 로깅).
- **🔴 구조적 증폭기 2개는 이번에 안 건드렸다**(사용자 선택 = A안·「살짝」). 값 조정으로는 안 사라지므로 남겨 기록한다:
  - **① 독 스택 cap이 dps ×5 배율로 작동한다**(`hazard_zone.POISON_STACK_CAP = 5`). 표기 dps의 5배가 실제 상한이고, `AB-110`은 **0.3s 틱마다 스택**이라 1.5초면 cap 도달 — 상한이 천장이 아니라 **즉시값**이다. 「스택을 쌓는 맛」이 실재하려면 도달 시간이 있어야 한다.
  - **② 화염 잔류가 체류시간과 무관하다**(`surface_grid._apply_medium_outcome` → 매 0.2s 틱마다 `IGNITE_DUR 5.0` **전체 갱신**). **불에 0.2초 발 담근 것 == 5초 서 있은 것** — 나가는 순간 둘 다 잔류 5s 풀타임을 문다. 장판이 부당하게 느껴지는 체감의 진짜 출처가 여기일 가능성이 크다. 고치려면 「잔류 = 체류 누적에 비례(상한 5s)」로 바꿔야 하고, 이는 **tuning이 아니라 rule** → 채택 시 `OPS_30` 전파 후보.
- **의도적으로 유지한 것(사용자 확인 필요 시 되돌릴 자리):**
  - `AB-010` Venom Spit `poison_dps 1.35`(풀스택 54) — 이미 축에서 가장 낮아 -30%를 더 먹이면 무의미해진다.
  - `Tethered` 3 dps — leash **밖일 때만** 도는 조건부 DoT라 피해 출력이 아니라 **CC 준수 압력**이다. 깎으면 CC의 교습 기능이 무뎌진다.
  - DPS 초월 `burn_dps_pct 0.4 × basic_damage`(총 ~19) — 게이지로 벌어들인 정체성 payoff이고 장판이 아니다.
- **영향 파일:** `data/slice01/abilities.json` · `data/slice01/skillbooks.json` · `reaction_system.gd` · `surface_grid.gd` · `outcome_status.gd` · `hazard_zone.gd` · `sb_poison.gd` · `ability_dispatch.gd` · `channel_field.gd` · `enemy_ai.gd`.
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 남은 것:** 수치 검증은 **헤드리스**다. -30%가 실제로 「살짝」인지, 특히 **적 독장판이 여전히 파티원(중앙 110HP)을 220으로 2번 죽인다**는 점은 **실플레이 확인이 필요하다.** 부족하면 ①의 cap을 손대는 것이 다음 레버다.

### DRIFT-195 — 캐스팅 일괄 -20%: 시전 시간 단축 패스 🟡 tuning · 전파 불요(로깅만)
- **근거:** 사용자 실플레이 판정 — 「캐스팅이 전반적으로 너무 길다, 20%만 낮추자」 + **「쿨은 그대로 둔다」**. 즉 축은 **응답성**이지 화력이 아니다 — `cooldown_s`는 한 자리도 안 건드렸다.
- **적용:** `skillbooks.json`의 `cast_s` **25건 전부** ×0.8(0.05초 격자로 스냅). 최장 5.5→4.4 · 최단 1.0→0.8. 25건 중 23건이 정확히 -20%, `1.1→0.9`(-18%)·`1.2→0.95`(-21%)만 격자 스냅 오차.
- **🔴 `unified` 능력 4종은 적 텔레그래프도 함께 줄었다** — `AB-003`·`AB-008`·`AB-010`·`AB-053`. `enemy_ai._begin_windup`이 `unified` 능력의 윈드업을 **스킬북 `cast_s`에서 읽기** 때문이다(적↔아군 파리티 계약, [[DRIFT-082]]). 설계상 의도된 전파지만 **결과적으로 난이도가 올라간다**(적 예고가 20% 짧아짐). 되돌리려면 파리티를 깨야 하므로 값이 아니라 **규칙 판정**이 필요하다 — 사용자 확인 대기.
- **의도적으로 안 건드린 것:**
  - **적 전용 `telegraph_s` 23건**(`abilities.json`) — 예고는 응답성이 아니라 **공정성 신호**다. 줄이면 "내 캐스팅이 빨라진" 게 아니라 **게임이 어려워진다**. 요청의 축과 다르므로 제외.
  - **채널 4종**(`skillbook_channeling` — AB-054 3.25s · AB-109 3.75s · AB-110 3.90s · AB-111 3.85s). 지속이 `cast_s`가 아니라 `ticks × tick_interval_s`이고, 이 레포는 채널을 캐스팅과 **다른 것**으로 분리해 왔다(전용 청록 바·이동중단 규칙, [[DRIFT-115]]). 같이 줄이려면 말씀해 주세요 — 틱 수를 줄일지 간격을 줄일지가 갈린다(전자는 총 피해도 줄고, 후자는 안 줄어든다).
- **⚠ 비율이 조용히 벌어진 자리 2곳(값은 안 건드렸다 — 판정 필요):** 둘 다 **캐스트 길이에 맞춰 설계된 지속**인데 캐스트만 줄어 20% 더 관대해졌다.
  - `AB-062` 오프너 은신 `veil_s` — 게이트 기준이 「누커 최장 시전보다 길 것」이다. 캐스트가 줄었으니 준비 창이 상대적으로 넉넉해졌다.
  - `AB-102` 속박 `root_s` — 게이트 기준이 「DPS 최장 광역기 캐스트 + 0.4s」다. 같은 이유로 여유가 늘었다.
- **🔴 게이트가 의도 대신 튜닝 리터럴을 검사하고 있었다(`party_pool_smoke`).** `AB-035 = 캐스트 커밋 **2.5s**`가 `is_equal_approx(cast_s, 2.5)`였다 — 지키려는 축은 주석에 적힌 「AB-035는 커밋 / AB-051은 즉발」인데, 검사는 **이번 패스의 숫자**를 붙들고 있었다. 튜닝 패스마다 게이트 숫자를 따라 고치면 **게이트가 지키는 것이 없어진다.** → `cast_s >= 1.0`(= "커밋"이라 부를 수 있는 하한)으로 바꿔 아래 `AB-051 = 즉발(== 0.0)`과 **쌍으로** 축을 고정했다. 같은 파일의 나머지 `cast_s` 게이트 6곳은 전부 **상대비교/비율**이라 -20% 균일 스케일에 무사했다 — 리터럴을 쓴 것만 깨졌다는 점이 진단을 뒷받침한다.
  - 덤으로 `AB-053 cast **3.0s** 사다리` 라벨이 실제 2.4s와 어긋나 있었다(검사는 `damage_mult`만 보므로 통과했지만 **라벨이 거짓말을 했다**) → 값을 읽게 고쳤다.
- **반증 확인:** `AB-035` `cast_s` 2.0→0.4로 일시 파괴 → `FAIL AB-035 = 캐스트 커밋(≥1.0s)` 확인 후 복구. 새 게이트가 실제로 축 위반을 잡는다.
- **영향 파일:** `data/slice01/skillbooks.json`(cast_s 25건) · `tools/party_pool_smoke.gd`(게이트 2건).
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 남은 것:** 헤드리스 검증이다. -20%가 실제로 충분한지, 특히 **최장 캐스트 4.4s(AB-066 채널힐)·4.0s(AB-059)가 여전히 긴지**는 실플레이 확인이 필요하다.

### DRIFT-196 — 시전선은 대상에 붙는다: 단일 대상 접근 점선 추종 + 색 분기 + 조준 표식 ✅ 전파 완료(DEC-20260827-001)
- **근거:** 사용자 실플레이 판정 — 「단일 대상 스킬을 사거리 밖 적에게 쓸 때 ① 점선이 적에게 고정돼 적이 움직이면 같이 움직여야 하고 ② 이동(연두)과 구분되게 색을 바꾸고 ③ 적 발밑에 표식을 그려 대상을 인지하게」.
- **🔴 ① 시전 해소는 이미 대상을 따라가고 있었다 — 뒤처진 건 접근과 표시뿐이었다.** `ability_dispatch`는 `p["_target"]`이 살아 있으면 `target_pos`를 **대상의 현위치로 덮어쓴다**([[DRIFT-122]] 단일 대상 잠금). 그런데 `aim_controller._confirm_cast`가 사거리 밖에서 부르는 `order_move_to(target_pos, cb, rng)`의 목적지는 **클릭 순간의 좌표**였다. 결과: 적이 자리를 뜨면 **빈 땅으로 걸어가** 도착해도 사거리에 못 들고(=시전이 헛돌고), 점선은 그 빈 땅을 가리켰다. → 오더에 **추종 대상**(`_order_follow`)을 실어 `order_desired_velocity`가 매 틱 `_order_target`을 대상 현위치로 갱신한다. `follow`는 **기본값 null**이라 좌표 오더(순수 이동·상자·소모품 투척)는 한 줄도 안 바뀐다.
  - **대상이 죽거나 사라지면 `cancel_order()`** — 쫓아갈 이유가 없다. `cancel_order`는 콜백을 안 쏘므로 **시전도 일어나지 않는다**(죽은 적에게 시전이 헛도는 걸 막는다).
- **② 색이 축이 됐다.** 이동선과 시전선이 같은 연두라 「걸어가는 중」과 「쓰러 가는 중」을 구분할 근거가 화면에 없었다. 시전 접근 = **캐스트바와 같은 파랑**(`_cast_bar_color`의 `Color(0.5,0.72,1.0)`) → 파란 점선 → 도착 → 파란 캐스트바로 **한 사슬로 읽힌다**. 색 분기의 근거는 `order_follow_target()` 하나뿐이라 판단이 한 곳에 있다.
- **③ 조준 표식 = 발밑 링 + 회전 브래킷.** 사용자 선택지(테두리 / 발밑 원) 중 **발밑 원**을 골랐다: 테두리는 적 메시마다 `next_pass` 셰이더와 해제 시점 관리가 붙지만, 링은 오버레이가 **매 프레임 다시 그리므로 추종이 공짜**이고 이미 있는 `_draw_ring`과 같은 수법이다. 다만 목적지 원과 같은 그림이면 「여기까지 간다」와 「이 적에게 쓴다」가 안 갈려서 **더 크게(0.75) + 대각 4방향 브래킷 + 회전**으로 벌렸다.
- **🔴 ④ 덤으로 잡은 선행 결함 — 점선이 절대 Y에 고정돼 층 맵에서 허공에 그려졌다.** `GROUND_Y := 0.06`이 **월드 절대값**이라, `layer_floor_y`가 `[0.0, -8.0]`인 `MAP-UPPER-001`에서 1층(y≈-8) 멤버의 점선이 **8m 위**에 떴다(`no_depth_test`라 지워지지도 않고 엉뚱한 높이로 보인다). 이번에 넣는 조준 표식도 같은 자리에 떴을 것이라 함께 고쳤다 → **각 점의 자기 Y + `GROUND_LIFT`**. [[DRIFT-193]] ②(유닛이 층을 안 따랐다)와 같은 계열의 「층을 모르는 좌표」 부채다.
- **파리티:** `AimController`·`MovePathOverlay`를 `dungeon_run`·`combat_sandbox`가 **공유**하므로 배선이 자동으로 양쪽에 든다(sandbox-input-parity). 새 인자는 전부 기본값이라 나머지 `order_move_to` 호출부 4곳(상호작용 2·소모품 1·샌드박스 1)은 무변경.
- **적용 범위:** `single_target: true` **11종**(AB-004/012/013/030/056/057/059/073/100/103/106). 나머지 `UNIT_AIM_KINDS` 7종은 유닛 잠금이 아니라 지면 해소라 추종할 대상 자체가 없다.
- **🔴 ⑤ 반증 확인이 게이트 하나가 공허함을 잡았다.** 「대상 사망 → 오더 취소」가 추종 처리를 빼도 **통과**했다 — nav 분기(`dist<0.05`)가 오더를 대신 끝내 `has_any_order()`가 똑같이 false가 되기 때문이다. 취소와 완료를 가르는 건 **콜백이 쐈는가** 하나뿐이라 둘을 한 검사로 묶었다. [[DRIFT-195]]의 리터럴 게이트와 같은 병(검사가 의도가 아니라 부수효과를 본다).
- **반증 확인:** 추종 갱신 블록 제거 → 「대상이 움직이면 목표가 따라간다」·「대상 사망 → 오더 취소 + 시전 미발화」 **둘 다 FAIL** 확인 후 복구.
- **게이트 [move-order] 신설 7항목:** 추종 노출 · 대상 이동 시 목표 갱신 · cancel 시 추종 해제 · 좌표 오더는 추종 없음(색 분기 근거) · 대상 사망 시 취소+시전 미발화 · **소스핀 4**(조준 확정이 대상을 넘기는가 · 오버레이가 추종으로 색을 가르는가 · 표식을 그리는가 · 점선이 자기 Y 기준인가). 조준 모달(마우스)과 오버레이(렌더)는 헤드리스에서 못 돌아 **배선은 소스로 못박는다**([[DRIFT-119]] 선례) — 상태기계가 옳아도 배선이 끊기면 화면엔 아무 변화가 없다.
- **영향 파일:** `party_member.gd`(`_order_follow`·`order_follow_target`·틱 갱신) · `player_controller.gd`(인자 전달) · `aim_controller.gd`(대상 전달) · `move_path_overlay.gd`(색 분기·조준 표식·Y 기준) · `tools/move_order_smoke.gd`.
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 남은 것(판정 필요):** **표식이 시전 중엔 사라진다.** 도착 → `_finish_order` → 오더 NONE → 오버레이가 그리기를 멈추는데, **바로 그 순간 캐스트바가 시작된다.** 즉 "스킬 사용 중"이라는 피드백이 정작 시전 구간에 없다(사거리 **안**에서 바로 쏘는 경우는 오더 자체가 없어 표식이 아예 안 뜬다). 요청 범위(접근)는 채웠지만 목적("사용 중 피드백")은 절반이다. 이으려면 시전 대상을 멤버 상태로 들어야 한다(`ability_dispatch`가 `cast_s>0`+`_target` 유효 시 set, 완료·취소·사망에서 clear) — **새 상태 + 수명 경계 3개**라 사용자 판정 후 착수.

### DRIFT-197 — 표식은 스킬이 나갈 때까지 남는다: 시전 구간 조준 표식 ✅ 전파 완료(DEC-20260827-001)
- **근거:** 사용자 판정 — 「사거리 안에서 시전돼도 시전 시간 동안 스킬이 **누구한테** 나가는지 알아야 하니, 표식이 **스킬 나가기 전까지** 계속 있어야 한다」. [[DRIFT-196]]이 남긴 ⚠ 항목의 집행이다.
- **🔴 문제:** 196의 표식 수명이 **이동 오더**에 매여 있었다. 접근 오더는 도착하면 `_finish_order`로 풀리는데 **바로 그 순간 캐스트바가 시작**된다 → "쓰러 간다"는 보이고 "쓰는 중"은 안 보였다. 사거리 **안**이면 접근 오더가 아예 없어 표식이 한 번도 안 떴다.
- **해결 — 표식 수명을 「시전 점유」에 매단다.** `begin_channel(dur, target)` / `end_channel()`이 이미 **캐스트의 모든 출구**(완료·취소·이동중단·스턴·시전자 사망)가 예외 없이 지나는 문이다. 여기 하나에 얹으면 **해제 지점을 새로 만들 필요가 없다** — 새 상태의 수명 경계를 3개 늘리는 대신 **이미 옳은 경계를 재사용**했다.
  - `end_channel()`이 완료 시 `_on_complete.call()` **직전**에 불린다 → 표식이 사라지는 시점 = **스킬이 나가는 시점**. 요청 문구 그대로다.
- **자기제한 2겹:** `cast_target()`은 ① `is_channeling()`이 아니면 ② 대상이 무효/사망이면 null을 준다. 점유 타이머는 스스로 소진되므로 `end_channel()`이 어떤 경로에서 누락돼도 표식이 캐스트 길이를 넘겨 남지 않고, 시전 중 적이 죽어도 **시체에 표식이 안 남는다**.
- **표식 형태는 196 그대로**(발밑 링 + 회전 브래킷). 사용자가 이번엔 「테두리」라고 불렀지만 196에서 두 선택지 중 발밑 원을 채택한 그 표식을 가리킨 것으로 읽었다 — 형태를 바꾸면 접근(파란 점선 끝의 표식)과 시전(같은 표식)이 갈라져 **한 사슬**이 깨진다. 접근 → 도착 → 시전 내내 **같은 표식이 끊김 없이 이어진다.**
- **오버레이 구조 변경:** 예전엔 「오더 없으면 `continue`」로 시작해 오더가 없는 멤버는 아예 못 그렸다. 이제 `moving`(오더)과 `casting`(시전 대상)을 **따로 물어** 둘 중 하나라도 있으면 그린다. 시전 중엔 캐스터가 멈춰 있으므로 **점선 없이 표식만**. 접근 중 같은 대상을 이미 그렸으면(`follow == casting`) 겹쳐 그리지 않는다.
- **🔴 반증 확인이 「이 줄은 부하를 안 받는다」를 잡았다.** `end_channel()`의 `_cast_target = null`을 지워도 게이트가 **통과**했다 — `cast_target()`의 `is_channeling()` 가드가 이미 같은 일을 하기 때문이다. 즉 그 줄은 **방어용(참조 조기 해제)이지 기제가 아니다.** 가드까지 함께 지우자 「시전 종료 → 표식 해제」·「점유 소진 → 표식 자동 소멸」이 **둘 다 FAIL** → 게이트는 실제 회귀를 잡는다(기제가 둘이라 하나만 빼면 안 무너질 뿐). 줄은 남긴다(참조를 오래 붙들지 않는 값이 있다) — 다만 **가드가 정본**임을 여기 적어 둔다.
- **게이트 [move-order] 신설 9항목:** 시전 전 없음 · **오더 없이도 시전 시작 시 대상 노출**(사거리 안 경로) · 시전 종료 시 해제 · 점유 소진 시 자동 소멸 · 대상 사망 시 해제 · 대상 없는 캐스트는 표식 없음 · **소스핀 3**(캐스트 노드가 대상을 받는가 · 점유와 함께 싣는가 · 오버레이가 시전 중 표식을 그리는가).
- **영향 파일:** `party_member.gd`(`_cast_target`·`begin_channel(dur,target)`·`cast_target()`) · `skill_cast.gd`(target 인자) · `ability_dispatch.gd`(대상 전달) · `move_path_overlay.gd`(오더/시전 분리 순회) · `tools/move_order_smoke.gd`.
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 남은 것:** 헤드리스 검증이다. 조준 모달·오버레이는 **소스로만 못박혀** 있으므로 **표식이 실제로 시전 내내 붙어 보이는지, 링 크기·회전이 적 발밑에서 읽히는지는 실플레이 확인이 필요하다.** 또 시전 중엔 캐스터→대상 **선을 안 긋는다**(표식만) — 여러 명이 동시에 시전하면 "누가 누구에게"는 아직 안 읽힌다. 필요하면 시전 중에도 파란 점선을 유지하는 것이 다음 레버다.

### DRIFT-198 — 표적이 작지 조준이 나쁜 게 아니다: 단일 대상 관대 선택 ✅ 전파 완료(DEC-20260827-001)
- **근거:** 사용자 실플레이 판정 — 「단일 공격할 때 적을 너무 정확히 찍어야 한다, 관대하게 보정할 수 있나」.
- **🔴 실측이 사용자 편이었다.** 적 몸통은 `BOX_BASE` **0.7m**, 카메라는 기본 거리 19 · FOV 75° → **1m ≈ 37px**이므로 표적이 화면에서 **약 26px**이다. 즉 **중심에서 13px** 안에 커서를 놓아야 했고, 빗나가면 `_pick_enemy_under_mouse()`가 null → **시전이 통째로 취소**됐다(빈 지면 = 무비용 취소). 조준 실력 문제가 아니라 **표적 크기** 문제다.
- **해결 — 정확 우선, 근접 스냅 보조.** ① 레이픽이 맞으면 그것이 답이다(**정밀 조준은 여전히 이긴다** — 겹친 적 사이에서 고를 수 있어야 한다). ② 빗나가면 커서에서 `PICK_SLACK_PX = 40` 안에 **몸통 중심**이 들어온 적 중 화면상 최근접.
- **왜 화면 거리(px)인가 — 월드 반경(m)이 아니라.** 불만은 「마우스를 너무 정확히 놓아야 한다」는 **입력** 문제이므로 보정도 입력 좌표계에서 해야 한다. 월드 반경으로 재면 **줌에 따라 관대함이 달라진다** — 카메라를 밀수록 같은 1m가 화면에선 좁아져 **멀리 볼 때 더 빡빡해진다**. 즉 지금 불편한 상황이 정확히 악화되는 방향이다. 뷰포트가 1920×1080 고정 + `canvas_items` 스트레치라 px는 해상도와 무관하게 안정적이다.
- **제외 규칙 2종:** **안 보이는 적**(안개/LOS) — 스냅이 **투시 수단**이 되면 안 된다(`enemy_unit.is_seen()` 공개 신설). **죽은 적** — 시체 스냅 방지.
- **🔴 스냅이 사거리 판정과 어긋나는 자리를 함께 막았다.** `_confirm_cast`가 거리를 **커서 지면점**으로 쟀는데, 스냅으로 커서와 대상이 최대 ~1m 어긋날 수 있어 「사거리 안이라 즉시 시전 → **실제 대상은 밖**」(반대로 안인데 헛걸음)이 생긴다. 시전 해소(`ability_dispatch`가 대상 위치로 덮어씀)도 접근 목적지([[DRIFT-196]] 추종)도 **이미 대상을 쓰므로**, 판정만 지면점에 남겨 둘 이유가 없었다 → 잠금 시 `aim_pos = unit.global_position`.
- **🔴 반증이 게이트 하나가 자기참조적임을 드러냈다.** 슬랙 경계 검사(0.8×/1.2×)를 `ac.PICK_SLACK_PX`에서 유도했더니 **상수를 2px로 줄여도 오프셋이 같이 줄어 통과**했다. 그 검사가 고정하는 것은 「스냅 반경 == 상수」라는 **기제**일 뿐 값이 쓸모 있는지가 아니다. → **의도 게이트**를 따로 세웠다: 관대 선택의 목적이 「몸통보다 후하게」인 이상 슬랙은 **몸통 반폭(0.35m)의 화면 크기 × 1.5보다 커야 한다.** 튜닝 리터럴이 아니라 **몸통에서 유도**하므로 줌·해상도가 바뀌어도 살아남는다([[DRIFT-195]]의 교훈을 이번엔 처음부터 적용).
- **게이트 [move-order] 신설 12항목 — 이번엔 소스핀이 아니라 실동작이다.** 스냅 계산은 카메라와 그룹만 있으면 순수 함수라, 수직 하향 카메라를 세우면 월드 XZ ↔ 화면 XY가 선형이 되어 **헤드리스에서 픽셀 거리를 손으로 잡을 수 있다**(px/m도 하드코딩 대신 두 적 사이 실측). 검사: 투영 유효 · 적 위 스냅 · **슬랙이 몸통보다 관대** · 슬랙 안/밖 경계 양쪽 · 두 적 중 최근접 · 안 보이는 적 제외 · 죽은 적 제외 · 소스핀 3.
- **덤 — 소스핀 하나를 덜 부서지게 고쳤다.** [[DRIFT-196]]이 박은 `"cb.cast_skillbook(m, slot, target_pos, unit), rng, unit)"`가 이번 `target_pos`→`aim_pos` 리네임에 깨졌다. 핀이 지켜야 할 계약은 「**4번째 인자 = 추종 대상**」 하나인데 위치 변수명까지 붙들고 있었다 — **핀이 회귀 대신 리팩터를 잡는다.** → `contains("order_move_to(")` + `contains(", rng, unit)")`로 완화.
- **반증 확인(3종):** `is_seen` 가드 제거 → 「안 보이는 적 제외」 FAIL · 슬랙 40→12(몸통 수준) → 「슬랙이 몸통 반폭보다 관대하다」 FAIL · 슬랙 40→2 → 경계 검사는 **통과**(자기참조 확인, 위 항목의 근거).
- **영향 파일:** `aim_controller.gd`(스냅·사거리 판정) · `enemy_unit.gd`(`is_seen()`) · `tools/move_order_smoke.gd`.
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 남은 것:** `PICK_SLACK_PX = 40`(기본 줌 ≈1.08m)은 **시작값**이다. 실플레이에서 「빈 땅을 찍었는데 스냅돼 원치 않는 적에게 나갔다」가 나오면 낮추고, 여전히 빡빡하면 올린다 — 게이트는 값이 아니라 **몸통보다 관대한가**만 잡으므로 튜닝 폭이 열려 있다. 또 이 보정은 **아군 대상 스킬(힐·보호막)엔 안 들어갔다** — `_single_target` 스킬 11종이 전부 적 대상이라 이번 범위에 없었을 뿐, 같은 불편이 아군 조준에도 있다면 같은 수법을 쓰면 된다.

### DRIFT-199 — 술어가 이름값을 못 했다: 채널 중 진형 추종에 끌려가 스스로 채널을 끊던 문제 ✅ 전파 완료(DEC-20260827-001)
- **근거:** 사용자 실플레이 판정 — 「채널링 스킬을 쓰고 스왑한 뒤 다른 캐릭터를 움직이면 채널 중이던 캐릭터도 같이 움직이며 스킬이 끊긴다. 채널 도중엔 아군이 움직여도 안 움직이게」.
- **🔴 가드는 이미 있었다 — 술어가 틀렸다.** `party_controller` Pass 1(팔로워)·Pass 3(비조작 앵커), `player_controller`(오더 일시정지) **세 자리 모두** 「채널 중이면 제자리」 분기를 갖고 있었고 주석도 그렇게 적혀 있었다. 그런데 셋 다 `is_channeling()`을 물었고, 그건 **`begin_channel` 점유**만 본다 = `cast_s` 윈드업(`SkillCat`)과 채널힐뿐이다.
  - **정작 「채널링」이라 불리는 스킬**(`sb_channeling` — AB-054 절단 광선 · AB-109 화염 분사 · AB-110 독무 살포 · AB-111 냉기 폭풍)은 **일부러 점유를 안 잡는다**: `sb_channeling.cast()` 주석대로 "채널은 move-lock/occupy가 아니다 — 시전자는 자유롭게 움직이거나 시전할 수 있고, 그러면 **끊긴다**". 등록은 `set_active_channel()`(별도 축)로만 한다.
  - 결과: **이름이 「채널」인 두 개념이 갈라져 있었고**(`is_channeling` = 점유 / `_active_channel` = 진행 중인 채널), 가드는 이름만 보고 **틀린 쪽**을 물었다. 채널링 스킬은 세 가드 전부를 그냥 통과했다.
- **🔴 왜 「이동 = 중단」 규칙을 어기는 게 아닌가.** 그 규칙이 뜻하는 것은 **시전자가 스스로 움직이는 선택**이다. 진형 추종은 선택이 아니다 — 플레이어는 **다른 캐릭터**를 움직였을 뿐인데 채널 중인 멤버가 딸려 오면서 자기 스킬을 끊는다. 즉 규칙이 「내가 움직이면 끊긴다」에서 **「남이 움직이면 끊긴다」**로 새고 있었다. → 막는 것은 **본인이 고르지 않은 이동**뿐이고, **WASD 직접 이동은 그대로 통하며 그러면 채널이 끊긴다**(`player_controller`의 WASD 분기가 가드보다 앞에 있어 규칙이 보존된다).
- **해결:** `is_casting_or_channeling()` 신설 — 점유(`is_channeling()`)든 진행 중인 채널(`_active_channel` 유효)이든 **자동 이동이 끌고 가면 안 되는 상태**를 하나로 묻는다. 세 가드가 같은 술어를 쓴다(하나라도 옛 술어로 남으면 그 경로로 다시 끌려간다 — 게이트가 이걸 잡는다).
  - `_active_channel`은 `channel_field._finish()`가 **`queue_free()` 전에** `clear_active_channel()`을 부르므로 즉시 풀리고, 통보 없이 사라진 경우도 `is_instance_valid()`가 받아 **stale 참조로 영구 정지하지 않는다**(게이트로 확인).
- **파리티:** `combat_sandbox`가 실제 `PartyController`를 그대로 쓰므로(`set_script(PartyController)`) 양쪽에 자동 적용.
- **게이트 [move-order] 신설 8항목:** 평시 false · 점유 → true → 해제 false · **채널 노드 등록 → true(점유 없이도)** · 채널 종료 → false · **노드 소멸 시 자동 해소** · 소스핀 2(세 자동 이동 경로가 **모두** 통합 술어를 쓰고 **옛 술어가 하나도 안 남았는지**).
- **반증 확인:** 술어를 옛 정의(`return is_channeling()`)로 되돌림 → 「채널 노드 등록 → 시전 중(점유 없이도)」 **FAIL** 확인 후 복구.
- **영향 파일:** `party_member.gd`(`is_casting_or_channeling()`) · `party_controller.gd`(가드 2) · `player_controller.gd`(가드 1) · `tools/move_order_smoke.gd`.
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **⚠ 남은 것:** 게이트는 **술어와 배선**까지다 — 「진형 추종이 실제로 채널러를 안 끌고 간다」는 end-to-end는 PartyController에 멤버·앵커·슬롯·진형 config를 다 세워야 해서 **안 걸었다**(이 레포 스모크의 기존 수준을 넘는다). **스왑 후 다른 멤버를 움직여 채널이 유지되는지는 실플레이 확인이 필요하다.** 또 이번 변경으로 채널 중 멤버는 **도발(AB-099) 강제이동에도 안 끌려간다**(가드가 도발 분기보다 앞) — 이는 `cast_s` 캐스트가 이미 그랬던 것과 **같은 거동**이라 일관되지만, 채널을 도발로 끊는 설계를 원한다면 판정이 필요하다.

---

## 2026-08-27 전파 완료 — `DEC-20260827-001` (스펙 핀 `64784c5` → `21ea9b8`)

**전파한 것 (rule):** `DRIFT-196`·`197`·`198`·`199` → 스펙 `staging@21ea9b8`.
- 채널 계열 **공통 계약 4곳**(`AB-054`·`AB-109`·`AB-110`·`AB-111`) — 「이동 = **시전자 본인의 이동 선택**」 명시.
- `F-003` **§3.5.5 신설**(시전·채널 중 자동 이동 정지) · **§3.5.0 추종 오더**(목적지 = 유닛·사거리 판정도 유닛 기준·대상 소멸 시 `cb` 미발화) · **§3.5.3 이동선**(색이 오더 종류를 가름 + 대상 표식이 스킬 나갈 때까지).
- `F-004` §3.1 — 참조만(진형 SSOT는 `F-003`).
- `D-016` §3 **`single_target`** — 「빈 지면」 판정 **허용오차**(정확 우선 → 화면 거리 스냅 · 비가시·사망 제외).

**전파하지 않은 것 (tuning · 전파 금지):** `DRIFT-194`(도트 −30%) · `DRIFT-195`(캐스팅 −20%) 전부, 그리고 위 4건 안의 **수치**(스냅 40 px · 표식 반경 0.75 m · 이동선/표식 색값). 규칙만 올리고 눈금은 게임이 쥔다.

**검증:** `mapper_sync_check --fix` 0건 · `RelationGraph` 재생성 · `spec_xref_check` broken-ref **0** / drift **0**(잔여 NOTE 1건 = `F-025`→`F-023`, **선행 건**·이번 diff 밖).

**⚠ 스펙측 부채 등재:** 채널 계열 공통 계약이 **능력 문서 4곳에 그대로 복제**돼 있어 이번에도 4곳을 함께 고쳐야 했다. 계열 계약은 한 곳에 두고 능력 문서가 참조하는 편이 옳다 — 스펙 `TODO.md`에 등재했다.

**⚠ 미판정 2건(스펙 `TODO.md`):** ① 도발(`AB-099`)이 채널을 끊는가 — 현재는 안 끊는다(시전 캐스트와 같은 거동). ② 아군 대상 스킬의 조준 허용오차 — `single_target` 11종이 전부 적 대상이라 이번 범위 밖.

### DRIFT-200 — 이동선은 「가는 곳」이지 「닿는 곳」이 아니다: 시전 지점 절단 + 착탄 표기 분리 ✅ 전파 완료(DEC-20260827-002)
- **근거:** 사용자 실플레이 판정 — 「범위기를 사거리 밖에서 쓸 때 이동선이 **착탄지점까지** 그려진다. 이동선은 **시전되는 지점**까지로 하고 착탄지점은 따로 표기해라. 이동선은 도달하면 사라지고, 착탄 표기는 **시전 동안 계속**」.
- **🔴 선이 거짓말을 하고 있었다.** 오더는 목표에서 `arrive_dist`(= 시전 사거리, 범위기는 9~16 m)만큼 **못 미쳐 멈추는데** 오버레이는 `order_target()`까지 이어 그렸다. 즉 화면은 「저 착탄점까지 걸어간다」고 말하고 실제로는 **한참 앞에서 선다.** 단일 대상([[DRIFT-196]])에서도 같은 거짓말이 있었지만 표식이 적 발밑에 있어 덜 드러났을 뿐이다 — 범위기는 **목표가 빈 땅**이라 정면으로 보였다.
- **해결 ① 선을 「실제로 서는 곳」에서 자른다.** `_truncate()`가 폴리라인이 목표 반경 `arrive_dist` 원에 **처음 닿는 지점**을 이차방정식으로 풀어 거기서 끊고, **목적지 링을 그 지점에** 찍는다. 판정식이 `order_desired_velocity`의 도착 판정과 **같다**(수평 직선거리 ≤ `arrive_dist`) — 두 곳이 다른 식을 쓰면 선의 끝과 실제 정지점이 어긋난다. 순수 이동 오더는 `arrive_dist` 0.4라 **눈에 띄는 변화가 없다**(같은 규칙이 모든 오더에 걸리되 값이 작아 무해).
  - 「**처음** 닿는 지점」이 중요하다 — 마지막 구간만 보면 꺾인 경로가 목표를 스치고 지나갔다 되돌아오는 경우를 놓친다(게이트에 꺾인 경로 케이스 포함).
- **해결 ② 착탄 표기를 분리한다.** 목적지 링(「여기까지 간다」)과 착탄 표기(「여기에 떨어진다」)는 **뜻이 다르므로 그림도 다르다**: 착탄은 **십자 + 반경 링**. 반경이 없는 형상(rect 존)은 **십자만** 그린다 — 없는 원을 그려 실제와 어긋나게 하느니 점만 찍는 편이 정직하다.
- **해결 ③ 수명 = 접근 오더 → 시전 점유로 이어 붙인다.** 이동선·목적지 링은 **도착과 함께 사라지고**, 착탄 표기는 **시전이 끝날 때까지 남는다**. [[DRIFT-197]]이 단일 대상에 쓴 것과 **같은 수법**(오더에 하나, 점유에 하나)이라 사거리 **안**에서 바로 쏘는 경우도 자동으로 덮인다.
- **🔴 분류는 조준이 소유한다.** `aim_radius`는 `aim_controller`가 조준 프리뷰에 쓰는 `disc`와 **같은 값**에서 나와 접근 오더·시전 양쪽에 실린다. `ability_dispatch`에서 `kind`·반경으로 다시 판정하지 않는다 — 그러면 「조준할 때 본 원」과 「접근·시전 중 보는 표기」가 **조용히 갈라진다**(볼트 4종이 예전에 그랬다). 조준을 안 거치는 경로(AI·적)는 기본값 `-1`이라 표기가 없다.
- **🔴 소스핀이 같은 병으로 세 번째 깨졌다 — 이번엔 규칙으로 적었다.** 핀 2건이 **인자 목록의 끝**을 붙들고 있어(`", rng, unit)"` · `"_cast_charge_color(p), target_unit)"`) 인자를 하나 추가하자 깨졌다. 앞서 [[DRIFT-198]]에서는 **변수명**을 박아 리네임에 깨졌다. 둘 다 **회귀가 아니라 리팩터**를 잡은 것이다. → **핀 작성 규칙**: 변수명도 닫는 괄호도 박지 않고 **「그 인자가 그 호출에 실린다」는 조각만** 본다. 세 핀 모두 교체.
- **게이트 [move-order] 신설 18항목(누적 75):** `_truncate` 실동작 5(도착 반경 절단 · 끝점 ≠ 목표 · **꺾인 경로 첫 진입점** · 반경 0 무변 · 이미 반경 안이면 선 없음) · 착탄 표기 수명 7(평시 없음 · 접근 오더 노출 · 도착 거리 = 시전 사거리 · 오더 해제 시 소멸 · **순수 이동 오더엔 안 붙음** · 시전 중 유지(대상 유닛 없이) · 시전 종료·점유 소진 시 소멸) · 소스핀 6.
- **반증 확인:** `_truncate` 무력화 → 절단 게이트 **3건 FAIL** · `cast_aim_radius` 무력화 → 「시전 중 착탄 표기 유지」 **FAIL**. 각각 복구 후 전 스위트 재확인.
- **영향 파일:** `party_member.gd`(`_order_aim_radius`·`order_arrive_dist()`·`_cast_aim`) · `player_controller.gd` · `aim_controller.gd`(분류 소유) · `ability_dispatch.gd` · `combat_controller.gd` · `skill_cast.gd` · `move_path_overlay.gd`(`_truncate`·`_draw_impact`) · `tools/move_order_smoke.gd`.
- **게이트:** `ci_smoke.sh` 전 스위트 PASS · map smoke DEMO·UPPER 모두 PASS.
- **✅ 전파 완료 — `DEC-20260827-002`(스펙 `a53e0cc`).** `F-003` §3.5.3을 재작성했다: 이동선 끝점 = **정지 지점**(목표 아님) · **착탄 표기 신설**(효과 반경 포함, 반경 없는 형상은 점만) · **수명 분리**(이동선 = 도착 시 소멸 / 표식 = 시전 종료까지) · 「어디에 떨어지는가」 판정은 **조준이 소유**. 같은 날 [[DEC-20260827-001]]이 쓴 *「오더 목표 지점까지」*를 **정정한 것**이다 — 오더는 애초에 목표에 도달하지 않는다. 수치(십자 길이·원 반경·색값)는 전파하지 않았다.
- **⚠ 남은 것:** 헤드리스 검증이다. `_truncate`는 실동작으로 잡았지만 **화면에서 선이 어디서 끊기는지·십자와 반경 링이 착탄점으로 읽히는지**는 실플레이 확인이 필요하다. 또 rect 존(`AB-042`)은 **점만** 표기하므로 복도 형상이 화면에 안 나온다 — 필요하면 rect 프리뷰를 착탄 표기에도 재사용하는 것이 다음 레버다.

---

## 2026-08-27 2차 전파 — `DEC-20260827-002` (스펙 핀 `21ea9b8` → `a53e0cc`)

`DRIFT-200` → `F-003` §3.5.3 재작성. **같은 날 1차 전파(`DEC-20260827-001`)가 쓴 조문을 하루도 못 가 정정**했다 — 그 조문은 *「오더 목표 지점까지 이동선」*이라 했는데 **오더는 목표에 도달하지 않는다**(`arrive_dist`만큼 못 미쳐 멈추는 것이 §3.5.0 심부름 오더의 성립 방식). 스킬 접근에서는 그 값이 곧 시전 사거리라 9~16 m다.
