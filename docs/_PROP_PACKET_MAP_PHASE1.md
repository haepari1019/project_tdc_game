...
| 상태 | 적이 건드리는가 | 돌아왔을 때 |
|---|---|---|
| `down` — **이미 시체** | ❌ 건드릴 수 없다 | 그대로 있다. **회수만 하면 된다** |
| `MIA` — **살아서 이탈** | ✅ **노출된다** | **죽어 있을 수 있다** — 어쩔 수 없다 |

**기본은 `MIA` 유지다.** `down` 즉시 전환은 **설계가 아니라 성능 폴백**이다 — 비활성 레이어에 *조작 불가 생존 유닛*을
유지하는 비용이 감당 안 되면 그때 `down`으로 떨어뜨린다(위치는 보존).
...
| 상태 선택 | **부활시켜 데려갈까 · `down`으로 안전하게 둘까 · `MIA`인 채 두고 갈까** — 마지막이 가장 위험하다 |
