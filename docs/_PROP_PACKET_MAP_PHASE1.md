...
> 레이어 전이는 **전 파티 결집**을 조건으로 하며, 발동 시 **4명 전원**이 함께 이동한다.
> `F-007` §3.6.2 `extractionCohesionRule` · `F-003` `unbound_anchor_max_m`과 **같은 모양**을 재사용한다.

- **다운/MIA 상태에서도 전이는 가능하다**(판정 2026-08-24). 아무도 남겨두지 않는다.
- **MIA 멤버는 전이 시 `down`으로 전환**되어 함께 옮겨진다 — MIA를 다른 레이어에 활성 상태로 유지하는 비용을
  피하고, 동시에 **공짜 구출이 되지 않게** 한다(부활 비용을 치른다).
- 이 규칙이 아키텍처를 지킨다: 파티가 갈릴 수 있으면 **두 레이어를 동시 활성**해야 하지만,
  전원이 항상 한 레이어에 있으면 **활성 레이어가 언제나 하나**다.
- ⚠ **튜닝 감시:** 계단이 **MIA 회수 수단**이 된다(압력판으로 갈린 뒤 계단으로 해소). 공짜는 아니지만 지켜본다.
