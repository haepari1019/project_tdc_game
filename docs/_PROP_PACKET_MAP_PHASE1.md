...
**필요한 가드는 이미 스펙에도 코드에도 있다.** `F-001` §3.6이 **MIA를 스왑 차단 사유**로 규정하고
`PartyController.try_swap_to()`가 **이미 구현**한다(다운도 함께 차단). 다른 레이어의 MIA 멤버로 스왑되면
그 순간 「조작 가능한 파티가 두 레이어에 나뉜」 상태가 되어 전제가 깨지는데, 그 길이 이미 막혀 있다.

> ⚠️ **정정(2026-08-24):** 초안은 「스펙에 있으나 **미구현**」이라고 적었는데 **틀렸다.**
> 근거로 삼은 `run_controller.gd`의 주석(「Control Lock / MIA … neither implemented」)이 낡은 것이었다 —
> `can_swap()`은 **Control Lock만** 보고, MIA·다운 차단은 다른 파일에 있다. 오독의 뿌리는 `F-001` §3.6의
> **자기모순**(예외에 MIA를 적고 아래에서 「Control Lock에서만」이라고 말함)이었고 `DEC-20260824-002`로 해소했다.
> 게임 `DRIFT-170` — 낡은 주석 정정 + **회귀 게이트 신설**(시험이 없었다).
